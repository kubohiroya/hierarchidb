...
### Doing
- #645 / `codex/fix/shape/vt-stage-tasklist-stuck-no-tasks-645` / start: 2026-02-28 22:20 JST
...
## 今日の運用ログ
- update: 2026-02-28 22:22 JST #645 原因は `useShapeBuildTaskSync.resolver.ts` / `useShapeBuildTaskSync.comparison.utils.ts` が同一 `taskId` の旧 completed キャッシュを優先し、新セッションの `running/queued`（`updatedAt` が新しい再実行）を拒否していたこと。修正として `shouldAcceptRestartedTaskTransition` を追加し、`terminal -> running/queued` かつ `next.updatedAt > current.updatedAt` の場合のみ新状態を採用するよう resolver と比較関数を統一。回帰テストを `useShapeBuildTaskSync.comparison.utils.unit.test.ts` に追加。検証: `pnpm -w turbo run test --filter @hierarchidb/shape-plugin -- --run src/ui/__tests__/hooks/unit/useShapeBuildTaskSync.comparison.utils.unit.test.ts` exit 0、`pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin` exit 0。
- start: 2026-02-28 22:20 JST #645 を起票（https://github.com/kubohiroya/hierarchidb/issues/645）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/fix/shape/vt-stage-tasklist-stuck-no-tasks-645` を作成して着手。
