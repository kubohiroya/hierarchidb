...
- 2025-10-06 20:52 progress: chore/ui/dep-fence-warn-cleanup — `pnpm --filter @hierarchidb/ui-datasource build` / `pnpm --filter @hierarchidb/ui-license build` を実行し、いずれもグリーン。`pnpm --filter @hierarchidb/plugins-location-plugin build:types` は `dist-types` 直下への書き込みが sandbox により `EPERM`（tsbuildinfo/宣言ファイル出力）で失敗したため、ユーザー環境での再実行を依頼。
