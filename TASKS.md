...
### Doing
- #587 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-28 01:08 JST
- #586 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-28 00:57 JST
- #585 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-28 00:50 JST
- #584 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-28 00:45 JST
- #583 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-28 00:40 JST
- #582 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-28 00:33 JST
- #581 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-28 00:26 JST
- #580 / `codex/feat/shape/step5-snackbar-overlay-preview` / start: 2026-02-27 21:40 JST
- #579 / `codex/refactor/runtime-worker/get-tree-node-readonly-579` / start: 2026-02-27 21:23 JST
- #578 / `codex/fix/shape/reset-saving-draft-timeout-578` / start: 2026-02-27 21:00 JST
...
## 今日の運用ログ
- start: 2026-02-28 01:08 JST #587 を起票（https://github.com/kubohiroya/hierarchidb/issues/587）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` で着手。
- update: 2026-02-28 01:00 JST #586 Geometry Preview FloatingWindow のタイトルをステージ名付き（`Geometry Preview: Fetch|Transform|VT`）へ変更。加えて表示待ちフィードバックとして wait カーソル制御を追加。原因はタイトルの識別不足と、icon/chip 押下時の表示待ち状態が不明瞭なこと。発生範囲は `TaskItemDetailFloatingWindow` と stage preview open 経路。修正として `TaskItemDetailFloatingWindow` に `stageId` を受け渡しタイトルを段階表示化、controller で `stagePreviewWindowPendingMap` を導入して open 時に短時間 pending を立て、`document.body.style.cursor='wait'` を表示完了まで適用。stage icon button と task list 側にも pending 時 wait カーソルを反映。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
- start: 2026-02-28 00:57 JST #586 を起票（https://github.com/kubohiroya/hierarchidb/issues/586）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` で着手。
- update: 2026-02-28 00:52 JST #585 TaskItemCard Chip の選択可視化を色/背景の入れ替えへ変更。原因は選択状態の視認性不足と、未表示状態で初回選択時にプレビューが見えないこと。発生範囲は `TaskItem`/`TaskItemCard`/`TaskItemCardListCard` と stage overlay の open 経路。修正として `isDetailSelected` を導入し、選択時は status color の塗りつぶし + contrastText で表示、未選択時は従来 outlined を維持。加えて stage overlay に `openStagePreviewWindow` を追加し、ステージ未選択からの初回クリック選択かつウィンドウ非表示時に自動表示するよう変更（クリックした Task を表示）。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
- start: 2026-02-28 00:50 JST #585 を起票（https://github.com/kubohiroya/hierarchidb/issues/585）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` で着手。
- update: 2026-02-28 00:48 JST #584 FloatingWindow の移動範囲をステージ内から画面全体へ拡張し、3ステージ同時表示時の前後関係制御を追加。原因は `TaskItemCardListCard` 内 `position:absolute` で局所座標に拘束されること。発生範囲は `TaskItemDetailFloatingWindow` と stage overlay state 配線。修正として FloatingWindow を `position:fixed` 化し、controller に `stagePreviewWindowZIndexMap` と `bringStagePreviewWindowToFront` を追加してクリック時最前面化を実装。ステージトグルOFF表示仕様は維持し、表示時に自動で最前面化。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- start: 2026-02-28 00:45 JST #584 を起票（https://github.com/kubohiroya/hierarchidb/issues/584）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` で着手。
- update: 2026-02-28 00:42 JST #583 ステージサマリー icon button を disabled 制御からトグル制御へ変更。原因は「表示中に再押下で閉じられない」UX 制約。発生範囲は stage header icon と FloatingWindow 開閉状態管理、および FloatingWindow 配置。修正として `stagePreviewWindowOpenMap` をトグルON/OFF状態として解釈し（デフォルトON）、OFF時にのみ FloatingWindow を表示するよう反転。icon button は `disabled` を撤去し `onClick` でトグル、OFF時に `color=primary` で視覚化。加えて `TaskItemDetailFloatingWindow` にタイトルバーを追加し、タイトルバーの `mousedown` を起点に `mousemove/mouseup` でドラッグ移動可能にした（ドラッグハンドルはタイトルバー限定）。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- start: 2026-02-28 00:40 JST #583 を起票（https://github.com/kubohiroya/hierarchidb/issues/583）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` で着手。
- update: 2026-02-28 00:34 JST #582 同一ステージ内 TaskItemCard の status Chip を click トグル選択に対応。原因は hover-only 制御でプレビュー固定表示ができないこと。発生範囲は `TaskItem`/`TaskItemCard`/`TaskItemCardListCard` の詳細表示連携。修正として click 選択 state（selectedDetail）を `TaskItemCardListCard` に追加し、`selected` がない場合のみ hover プレビューを反映、hover中 Chip border を `primary` 表示、選択中は hover で上書きしない制御へ変更。FloatingWindow は `selected ?? hovered` を表示するよう更新。検証: `pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0、`pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0。
- start: 2026-02-28 00:33 JST #582 を起票（https://github.com/kubohiroya/hierarchidb/issues/582）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` で着手。
- update: 2026-02-28 00:32 JST #581 TaskItemCard の status Chip hover/click 起点 Snackbar/Popper を撤去し、ステージごとの FloatingWindow 表示に移行。`TaskItemDetailSnackbar.tsx` を `TaskItemDetailFloatingWindow` に再編し、ステージヘッダー icon を `IconButton` 化（押下で各ステージ preview window open、表示中は disabled）。`BuildProgressStageContent*` と controller overlay/base state に `stagePreviewWindowOpenMap/open/close` を追加してステージ単位表示制御を実装。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- start: 2026-02-28 00:26 JST #581 を起票（https://github.com/kubohiroya/hierarchidb/issues/581）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` で着手。
- update: 2026-02-27 23:05 JST #580 TaskItemCard 詳細表示に click 固定モードを追加。hover は従来どおり右下 Snackbar、click は Snackbar を閉じてクリック位置 Popper を表示する排他制御を実装。Popper タイトル右端に Download アイコンボタンを追加し、プレビュー中重ね描画（source/original/result）を `FeatureCollection`（`__previewLayer` 付与）として GeoJSON ダウンロード可能にした。`Esc`/外側クリックで Popper close。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- update: 2026-02-27 22:35 JST #580 Step5 TaskItemCard hover Snackbar を2カラム化し、右カラムに Leaflet プレビューを実装。`buildTasks.metadata.preview` へ `fetchCacheId`/`transformCacheId`/`sourceKey`/`dataSource`/`sourceUrl`/`sourceCountryCode`/`fetchCacheFormat` などの描画ヒントを格納し、UI は同 metadata を優先して fetch/transform の元データ・処理結果を重ね描画。Transform では前ステップ元データを 50% 透明度で追加描画。8px fit、パン/ズーム、mouseleave 時 fit リセット、左上/右下の `0kB` サイズラベル表示を追加。
- update: 2026-02-27 22:35 JST #580 検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- start: 2026-02-27 21:40 JST #580 を起票（https://github.com/kubohiroya/hierarchidb/issues/580）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-snackbar-overlay-preview` を作成して着手。
- update: 2026-02-27 21:26 JST #579 原因は `TreeNodeUpdaterService.getTreeNode` が read API でありながら `ensureDraftMetadata(..., true)` / `ensureDraftData(..., true)` により DB write を発生させる実装だったこと。発生範囲は `packages/runtime-worker/src/services/TreeNodeUpdaterService.ts#getTreeNode`。修正として `getTreeNode` 内補完を `persist=false` に変更し read-only 化。返却値の正規化は維持。適用範囲は runtime-worker の当該メソッドのみ。関連テスト `packages/runtime-worker/src/__tests__/headless/draft-initialization.shape.headless.test.ts` を更新し、返却補完は維持しつつ DB 非更新を検証。
- blocked: 2026-02-27 21:26 JST #579 `pnpm -w turbo run test --filter @hierarchidb/runtime-worker --only -- --run src/__tests__/headless/draft-initialization.shape.headless.test.ts` は `src/**/*.headless.test.ts` が default exclude のため exit 1。
- update: 2026-02-27 21:26 JST #579 `WORKER_E2E=1 pnpm -w turbo run test --filter @hierarchidb/runtime-worker --only --env-mode=loose -- --run src/__tests__/headless/draft-initialization.shape.headless.test.ts` を実行し exit 0。あわせて `pnpm -w turbo run build --filter @hierarchidb/runtime-worker --only --output-logs errors-only` exit 0、`pnpm -w turbo run typecheck --filter @hierarchidb/runtime-worker --only --output-logs errors-only` exit 0 を確認。
- start: 2026-02-27 21:23 JST #579 を起票（https://github.com/kubohiroya/hierarchidb/issues/579）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/refactor/runtime-worker/get-tree-node-readonly-579` を作成して着手。
- update: 2026-02-27 21:18 JST #578 原因は `useShapeBuildDraftSaver.ts` が build開始時に `getTreeNode -> updateTreeNode(save-draft)` の順で Worker RPC を行い、reset後の再開始で `saving-draft` 待機が解消されず watchdog timeout（約46s）へ到達すること。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/internal/useShapeBuildStepLogic/useShapeBuildDraftSaver.ts`。修正として `getTreeNode` の事前読み出しを撤去し、UIの最新 `data` を `currentDraftData` として `updateTreeNode(mode: 'save-draft')` を直接実行するよう変更。適用範囲は shape build startup の draft 保存前処理のみ。追加で `plugins/shape-plugin/src/ui/__tests__/hooks/unit/useShapeBuildDraftSaver.unit.test.tsx` を作成し、`getTreeNode` 非依存で save-draft が呼ばれることを検証。
- blocked: 2026-02-27 21:20 JST #578 `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` は差分外既知ブロッカーで exit 2（`TS2307 @hierarchidb/ui-stacked-barchart`、`TS18049 message possibly null`、`TS18048/TS2345 safeProgress`）。
- start: 2026-02-27 21:00 JST #578 を起票（https://github.com/kubohiroya/hierarchidb/issues/578）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/fix/shape/reset-saving-draft-timeout-578` を作成して着手。
...
- done: 2026-02-27 23:01 JST shape step5 TaskItemCard のプレビュー改善（Chip hoverトリガー化、Transformのstatus表記を `Failed: retry N` / `Completed: retry N` へ統一、プレビュー右下サイズラベルを `[orig] → [result]` へ統合、Leafletプレビューのキャッシュデコードを format fallback 対応）を反映し、`pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` と `pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` がともに exit 0。
- update: 2026-02-27 23:40 JST preview `0kB -> 0kB` 対策として、workerの task summary 生成時に `inputData/outputData` から `metadata.preview` を補完（fetch/transform 両方）し、`nodeId` も summary に含めるよう修正。これにより既存タスク（preview未埋め込み）でも cacheId 解決が可能になり、UI 側で実データサイズ取得が可能。`pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` と `pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` は exit 0。
- done: 2026-02-27 23:56 JST Fetchステージ完了時に全fetchタスクの `metadata.fetchDetail` から `featureMax/polygonMax` を集計し、`sessions` レコードへ `fetchStageMaxima` として保存する処理を追加。Snackbar/Popper の Features/Polygons 棒グラフはこの最大値を100%基準にスケーリングし、最大値未保存または0時は棒グラフを描かず数値のみ表示に変更。検証は `pnpm -w turbo run typecheck --filter @hierarchidb/runtime-worker --filter @hierarchidb/shape-plugin --filter @hierarchidb/shape-api --filter @hierarchidb/shape-store --only` と `pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` で exit 0。
- done: 2026-02-28 00:08 JST Transform TaskItemCard message欄のミニ棒/円グラフを撤去。Leafletプレビューを 1:1 正方形化し、表示ズームから推定した xyz タイル境界線とタイル左上ラベル（x/y/z）を重ね描画。サイズ表示は `[処理後 / 元]` の固定幅バー＋重ねテキストへ変更し、元サイズが0または最大値未保存時は棒グラフを描かず数値のみ表示に統一。検証は `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --filter @hierarchidb/runtime-worker --filter @hierarchidb/shape-api --filter @hierarchidb/shape-store --only` と `pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` で exit 0。
- update: 2026-02-28 01:11 JST #587 Geometry Preview FloatingWindow の右カラムを正方形固定プレビューへ調整し、GeoJSON 不在時も同サイズプレースホルダーを表示してウィンドウ寸法の揺れを防止。原因は描画成功時と失敗時で preview 領域サイズ計算が分離していたことと、右下角のみのリサイズ実装で要件の「エッジドラッグ」に不足があったこと。発生範囲は `TaskItemDetailSnackbar.tsx` の `TaskDetailContent` と `TaskItemDetailFloatingWindow`。修正として preview を `previewSide`（縦横同値）で描画/プレースホルダー共通化し、FloatingWindow に上下左右＋四隅のリサイズハンドルを追加、方向別に位置・サイズを再計算するよう更新。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
- update: 2026-02-28 01:14 JST #587 Geometry Preview FloatingWindow の本文レイアウトを左右2カラムから上下2段へ変更。原因は右カラム固定幅+正方形制約で、ウィンドウ下方の空き領域を活かせなかったこと。発生範囲は `TaskItemDetailSnackbar.tsx` の `TaskDetailContent` と `GeometryPreviewMap`。修正として上段へ指標表示を集約し、下段 preview を `flex:1` で残り領域に追従させ、`aspectRatio: 1/1` を撤去して高さ可変化。GeoJSON不在時も同サイズプレースホルダーを表示するよう維持。さらに `ResizeObserver` で Leaflet の `invalidateSize` を呼び、ウィンドウリサイズ追従を安定化。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
- update: 2026-02-28 01:14 JST #587 Geometry Preview の Leaflet `+/-` コントロールを右上へ移動し、表示サイズを縮小。原因は既定配置/既定サイズがプレビュー領域に対して視認ノイズが大きかったこと。発生範囲は `TaskItemDetailSnackbar.tsx` の `GeometryPreviewMap` 初期化と container style。修正として Leaflet map 初期化時に `zoomControl:false` + `L.control.zoom({ position: 'topright' })` を適用し、`.leaflet-control-zoom` と `a` 要素のスケール/寸法を小さめに上書き。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
- update: 2026-02-28 01:20 JST #587 Geometry Preview FloatingWindow タイトル左端にステージアイコンを追加。原因はタイトル文字列だけではステージ判別が遅く視認性が不足していたこと。発生範囲は `TaskItemDetailSnackbar.tsx` のタイトルバー描画。修正として `fetch/transform/vt/other` でアイコンを解決する `resolveStageIcon` を追加し、タイトル先頭に表示。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
- update: 2026-02-28 01:23 JST #587 Geometry Preview タイトル左端アイコンの Transform/VT 誤りを修正。原因は FloatingWindow 側で独自の stage→icon マッピング（`AutoFixHigh`/`GridView`）を固定しており、実際のステージ定義アイコンと不一致だったこと。発生範囲は `TaskItemDetailSnackbar.tsx` タイトルアイコン解決処理。修正として独自マッピングを撤去し、`buildStagesAtom` の `stage.icon` を `stageId` で解決して表示する実装へ変更（未定義時のみ `Layers` をフォールバック）。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
- update: 2026-02-28 01:41 JST #587 Features/Polygons 棒グラフの最大値スケーリング不一致を修正。原因は (1) UI 側が input/output を重ね描きせず単一バー表示だったこと、(2) Fetch stage maxima の polygons 集計が「1フィーチャーあたり最大ポリゴン数」を保持できていなかったこと。発生範囲は `TaskItemDetailSnackbar.tsx` の `renderStackedRatioRow`、`shapeFetchStage.ts` の fetchDetail/build metadata、`shapePipelineFetchStage.ts` の maxima 集計。修正として棒グラフを灰色 input + 結果色 output の左詰オーバーレイ表示へ変更し、Fetch 処理で `maxPolygonPerFeature` を算出して `fetchDetail.polygonsPerFeature` / cache metadata に保存、Fetch 完了時 maxima も `polygonsPerFeature` 優先で集計するよう更新。併せて fetch summary builder は `fetchDetail.polygonsPerFeature` を優先参照。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0（7 tests passed）。
