...
## 今日の運用ログ
- 2026-03-06: Issue #827 完了 - 認証トークンの有効期限事前検証機能を実装し、タイムアウト待機を回避する改善を完了。`resolveStoredToken()`にトークン有効期限チェック機能を追加（`token_expires_at`とJWT `exp`フィールドの両方を検証）、期限切れトークンは即座に`null`を返してストレージからクリア、詳細なデバッグログを追加。`pnpm -w turbo run typecheck --filter @hierarchidb/auth`と`pnpm -w turbo run build --filter @hierarchidb/auth`が成功（exit 0）。コミット`2cc98265b`でプッシュ完了。これにより、期限切れトークンでのAuthentication Requiredダイアログの遅延が解消される。
