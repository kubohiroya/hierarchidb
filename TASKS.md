...
### Doing
- #564 / `codex/fix/ui/stage-resume-queued-snapshot-order` / start: 2026-02-26 11:09 JST
- #563 / `fix/worker/fetch-auth-timer-resume-563` / start: 2026-02-26 10:33 JST
- #562 / `codex/feat/shape/step5-task-summary-snackbar-562` / start: 2026-02-26 09:06 JST
- #561 / `codex/fix/shape-step5-input-transform-effective-tolerance-561` / start: 2026-02-26 08:28 JST
...
- #564 / `codex/fix/ui/stage-resume-queued-snapshot-order` / blocked: 2026-02-26 11:16 JST（DoD検証で実行した `pnpm lint && pnpm format && pnpm typecheck && pnpm test` が `pnpm format` 内の差分外既知エラー `@hierarchidb/plugin-base#format`（`src/types/PluginDBQueryAPI.ts` ほか `noExplicitAny`/`noBannedTypes`）で exit 1）
...
## 今日の運用ログ
- update: 2026-02-26 11:16 JST #564 ステージ再開時の誤完了を修正。原因は `useShapeBuildTaskSync.resolver` が terminal確定タスクを常に優先し、Worker側再開で `failed/skipped -> queued` へ戻す更新をUIが握り潰していたこと、および `useShapeBuildStepStageState` の terminal完了通知が in-flight タスク/再開トランジションを考慮せず即時にセッション完了化できたこと。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/useShapeBuildTaskSync/useShapeBuildTaskSync.resolver.ts`、`.../internal/useShapeBuildStepStageState.ts`、`.../internal/useShapeBuildStepLogic.impl.ts`。修正方法は (1) terminalキャッシュのうち `failed/skipped` は `queued/running` 更新を許可、(2) terminal完了通知を `hasInFlightTasks` 中は抑止、(3) `buildSessionTransition.active` 中は `onTerminalStageCompletion` で sessionRecord確定しないガード追加。再現テストとして `useShapeBuildTaskSnapshotProgressState.unit.test.tsx` に retry再キュー2ケースを追加。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/hooks/unit/useShapeBuildTaskSnapshotProgressState.unit.test.tsx -t "allows retry queue updates"` exit 0。全体検証 `pnpm lint && pnpm format && pnpm typecheck && pnpm test` は差分外既知 `@hierarchidb/plugin-base#format` で exit 1（Blockedへ記録）。
- start: 2026-02-26 11:09 JST #564 を起票（https://github.com/kubohiroya/hierarchidb/issues/564）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/fix/ui/stage-resume-queued-snapshot-order` を作成して着手。
- done: 2026-02-26 10:57 JST #563 Fetch認証割り込み時の計時/再開と不当Failed化を修正。原因は (1) `runStageTasks` が `Authentication required` を通常失敗として `failed` 化していたこと、(2) `shapePipelineFetchStage.ts` が pending task を一律 `aborted: fetch stage completed with pending tasks` で failed 化していたこと、(3) UI側で認証通知に連動した build pause/resume がなかったこと。発生範囲は `packages/vt-orchestrator/src/compareTaskOrder.ts`、`plugins/shape-plugin/src/services/vt/shapePipelineStageHelpers.ts`、`plugins/shape-plugin/src/services/vt/shapePipelineFetchStage.ts`、`plugins/shape-plugin/src/worker/api/api-internal-execution-core.ts`、`app/src/contexts/useAuthRequiredDialogHost.ts`。修正方法は auth required を `queued+authState=required` へ再キューし、pending最終化では auth pending を failed 化せず、fetchステージでは auth pending を `FetchStageAuthPendingError` として通常失敗と分離、UIで `AUTH_REQUIRED` 時 pause / `AUTH_SUCCESS` 時 resume を自動実行する構成。適用範囲は shape fetch stage と共通 runStageTasks、認証ダイアログホスト。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/vt-orchestrator --filter @hierarchidb/shape-plugin --filter @hierarchidb/app --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/__tests__/unit/shapePipelineFetchStageSection.unit.test.ts src/__tests__/unit/shapePipelineStageHelpers.unit.test.ts` exit 0、`pnpm -w turbo run test --filter @hierarchidb/vt-orchestrator --only -- --run src/__tests__/runStageTasks.auth.unit.test.ts` exit 0、`pnpm -w turbo run build --filter @hierarchidb/vt-orchestrator --filter @hierarchidb/shape-plugin --filter @hierarchidb/app --only --output-logs errors-only` exit 0。
- update: 2026-02-26 10:52 JST #562 追補。Fetchステージ詳細Snackbarに要件項目を追加。原因は従来のfetch表示が簡易summaryのみで、URLと小島省略後/元データ比（feature/polygon）が確認できなかったこと。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/TaskItemCard/taskOutcomeSummaryBuilders.ts`、`.../TaskItemCard/TaskItemDetailSnackbar.tsx`、`.../TaskItemCard*.tsx`、`plugins/shape-plugin/src/services/vt/shapeFetchStage.ts`。修正として fetch 完了時に `metadata.fetchDetail`（country/code/adminLevel/url/features/polygons の input/output）を保存し、Fetch専用summary builderを追加、Snackbarで `URL`・`Features output/input(%)`・`Polygons output/input(%)` を3桁カンマ付きで表示するよう変更。適用範囲は shape-plugin Step5 task card の fetch 詳細表示のみ。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- start: 2026-02-26 10:33 JST #563 を起票（https://github.com/kubohiroya/hierarchidb/issues/563）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `fix/worker/fetch-auth-timer-resume-563` を作成して着手。
- update: 2026-02-26 10:20 JST #562 追補。TaskItem詳細可視化ブロックを要件に合わせて再構成。原因は既存詳細がミニ棒/ドーナツ中心で、数値把握（実効トレランス・retry現在/最大）と「最終/元」の3行比較要件を満たしていなかったこと。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/TaskItemCard/TaskItemDetailSnackbar.tsx`。修正として Transform詳細を「Effective tolerance（数値）」「Retry attempts（現在/最大の数値）」表示へ変更し、可視化は Features/Polygons/Vertices の3行100%横棒へ統一。各行は `最終 / 元 (率%)` を棒内テキスト表示（3桁カンマ区切り）。頂点行には固定上限 `6553` の warning 線を常時表示。ドーナツと補助テキストは削除。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- update: 2026-02-26 09:49 JST #562 追補。TaskItem 詳細を「TaskItem外」へ移設するため、`TaskItem` 内部にあった Snackbar を撤去し、`TaskItemCardListCard` で単一の `TaskItemDetailSnackbar` を管理・描画する構成へ変更。原因は TaskItem 行ごとの Snackbar 描画が見た目上 Tooltip 風の挙動になり、固定位置表示という要件と乖離していたこと。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/TaskItem/TaskItem.tsx`、`.../TaskItemCard/TaskItemCard.tsx`、`.../TaskItemCardListCard/TaskItemCardListCard.tsx`。修正方法は hover情報のコールバック伝播（TaskItem→TaskItemCard→TaskItemCardListCard）と、外部Snackbarコンポーネント `TaskItemCard/TaskItemDetailSnackbar.tsx` の新設。適用範囲は shape-plugin Step5 タスクカード詳細表示経路のみ。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- update: 2026-02-26 09:37 JST #562 追補。詳細説明を Snackbar へ統一する要件に合わせ、`BuildProgressStageContentView` のスクロール矢印に残っていた Tooltip を撤去し `aria-label` のみへ変更。原因は TaskItem 側の Tooltip 撤去後も同画面内別コンポーネント（矢印ボタン）で Tooltip が残存していたこと。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/ShapeBuildProgressPanel/BuildProgressStageContent/BuildProgressStageContentView.tsx`。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- update: 2026-02-26 09:35 JST #562 追加修正。原因は先行差分で TaskItem の可視化を全ステージへ一律適用したため、Transform専用の統計指標を持たない Fetch/VT タスクで `N/A` が多発したこと。発生範囲は `TaskItemCard`→`TaskItem` のサマリー生成/描画経路（`plugins/shape-plugin/src/ui/components/build-progress/TaskItemCard/*.tsx` および `.../TaskItem/TaskItem.tsx`）。修正方法としてステージ別サマリー注入ポイントを追加し、`TaskItemCard` へ `summaryBuilder`、`TaskItemCardListCard` へ `summaryBuilders` マップを導入。Transform は `buildTransformTaskOutcomeSummary`（可視化あり）を注入、Fetch/VT は `buildSimpleTaskOutcomeSummary`（簡易表示、可視化なし）を注入する構成へ変更。適用範囲は shape-plugin Step5 タスクカード表示ロジックのみ。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- update: 2026-02-26 09:12 JST #562 Shape Step5 TaskItemCard の message 表示を平文+Tooltip から状態別サマリーへ置換。原因は旧実装が 1 文字列の切り詰め表示に依存し、Completed/Skipped/Failed ごとの判断材料（実効トレランス・リトライ・最終データ量・縮減率）を即時に読めなかったこと。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/TaskItemCard/TaskItemCard.tsx` と `.../TaskItem/TaskItem.tsx`。修正方法は `TaskOutcomeSummary` を導入し、TaskItemCard 側で metadata/display から状態別サマリーを構築、TaskItem 側で 1行サマリー+ミニ可視化（Tol/Retry/Final F/P/V + vertex limit線 + vertex削減率ドーナツ）とホバーSnackbar詳細表示へ置換。適用範囲は shape-plugin の Step5 タスクカードUI。検証: `pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0。
- start: 2026-02-26 09:06 JST #562 を起票（https://github.com/kubohiroya/hierarchidb/issues/562）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/feat/shape/step5-task-summary-snackbar-562` を作成して着手。
- update: 2026-02-26 09:03 JST #561 再再修正。原因は PluginDialog 共通のクリックフォーカス遷移で、入力要素への pointerdown 後にフォーカスが安定しない経路があり、Step1（name/description）と Step5（Search tasks）の双方に波及していたこと。発生範囲は `packages/ui/dialog/src/headless/usePluginDialogFrame.ts` を経由する全プラグインダイアログ入力欄。修正方法として `handleFramePointerDown` にテキスト入力ターゲット判定（`input/textarea/contenteditable/role=textbox`）を追加し、マイクロタスクで `focus({ preventScroll: true })` を明示補正する共通ガードを実装（disabled/readOnly は除外）。適用範囲は PluginDialog 共通フレーム層（`@hierarchidb/ui-dialog`）のみ。検証: `pnpm -w turbo run test --filter @hierarchidb/ui-dialog --only -- --run src/headless/PluginDialogFrame.inputFocus.unit.test.tsx` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/ShapeBuildProgressPanelControlRightContent.unit.test.tsx src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0、`pnpm -w turbo run typecheck --filter @hierarchidb/ui-dialog --filter @hierarchidb/ui-search-input --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0。
- update: 2026-02-26 08:49 JST #561 再修正。原因(1) Step5 検索欄はダイアログ内ポインタイベント連鎖時に input focus が確立されない経路があり、発生範囲は `@hierarchidb/ui-search-input` を利用する `TreeTableSearchInput`。修正方法として `packages/ui/search-input/src/SearchInput.tsx` で `onMouseDownCapture/onPointerDownCapture` の伝播停止に加え `inputRef` を用いた明示 focus をマイクロタスクで実行するよう変更。適用範囲は同コンポーネントのみ。原因(2) Transform `effectiveTolerance` 表示は progress message 解析に依存していたため、metadata-only 方針と不整合。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/TaskItemCard/TaskItemCard.tsx` の suffix 生成。修正方法として message 解析を撤去し、`task.metadata`（`effectiveTolerance/effective_tolerance/finalTolerance/finalEffectiveTolerance` と nested metadata）だけで解決するよう変更。適用範囲は Transform タスクの表示ロジック。検証: `pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/ShapeBuildProgressPanelControlRightContent.unit.test.tsx src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0、`pnpm -w turbo run typecheck --filter @hierarchidb/ui-search-input --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0。
- update: 2026-02-26 08:34 JST #561 Shape Step5 検索入力不可の原因はダイアログ配下ポインタイベント干渉で入力フォーカスが阻害される経路にあり、発生範囲は `TreeTableSearchInput` を使う Step5 右上検索欄。`packages/ui/search-input/src/SearchInput.tsx` に `onMouseDownCapture/onPointerDownCapture` の伝播停止を追加して入力フォーカスを維持するよう修正。Transform message の `effectiveTolerance` は `TaskItemCard` が `effectiveTolerance=` 形式のみを抽出しており `finalTolerance=` や `:` 区切りを拾えなかったため、`plugins/shape-plugin/src/ui/components/build-progress/TaskItemCard/TaskItemCard.tsx` で抽出正規表現とフォールバックキーを拡張（適用範囲: Transform タスク表示 suffix 生成）。検証: `pnpm -w turbo run test --filter @hierarchidb/shape-plugin --only -- --run src/ui/__tests__/components/build-progress/ShapeBuildProgressPanelControlRightContent.unit.test.tsx src/ui/__tests__/components/build-progress/TaskItemCardListCard.unit.test.tsx` exit 0、`pnpm -w turbo run typecheck --filter @hierarchidb/ui-search-input --filter @hierarchidb/shape-plugin --only --output-logs errors-only` exit 0。
- start: 2026-02-26 08:28 JST #561 を起票（https://github.com/kubohiroya/hierarchidb/issues/561）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/fix/shape-step5-input-transform-effective-tolerance-561` を作成して着手。
