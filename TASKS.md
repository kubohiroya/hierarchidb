...
### Doing
- #648 / `codex/fix/shape/reset-start-empty-snapshot-premature-complete-648` / start: 2026-02-28 22:38 JST
...
## 今日の運用ログ
- update: 2026-02-28 22:41 JST #648 原因は `resolveReceivingTaskSnapshotDecision` が `hasReceivingTaskSnapshotSignal && taskCount===0` を buildStatusに関係なく即 `completed-without-generating-tasks` と判定していたこと。発生範囲は `plugins/shape-plugin/src/ui/components/build-progress/resolveReceivingTaskSnapshotDecision.ts` と関連unit。修正として「empty snapshot で no-task completion を返すのは `buildStatus==='completed'` のときだけ」に制限し、`running` 中は `continue` へ変更。回帰テストを更新し、`running+empty snapshot` は継続待機、`completed+empty snapshot` のみ成功確定を検証。検証: `pnpm -w turbo run test --filter @hierarchidb/shape-plugin -- --run src/ui/__tests__/hooks/unit/resolveReceivingTaskSnapshotDecision.unit.test.ts` exit 0、`pnpm -w turbo run test --filter @hierarchidb/shape-plugin -- --run src/ui/__tests__/components/build-progress/ShapeBuildProgressPanel.unit.test.tsx` exit 0、`pnpm -w turbo run typecheck --filter @hierarchidb/shape-plugin` exit 0。
- start: 2026-02-28 22:38 JST #648 を起票（https://github.com/kubohiroya/hierarchidb/issues/648）し、Project `hierarchidb` へ追加後 Status を `In Progress` に設定。ブランチ `codex/fix/shape/reset-start-empty-snapshot-premature-complete-648` を `origin/main` 起点で作成して着手。
