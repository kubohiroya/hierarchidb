...
- fix/ui-dialog/fullscreen-resize-sync — Fullscreen モード遷移後にダイアログサイズを再測定
  ...
  - 運用ログ：
    ...
    - progress: 2025-09-21 23:59 TrashDialog の listChildren 結果処理を見直し、`*:trash` 直下では孫ノードのみを UI へ渡し placeholder を隠蔽（`holderLookup` を用いて実ノードだけを抽出）。`pnpm -C app typecheck` を再実行し成功
