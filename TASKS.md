...
- fix/ui-dialog/fullscreen-resize-sync — Fullscreen モード遷移後にダイアログサイズを再測定
  ...
  - 運用ログ：
    ...
    - progress: 2025-09-21 23:59 TrashDialog の listChildren 結果処理を見直し、`*:trash` 直下では depth=1 の孫ノードのみを TreeTable に流すようフィルタリング（placeholder は UI から隠蔽）。`pnpm -C app typecheck` を再実行し成功
...
- 2025-09-20: runtime env 読み出しの共通化 — `@hierarchidb/util` に `readRuntimeEnvValue` / `readRuntimeMode` などのヘルパーを追加し、app / map-adapter / node-type plugins からの `process.env` 参照を排除。`eslint.config.js` の `no-restricted-globals: process` を `app/src` と `packages/**/src` へ拡張し、`pnpm --filter` {app,node-type-route-plugin,node-type-location-plugin,node-type-resolver-plugin,map-adapter} `typecheck` を実行してグリーンを確認。
