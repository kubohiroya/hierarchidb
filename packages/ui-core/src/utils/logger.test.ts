...
describe('Logger Utilities', () => {
  ...
  describe('正常系テストケース', () => {
    ...
    it('開発環境でエラーログが正しく出力される', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('[ErrorModule Error]', 'Error occurred', testError); // 【確認内容】: エラープレフィックスとエラー詳細が正しく渡されているか 🟢
    });
  ...
  describe('境界値テストケース', () => {
    ...
    it('非常に長いプレフィックスでも正常動作', () => {
      ...
      process.env.NODE_ENV = 'development';
      const longPrefix = 'VeryLongModuleNameForTestingPurposesWithMoreThan100CharactersToCheckIfTheLoggerCanHandleLongPrefixesWithoutAnyIssues';
