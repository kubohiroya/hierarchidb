...
interface TreeSchema {
  ...
  name: unknown;
  rootNodeId: unknown;
  trashRootNodeId: unknown;
  superRootNodeId: unknown;
}
...
export class ValidationError extends Error {
  constructor(field: string, expected: string, received: unknown) {
    super(`Validation failed for field '${field}': expected ${expected}, received ${typeof received} (${received})`);
...
export function deserializeTreeNode(data: unknown): TreeNode {
  ...
  if (typeof obj.parentNodeId !== 'string') {
    throw new ValidationError('parentNodeId', 'string', obj.parentNodeId);
  }
  ...
  const treeNode: TreeNode = {
    ...
    parentNodeId: createNodeId(obj.parentNodeId),
  ...
  if (obj.originalParentNodeId !== undefined && typeof obj.originalParentNodeId === 'string') {
    treeNode.originalParentNodeId = createNodeId(obj.originalParentNodeId);
  }
...
export function deserializeTree(data: unknown): Tree {
  ...
  if (typeof obj.rootNodeId !== 'string') {
    throw new ValidationError('rootNodeId', 'string', obj.rootNodeId);
  }
  if (typeof obj.trashRootNodeId !== 'string') {
    throw new ValidationError('trashRootNodeId', 'string', obj.trashRootNodeId);
  }
  if (typeof obj.superRootNodeId !== 'string') {
    throw new ValidationError('superRootNodeId', 'string', obj.superRootNodeId);
  }
  
  return {
    id: createTreeId(obj.id),
    name: obj.name,
    rootNodeId: createNodeId(obj.rootNodeId),
    trashRootNodeId: createNodeId(obj.trashRootNodeId),
    superRootNodeId: createNodeId(obj.superRootNodeId),
  };
}
...
export function serializeTreeNode(node: TreeNode): Record<string, unknown> {
  return {
    ...
    originalParentNodeId: node.originalParentNodeId as string | undefined,
...
export function serializeTree(tree: Tree): Record<string, unknown> {
  return {
    ...
    name: tree.name,
    rootNodeId: tree.rootNodeId as string,
    trashRootNodeId: tree.trashRootNodeId as string,
    superRootNodeId: tree.superRootNodeId as string,
  };
}
