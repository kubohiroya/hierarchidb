...
describe('Auto Lifecycle Management System Integration', () => {
  ...
  const styleMapNodeType = 'stylemap' as TreeNodeType;
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle database transaction failures gracefully', async () => {
      ...
      vi.spyOn(db, 'transaction').mockImplementation((...args: any[]) => {
        return originalTransaction.apply(db, ['rw', ['styleMapEntities'], async () => {
    ...
    it('should clean up properly on partial failures', async () => {
      ...
      await expect(
        styleMapHandler.commitWorkingCopy(nodeId1, workingCopy)
