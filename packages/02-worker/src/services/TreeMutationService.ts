import type { TreeMutationAPI } from '@hierarchidb/01-api';
import type {
  CommandEnvelope,
...
export class TreeMutationService implements TreeMutationAPI {
  ...
  private async getParentId(nodeId: NodeId): Promise<NodeId> {
    ...
    return node?.parentNodeId || ('' as NodeId);
  }
  ...
  async createWorkingCopyForCreate(
    ...
    const { workingCopyId, parentNodeId, name, description } = cmd.payload;
  ...
  async commitWorkingCopyForCreate(
    ...
    try {
      ...
      const newNode: TreeNode = {
        id: newNodeId,
        parentNodeId: workingCopy.parentNodeId,
  ...
  async moveNodesCommand(cmd: CommandEnvelope<'moveNodes', MoveNodesPayload>): Promise<CoreCommandResult> {
  ...
  async recoverFromTrash(
    ...
    try {
      ...
      for (const nodeId of nodeIds) {
        ...
        const targetParentId = toParentId || node.originalParentNodeId;
        if (!targetParentId) {
  ...
  async importNodes(
    ...
    for (const nodeId of nodeIds) {
      ...
      const newParentId = idMapping.get(node.parentNodeId) || toParentId;
