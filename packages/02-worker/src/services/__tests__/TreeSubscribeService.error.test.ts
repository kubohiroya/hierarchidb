import type { CommandEnvelope, ObserveNodePayload, Timestamp, NodeId } from '@hierarchidb/00-core';
    ...
    it('should cleanup resources on service destruction', async () => {
      ...
      const observables = await Promise.all([
        ...
        service.subscribeChildren({
          ...
          payload: { parentNodeId: 'root' as NodeId },
          issuedAt: Date.now() as Timestamp,
        }),
      ]);
