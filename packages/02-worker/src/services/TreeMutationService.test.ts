  ...
  beforeEach(() => {
    ...
    coreDB = {
      ...
      getChildren: vi.fn(async (parentId: NodeId) => {
        return Array.from(coreDB.treeNodes.values()).filter((n) => n.parentNodeId === parentId);
      }),
    };
      ...
      it('should move nodes to new parent', async () => {
        ...
        nodeIds.forEach((id) => {
          coreDB.treeNodes.set(id, {
            id: id,
            parentNodeId: 'old-parent' as NodeId,
            name: `Node ${id}`,
            nodeType: 'folder' as TreeNodeType,
            createdAt: Date.now() as Timestamp,
            updatedAt: Date.now() as Timestamp,
    ...
    describe('recoverFromTrash', () => {
      ...
      it('should restore to specified parent', async () => {
        ...
        coreDB.treeNodes.set(nodeId, {
          ...
          parentNodeId: 'trash' as NodeId,
          name: 'Trashed Node',
          originalParentNodeId: 'old-parent' as NodeId,
          originalName: 'Original Name',
          removedAt: Date.now() as Timestamp,
          nodeType: 'folder' as TreeNodeType,
          createdAt: Date.now() as Timestamp,
