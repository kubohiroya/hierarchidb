import type { Seq, Timestamp, TreeNode, NodeId } from "@hierarchidb/00-core";
...
class NullDatabaseOperations implements DatabaseOperations {
  ...
  async createNode(node: TreeNode): Promise<void> {
    throw new Error(
      `Database operations not configured - cannot create node ${node.id}. ` +
        "Please provide DatabaseOperations implementation to CommandProcessor constructor.",
    );
  }
}
...
export class CommandProcessor {
  ...
  private readonly MAX_EVENT_HISTORY_SIZE =
    PERFORMANCE_CONFIG.MAX_EVENT_HISTORY_SIZE;
  ...
  async processCommand<TType extends string, TPayload>(
    ...
    try {
      ...
      if (
        envelope.commandId.length > PERFORMANCE_CONFIG.MAX_COMMAND_ID_LENGTH
      ) {
        return this.createErrorResult(
  ...
  private async executeReverseCommand<TType extends string, TPayload>(
    ...
    switch (command.kind) {
      ...
      default:
        ...
        throw new Error(
          `Reverse operation not implemented for command type: ${command.kind}`,
        );
    }
  }
  ...
  private async executeRedoCommand<TType extends string, TPayload>(
    ...
    switch (command.kind) {
      ...
      default:
        // 【未対応コマンド】: Refactorフェーズで拡張予定 🔴
        throw new Error(
          `Redo operation not implemented for command type: ${command.kind}`,
        );
    }
  }
  ...
  constructor(databaseOperations?: DatabaseOperations) {
    ...
    this.databaseOperations =
      databaseOperations || new NullDatabaseOperations();
  }
}
