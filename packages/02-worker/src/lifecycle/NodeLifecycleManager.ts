import type { TreeNode, NodeId, TreeNodeType, EntityReferenceHints } from '@hierarchidb/00-core';
...
export class NodeLifecycleManager {
   ...
   * Handle node load
   */
  async handleNodeLoad(nodeId: NodeId, nodeType: TreeNodeType): Promise<void> {
   ...
   * Handle node unload
   */
  async handleNodeUnload(nodeId: NodeId, nodeType: TreeNodeType): Promise<void> {
   ...
   */
  async handleBatchDelete(nodeIds: NodeId[], nodeType: TreeNodeType): Promise<void> {
  ...
  private async moveNodeCore(nodeId: NodeId, newParentId: NodeId): Promise<void> {
    ...
    await this.updateNodeCore(nodeId, { parentNodeId: newParentId });
  }
   ...
   */
  private async handleReferenceCountIncrement(nodeId: NodeId, nodeType: TreeNodeType): Promise<void> {
    try {
      const config = this.registry.getNodeTypeConfig(nodeType);
      const entityHints = (config as any)?.metadata?.entityHints as EntityReferenceHints | undefined;
      ...
      workerError(`Failed to increment reference count for ${nodeType} node ${nodeId}:`, e as Record<string, any>);
    }
  }
   ...
   */
  private async handleReferenceCountDecrement(nodeId: NodeId, nodeType: TreeNodeType): Promise<void> {
    try {
      const config = this.registry.getNodeTypeConfig(nodeType);
      const entityHints = (config as any)?.metadata?.entityHints as EntityReferenceHints | undefined;
      ...
      workerError(`Failed to decrement reference count for ${nodeType} node ${nodeId}:`, e as Record<string, any>);
    }
  }
