...
export interface INodeRegistry extends IPluginRegistry {
  ...
  // Plugin retrieval
  getPluginDefinition(nodeType: TreeNodeType): PluginDefinition | undefined;
  getEntityHandler(nodeType: TreeNodeType): EntityHandler<PeerEntity, GroupEntity, PeerEntity & WorkingCopyProperties> | undefined;

  // Router actions
  getRouterAction(nodeType: TreeNodeType, action: string): WorkerPluginRouterAction | ((...args: any[]) => Promise<any>) | undefined;
...
export class NodeRegistry implements INodeRegistry {
  ...
  private pluginDefinitions: Map<TreeNodeType, PluginDefinition> = new Map();
  private entityHandlers: Map<TreeNodeType, EntityHandler> = new Map();
  private routingActions: Map<TreeNodeType, Map<string, WorkerPluginRouterAction | ((...args: any[]) => Promise<any>)>> = new Map();
  ...
  >(definition: PluginDefinition<TEntity, TGroupEntity, TWorkingCopy>): void {
    ...
    if (definition.routing?.actions) {
      const actions = new Map<string, WorkerPluginRouterAction | ((...args: any[]) => Promise<any>)>();
   ...
   */
  getPlugin(nodeType: TreeNodeType): PluginDefinition | undefined {
   ...
   */
  get(nodeType: TreeNodeType): PluginDefinition | undefined {
    return this.getPluginDefinition(nodeType);
  }
