...
export async function initializePluginAPIs(workerAPI: any): Promise<void> {
  ...
  // try {
  //   const { basemapPluginAPI } = await import('@hierarchidb/20-plugin-basemap/worker' as string);
  ...
  // try {
  //   const { stylemapPluginAPI } = await import('@hierarchidb/20-plugin-stylemap/worker' as string);
