...
import type { PeerEntity, GroupEntity, WorkingCopy, NodeId, EntityId } from '@hierarchidb/00-core';
...
export class WorkingCopyHandler extends GroupEntityHandler {
  ...
  async createWorkingCopy(nodeId: NodeId): Promise<EnhancedWorkingCopy> {
    ...
    if (this.groupEntityTableName) {
      try {
        ...
        const allGroupEntities = await this.db.table(this.groupEntityTableName)
          .where('parentNodeId')
          .equals(nodeId)
          .toArray();
  ...
  private async commitGroupEntities(
    ...
    if (this.groupEntityTableName) {
      await this.db.table(this.groupEntityTableName)
        .where('parentNodeId')
        .equals(nodeId)
        .delete();
    }
