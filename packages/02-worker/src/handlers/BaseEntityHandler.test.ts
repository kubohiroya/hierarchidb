...
import type { PeerEntity, GroupEntity, WorkingCopyProperties, NodeId, EntityId } from '@hierarchidb/00-core';
...
class TestEntityHandler extends BaseEntityHandler<TestEntity, TestSubEntity, TestWorkingCopy> {
  constructor(db: Dexie, tableName: string, workingCopyTableName: string, subEntityTableName?: string) {
  ...
  async updateEntity(nodeId: NodeId, data: Partial<TestEntity>): Promise<void> {
    ...
    await this.db.table(this.tableName).where('nodeId').equals(nodeId).modify({
      ...data,
      updatedAt: this.now(),
