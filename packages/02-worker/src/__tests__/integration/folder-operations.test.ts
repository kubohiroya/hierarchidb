...
import type { NodeId, TreeId, Tree } from '@hierarchidb/00-core';
...
describe.skip('フォルダ操作の統合テスト (needs update to new API)', () => {
  ...
  beforeEach(async () => {
    ...
    const tree: Tree = {
      ...
      trashRootNodeId: 'trash' as NodeId,
    };
  ...
  describe('フォルダ移動', () => {
    it('フォルダを別の親に移動できる', async () => {
      ...
      expect(childBeforeMove?.parentNodeId).toBe(parent1);
      ...
      expect(childNode?.parentNodeId).toBe(parent2);
    });
    ...
    it('フォルダをコピー&ペーストできる', async () => {
      ...
      if (pasteResult.success && pasteResult.nodeIds) {
        ...
        if (pastedNodeId) {
          ...
          expect(pastedNode?.parentNodeId).toBe(targetParent);
        }
      }
    });
  });
...
async function createFolder(
  ...
  if (!result.success) {
    throw new Error(`Failed to create folder: ${'error' in result ? result.error : 'Unknown error'}`);
  }
