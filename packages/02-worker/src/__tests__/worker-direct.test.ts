...
import type { NodeId, Tree, TreeNode } from '@hierarchidb/00-core';
...
describe('Worker層直接呼び出しテスト', () => {
  ...
  beforeEach(async () => {
    ...
    const dbAdapter = {
      async deleteNode(nodeId: NodeId) { return await coreDB.deleteNode(nodeId); },
      async createNode(node: TreeNode) { await coreDB.createNode(node); }
    };
    ...
    mutationService = new TreeMutationService(coreDB, ephemeralDB, commandProcessor, lifecycleManager);
    ...
    it('インデックスを使用した高速検索', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        testNodes.push({
          ...
          parentNodeId: i % 10 === 0 ? 'root' as NodeId : `search-node-${i - 1}` as NodeId,
          nodeType: 'folder',
          name: `Search Test ${i}`,
          createdAt: Date.now(),
          updatedAt: Date.now(),
