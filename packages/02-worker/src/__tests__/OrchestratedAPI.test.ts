...
import type { NodeId, TreeId, TreeNodeType } from '@hierarchidb/00-core';
import type { ClipboardData } from '@hierarchidb/00-core';
...
describe.skip('OrchestratedAPI (needs update to new API)', () => {
  ...
  beforeEach(async () => {
    ...
    testRootNodeId = 'test-root' as NodeId;
    testParentNodeId = 'test-parent' as NodeId;
  ...
  async function setupTestTree() {
    ...
    const tree = {
      treeId: testTreeId,
      treeRootNodeId: testRootNodeId,
      treeTrashRootNodeId: 'trash-root' as NodeId,
      superRootNodeId: 'super-root' as NodeId,
      name: 'Test Tree',
      createdAt: Date.now(),
      updatedAt: Date.now(),
      version: 1,
  ...
  describe('execute.createNode', () => {
    ...
    it('should create different node types', async () => {
      const nodeTypes: TreeNodeType[] = ['folder', 'basemap', 'stylemap'];
  ...
  describe('Error Handling', () => {
    it('should handle database errors gracefully', async () => {
      // Mock database error
      vi.spyOn(workerAPI, 'createFolder').mockRejectedValueOnce(new Error('Database connection failed'));
