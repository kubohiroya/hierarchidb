...
import type { NodeId, TreeNode, TreeChangeEvent, TreeId } from '@hierarchidb/00-core';
...
// 検索状態
const searchQueryAtom = atom<string, [string], void>(
  '',
  (_get, _set, newValue) => newValue
);
const searchResultsAtom = atom<NodeId[], [NodeId[]], void>(
  [],
  (_get, _set, newValue) => newValue
);
      ...
      case 'node-updated':
        if (event.nodeId) {
          ...
          if (existingNode && event.node) {
            ...
            const oldParentId = existingNode.parentNodeId;
            const newParentId = event.node.parentNodeId;
...
describe.skip('ユーザーストーリー: フォルダ管理 (needs update to new API)', () => {
    ...
    it('フォルダ名変更 → 移動 → 削除の操作フロー', async () => {
      ...
      if (movedFolder) {
        uiIntegration['handleWorkerChange']({
          type: 'node-updated',
          nodeId: folder1Id,
          node: { ...movedFolder, parentNodeId: folder2Id },
      ...
      expect(state2.nodes.get(folder1Id)?.parentNodeId).toBe(folder2Id);
  ...
  describe('Story: 複数フォルダの一括操作', () => {
    it('複数フォルダ選択 → 一括移動 → 一括削除', async () => {
      ...
      folderIds.forEach(nodeId => {
        ...
        if (existingNode) {
          uiIntegration['handleWorkerChange']({
            type: 'node-updated',
            nodeId,
            node: { ...existingNode, parentNodeId: parentId },
      ...
      folderIds.forEach(nodeId => {
        expect(stateAfterMove.nodes.get(nodeId)?.parentNodeId).toBe(parentId);
      });
