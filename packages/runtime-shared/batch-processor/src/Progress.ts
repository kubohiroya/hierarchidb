...
export class ProgressEmitter {
  ...
  private flush(s: ProgressSnapshot) { this.lastEmit = Date.now(); for (const fn of this.listeners) fn(s); }
}
...
export class MemoryProgressStore implements ProgressSnapshotStore {
  ...
  async upsert(jobId: string, snap: ProgressSnapshot): Promise<void> { this.map.set(jobId, snap); }
  async get(jobId: string): Promise<ProgressSnapshot | undefined> { return this.map.get(jobId); }
}
