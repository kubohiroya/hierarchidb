/**
 * Shared types for BaseMap plugin
 * Based on existing BaseMapEntity but organized for 3-layer architecture
 */

import type { NodeId, EntityId, WorkingCopy, PeerEntity } from '@hierarchidb/common-core';
...
/**
 * Create BaseMap data interface
 */
export interface CreateBaseMapData {
  name: string;
  description?: string;
  mapStyle: 'streets' | 'satellite' | 'hybrid' | 'terrain' | 'custom';
  styleUrl?: string;
...
/**
 * Update BaseMap data interface
 */
export interface UpdateBaseMapData {
  name?: string;
  description?: string;
  mapStyle?: 'streets' | 'satellite' | 'hybrid' | 'terrain' | 'custom';
  styleUrl?: string;
