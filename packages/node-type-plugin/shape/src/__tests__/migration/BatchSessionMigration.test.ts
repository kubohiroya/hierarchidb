...
describe('BatchSession Migration Tests', () => {
  ...
  beforeEach(() => {
    // Given: テスト用の設定を準備
    mockNodeId = createNodeId('test-shape-node');
  ...
  describe('バッチセッション開始テスト', () => {
    ...
    it('バッチセッション状態が正常に追跡される', async () => {
      ...
      expect(initialStatus.treeNodeId).toBe(mockTreeNodeId);
  ...
  describe('4段階パイプライン実行テスト', () => {
    ...
    it('VectorTiles段階が正常に実行される', async () => {
      ...
      expect(vectorTilesResult.maxZoomLevel).toBeLessThanOrEqual(mockBatchConfig.vectorTiles.maxZoom);
