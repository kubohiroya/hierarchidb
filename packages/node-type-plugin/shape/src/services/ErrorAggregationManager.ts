...
export interface TaskErrorState {
  taskId: string;
  taskType: 'download' | 'process' | 'simplify' | 'tile';
  target: {
    country?: string;
    adminLevel?: number;
...
export class ErrorAggregationManager {
  ...
  async handleWorkerError(message: WorkerErrorMessage): Promise<void> {
    console.log(`[ErrorAggregation] Received error from worker ${message.workerId}:`, message.error);
  ...
  private detectTaskPatterns(taskError: TaskErrorState): void {
    ...
    if (!taskError.error || !taskError.error.type) {
      console.warn('[ErrorAggregationManager] Invalid error object in detectTaskPatterns:', taskError);
      return;
    }
    ...
    // エラータイプの頻度
    this.patterns.commonErrors.set(
      errorKey,
      (this.patterns.commonErrors.get(errorKey) || 0) + 1
    );
  ...
  private async checkEscalation(): Promise<void> {
    ...
    for (const [stageId, stage] of this.hierarchicalState.stageErrors) {
      ...
      if (failureRate > this.escalationThresholds.failureRateThreshold ||
          stage.status === 'failed') {
  ...
  private async escalateToSession(stageId: string, stage: StageErrorState): Promise<void> {
    ...
    // データ整合性を評価
    this.hierarchicalState.sessionError.dataIntegrity = 
      this.evaluateDataIntegrity();
  ...
  private async persistCurrentState(): Promise<void> {
    ...
    const latestError = Array.from(this.hierarchicalState.taskErrors.values())
      .sort((a, b) => (b.workerInfo?.timestamp || 0) - (a.workerInfo?.timestamp || 0))[0];
    
    if (latestError) {
      // 基本情報のみ保存（複雑なオブジェクトを除外）
      await this.persistenceManager.saveError(
        this.sessionId,
        this.treeNodeId,
        latestError.error,
        {
  ...
  private inferTaskType(stage: string): TaskErrorState['taskType'] {
    switch (stage) {
      ...
      case 'simplify1':
      case 'simplify2': return 'simplify';
      case 'vectorTiles': return 'tile';
  ...
  private canSkipTask(message: WorkerErrorMessage): boolean {
    ...
    return message.error.category !== 'SYSTEM';
  }
  ...
  private evaluateDataIntegrity(): 'intact' | 'partial' | 'compromised' {
    const stageFailures = Array.from(this.hierarchicalState.stageErrors.values())
  ...
  private recordEscalation(from: ErrorHierarchyLevel, to: ErrorHierarchyLevel, trigger: string): void {
  ...
  private detectCyclicalPattern(errors: Array<{ type: string; timestamp: number }>): boolean {
    ...
    for (const [type, timestamps] of Object.entries(errorsByType)) {
   ...
   * メモリ関連エラーの増加傾向を検出
   */
  private detectMemoryIncreasingPattern(errors: Array<{ type: string; timestamp: number }>): boolean {
    const memoryErrors = errors.filter(e => 
      e.type.includes('MEMORY') || e.type.includes('OUT_OF_MEMORY')
    );
  ...
  private detectNetworkSpikes(errors: Array<{ type: string; timestamp: number }>): boolean {
    const networkErrors = errors.filter(e => 
      e.type.includes('NETWORK') || e.type.includes('CONNECTION') || e.type.includes('CORS')
    );
