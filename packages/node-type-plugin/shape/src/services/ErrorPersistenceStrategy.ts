...
import type { TreeNodeId, NodeId } from '@hierarchidb/core';
import type { BaseShapeError, ErrorCategory, ErrorSeverity } from '../types/ShapeErrorHierarchy';
...
export class RuntimeErrorCache {
  ...
  // セッションエラーの履歴
  public sessionErrors = new Map<string, Array<{
    timestamp: number;
    stage?: string;
    metadata?: any;
  }>>();
  ...
  private updateStatistics(error: BaseShapeError): void {
    ...
    this.stats.errorsByType.set(
      error.type,
      (this.stats.errorsByType.get(error.type) || 0) + 1
    );
    this.stats.lastErrorTime = Date.now();
  }
  
  private updateCircuitBreaker(error: BaseShapeError): void {
    if (error.category !== 'NETWORK' && error.category !== 'DATA') return;
