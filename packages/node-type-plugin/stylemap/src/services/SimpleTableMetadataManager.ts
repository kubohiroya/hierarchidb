...
import type { CSVTableMetadata } from '../../../../ui/csv-extract/src/types/index';

/**
 * 【データベース定義】: CSVテーブルメタデータ専用のIndexedDB
 * 【設計方針】: StyleMap専用の軽量データベース設計
 * 【テーブル設計】: 
 ...
 * 【テスト対応】: StyleMapCSVApiDriver.test.ts の全データ管理テストケース対応
...
export class SimpleTableMetadataManager {
   ...
   */
  constructor() {
    // 【DB初期化】: StyleMap専用のCSVメタデータDB作成
    this.db = new Dexie('StyleMapCSVMetadata') as CSVMetadataDB;
  ...
  async store(metadata: CSVTableMetadata): Promise<void> {
    try {
      ...
      throw new Error(`メタデータの保存に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      throw new Error(`メタデータの削除に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async findByContentHash(hash: string): Promise<CSVTableMetadata | null> {
    try {
      ...
      const result = await this.db.csvMetadata
        .where('contentHash')
        .equals(hash)
        .first();
  ...
  async addReference(tableId: string, pluginId: string): Promise<void> {
    try {
      ...
      throw new Error(`参照の追加に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async removeReference(tableId: string, pluginId: string): Promise<void> {
    try {
      ...
      throw new Error(`参照の削除に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async clear(): Promise<void> {
    try {
      ...
      throw new Error(`データのクリアに失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
