...
export const simplifyTileFeatureCollectionWithTopojson = async (params: {
  ...
  if (
    !Number.isFinite(params.toleranceK) ||
    params.toleranceK <= 0 ||
    featureCount === 0 ||
    params.maxVerticesPerTile <= 0
  ) {
    return {
      collection: params.collection,
      finalToleranceK: params.toleranceK,
  ...
  const retryStep =
    Number.isFinite(params.retryToleranceStep) && params.retryToleranceStep > 0
  ...
  const objectEntry = Object.entries(topology.objects ?? {}).find(([key]) => Boolean(key));
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const tolerance = resolveTopojsonToleranceDegrees(
      params.zTarget,
      currentToleranceK,
      params.extent
    );
    const simplifiedTopology =
      tolerance > 0 ? runtime.simplify(presimplified, tolerance) : topology;
    const geojson = runtime.feature(
      simplifiedTopology,
      objectGeometry as Topology['objects'][string]
    ) as FeatureCollection | Feature<Geometry>;
