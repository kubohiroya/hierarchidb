...
describe('WorkingCopyHandler', () => {
  ...
  const nodeId = 'node-123' as TreeNodeId;
  const nodeId2 = 'node-456' as TreeNodeId;
  ...
  describe('Basic Working Copy Operations', () => {
    describe('createWorkingCopy', () => {
      ...
      it('should include sub-entities in working copy', async () => {
        await handler.createSubEntity(nodeId, 'attachment', {
          name: 'File 1',
          data: { size: 100 },
        });
        await handler.createSubEntity(nodeId, 'comment', {
          name: 'Comment 1',
          data: { text: 'Hello' },
        });
      ...
      it('should throw error if entity does not exist', async () => {
        await expect(handler.createWorkingCopy('non-existent' as TreeNodeId)).rejects.toThrow(
          'Entity not found'
        );
      });
    });
  ...
  describe('Commit Operations', () => {
    ...
    describe('commitWorkingCopy', () => {
      ...
      it('should commit sub-entities', async () => {
        ...
        await handler.createSubEntity(nodeId, 'task', {
          name: 'New Task',
          data: { status: 'pending' },
        });
        ...
        const subEntities = await handler.getSubEntities(nodeId);
        expect(subEntities).toHaveLength(1);
      });

      it('should throw error if working copy does not exist', async () => {
        await expect(handler.commitWorkingCopy(nodeId2)).rejects.toThrow('Working copy not found');
      });
    });
  });
  ...
  describe('Conflict Detection and Resolution', () => {
    ...
    it('should detect conflicts when entity modified', async () => {
      ...
      // Update entity directly (simulating another user's change)
      await handler.updateEntity(nodeId, { name: 'Direct Update Name' });
    ...
    it('should resolve conflicts with working strategy', async () => {
      ...
      const entity = await handler.getEntity(nodeId);
      expect(entity?.name).toBe('Working Copy Name');
    });
    ...
    it('should resolve conflicts with current strategy', async () => {
      ...
      const entity = await handler.getEntity(nodeId);
      expect(entity?.name).toBe('Direct Update Name');
    });
    ...
    it('should attempt automatic merge', async () => {
      ...
      await handler.updateEntity(nodeId, {
        name: 'Current Name',
        data: { value: 150, another: 'current' },
      });
  ...
  describe('Advanced Operations', () => {
    describe('branchWorkingCopy', () => {
      ...
      it('should throw error if source does not exist', async () => {
        await expect(
          handler.branchWorkingCopy('non-existent' as TreeNodeId, nodeId2)
    ...
    describe('mergeWorkingCopies', () => {
      ...
      it('should throw error if working copies do not exist', async () => {
        await expect(
          handler.mergeWorkingCopies('non-existent' as TreeNodeId, nodeId2)
  ...
  describe('Auto-save', () => {
    ...
    it('should cancel auto-save on discard', async () => {
      ...
      await handler.createWorkingCopy(nodeId, { autoSave: true });
      await handler.discardWorkingCopy(nodeId);
