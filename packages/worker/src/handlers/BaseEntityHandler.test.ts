...
class TestEntityHandler extends BaseEntityHandler<TestEntity, TestSubEntity, TestWorkingCopy> {
  ...
  async getEntity(nodeId: TreeNodeId): Promise<TestEntity | undefined> {
    return await this.db.table(this.tableName).get(nodeId);
  }
  ...
  async updateEntity(nodeId: TreeNodeId, data: Partial<TestEntity>): Promise<void> {
    ...
    await this.db.table(this.tableName).update(nodeId, {
      ...data,
      updatedAt: this.now(),
...
describe('BaseEntityHandler', () => {
  ...
  const testNodeId = 'test-node-123' as TreeNodeId;
  ...
  describe('Sub-Entity Operations', () => {
    it('should create a sub-entity', async () => {
      ...
      await handler.createSubEntity?.(testNodeId, 'test-type', subEntity);

      const subEntities = await handler.getSubEntities?.(testNodeId, 'test-type');
    ...
    it('should get sub-entities by type', async () => {
      ...
      await handler.createSubEntity?.(testNodeId, 'type1', { value: 'value1' } as any);
      await handler.createSubEntity?.(testNodeId, 'type1', { value: 'value2' } as any);
      await handler.createSubEntity?.(testNodeId, 'type2', { value: 'value3' } as any);

      const type1Entities = await handler.getSubEntities?.(testNodeId, 'type1');
      expect(type1Entities).toHaveLength(2);

      const type2Entities = await handler.getSubEntities?.(testNodeId, 'type2');
    ...
    it('should delete sub-entities by type', async () => {
      ...
      await handler.createSubEntity?.(testNodeId, 'type1', { value: 'value1' } as any);
      await handler.createSubEntity?.(testNodeId, 'type2', { value: 'value2' } as any);

      await handler.deleteSubEntities?.(testNodeId, 'type1');
  ...
  describe('Special Operations', () => {
    ...
    it('should duplicate an entity', async () => {
      const newNodeId = 'new-node-456' as TreeNodeId;
    ...
    it('should create a backup', async () => {
      // Add sub-entities
      await handler.createSubEntity?.(testNodeId, 'type1', { value: 'sub1' } as any);
      await handler.createSubEntity?.(testNodeId, 'type2', { value: 'sub2' } as any);

      const backup = await handler.backup?.(testNodeId);

      expect(backup?.entity.nodeId).toBe(testNodeId);
      expect(backup?.entity.name).toBe('Original Entity');
    ...
    it('should cleanup entity resources', async () => {
      ...
      const subEntities = await handler.getSubEntities?.(testNodeId, 'type1');
