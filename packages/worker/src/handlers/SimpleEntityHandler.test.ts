/**
 * @file SimpleEntityHandler.test.ts
...
describe('SimpleEntityHandler', () => {
  let db: Dexie;
  let handler: SimpleEntityHandler;
  const testNodeId = 'test-node-123' as TreeNodeId;
  const testNodeId2 = 'test-node-456' as TreeNodeId;
  ...
  beforeEach(async () => {
    ...
    handler = new SimpleEntityHandler(db, 'entities', 'workingCopies', 'subEntities');
  });
  ...
  describe('Entity CRUD Operations', () => {
    ...
    describe('getEntity', () => {
      ...
      it('should return undefined for non-existent entity', async () => {
        const entity = await handler.getEntity('non-existent' as TreeNodeId);
        expect(entity).toBeUndefined();
      });

      it('should throw error for invalid nodeId', async () => {
        await expect(handler.getEntity('' as TreeNodeId)).rejects.toThrow('nodeId is required');
      });
    });
    ...
    describe('updateEntity', () => {
      ...
      it('should throw error for non-existent entity', async () => {
        await expect(
          handler.updateEntity('non-existent' as TreeNodeId, { name: 'Test' })
      ...
      it('should not update system fields directly', async () => {
        ...
        await handler.updateEntity(testNodeId, {
          nodeId: 'different-id' as TreeNodeId,
          createdAt: 12345,
          version: 999,
        } as any);
    ...
    describe('batchUpdateEntities', () => {
      ...
      it('should handle missing entities gracefully', async () => {
        ...
        await handler.batchUpdateEntities([
          ...
          { nodeId: 'non-existent' as TreeNodeId, data: { name: 'Test' } },
        ]);
  ...
  describe('Integration with BaseEntityHandler', () => {
    ...
    it('should inherit sub-entity functionality', async () => {
      ...
      await handler.createSubEntity?.(testNodeId, 'test-type', {
        id: 'sub-1',
