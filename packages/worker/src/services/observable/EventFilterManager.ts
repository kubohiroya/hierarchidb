import type { TreeChangeEvent, TreeNodeId, SubscriptionFilter } from '@hierarchidb/core';
...
export class EventFilterManager {
   ...
   */
  constructor(private getNodeFromDB: (nodeId: TreeNodeId) => any) {}
  ...
  private calculateDepthFromAncestor(nodeId: TreeNodeId, ancestorId: TreeNodeId): number {
    ...
    while (currentNode && currentNode.parentTreeNodeId) {
      // 循環参照の検出
      if (visited.has(currentNode.treeNodeId)) {
        break;
      }
      visited.add(currentNode.treeNodeId);
      depth++;

      const parentNode = this.getNodeFromDB(currentNode.parentTreeNodeId);
      if (!parentNode) {
        break;
      }

      // 祖先ノードが見つかった
      if (parentNode.treeNodeId === ancestorId) {
        return depth;
      }
