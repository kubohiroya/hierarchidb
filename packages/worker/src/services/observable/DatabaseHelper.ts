...
export class DatabaseHelper {
   ...
   */
  getNodeFromDB(nodeId: TreeNodeId): TreeNode | undefined {
   ...
   * @returns 子ノードの配列
   */
  getChildrenFromDB(parentId: TreeNodeId, filter?: SubscriptionFilter): TreeNode[] {
    // モックデータベースへの直接アクセス（テスト用）
    if (this.coreDB && 'treeNodes' in this.coreDB && this.coreDB.treeNodes instanceof Map) {
   ...
   * @returns 初期ノードイベント
   */
  createInitialNodeEvent(nodeId: TreeNodeId): TreeChangeEvent {
    // 現在のノード状態を取得
   ...
   * 'children-changed'イベントとして返します。
   *
   * @param parentId 親ノードのID
   * @param filter フィルタリング条件（オプション）
   * @returns 初期子ノードイベント
   */
  createInitialChildrenEvent(parentId: TreeNodeId, filter?: SubscriptionFilter): TreeChangeEvent {
    // 現在の子ノード一覧を取得
