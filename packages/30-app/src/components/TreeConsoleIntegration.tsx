...
import { TreeConsoleToolbar } from "@hierarchidb/12-ui-treeconsole-toolbar";
import type { TreeConsoleToolbarActionParams } from "@hierarchidb/12-ui-treeconsole-toolbar";
import { useTreeConsoleIntegration } from "~/hooks/useTreeConsoleIntegration";
import {
  TopPageGuidedTour,
  ProjectsGuidedTour,
  ResourcesGuidedTour,
} from "@hierarchidb/app-tour";
...
export const TreeConsoleIntegration: React.FC<TreeConsoleIntegrationProps> = ({
    ...
    const checkTrashItems = async () => {
      if (workerClient && treeId) {
        try {
          ...
          if (tree?.trashRootNodeId) {
            const trashChildren = await api.getChildren({
              parentNodeId: tree.trashRootNodeId,
            });
    ...
    return (
      <Box sx={{ p: 2 }}>
        <Alert severity="error">
          Failed to initialize TreeConsole: {workerError}
        </Alert>
      </Box>
    );
  }
  ...
  const renderGuidedTour = () => {
    ...
    if (
      ...
      pageTreeNode?.name?.toLowerCase().includes("project")
    ) {
