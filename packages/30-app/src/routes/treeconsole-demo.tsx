...
import { TreeTableConsolePanelContext } from "@hierarchidb/13-ui-treeconsole-base";
import { WorkerAPIClient } from "@hierarchidb/10-ui-client";
import type { NodeId, Tree } from "@hierarchidb/00-core";
...
export default function TreeConsoleDemo() {
  ...
  const rootNodeId = `${defaultTree.id}Root` as NodeId;
    ...
    >
      ...
      >
        ...
        <Typography variant="caption" display="block" sx={{ mt: 1 }}>
          Tree: {defaultTree.name} (ID: {defaultTree.id})
        </Typography>
      </Box>
      ...
      <Box sx={{ flex: 1, overflow: "hidden", position: "relative" }}>
        <TreeTableConsolePanelContext
          ...
          workerClient={client as any} // Pass the WorkerAPIClient
        />
      </Box>
      ...
      >
        <Typography variant="caption" component="div">
          Debug: Client initialized | Tree: {defaultTree.id} | Root:{" "}
          {rootNodeId}
        </Typography>
      </Box>
    </Container>
