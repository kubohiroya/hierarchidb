...
export interface PluginDiscoveryResult {
  /** Plugins directly listed in app's package.json */
...
/**
 * Simple plugin discovery system that reads dependencies directly from package.json files
 * No need for separate manifest files - package.json is the source of truth
 */
  ...
  private topologicalSort(graph: Record<NodeType, NodeType[]>): NodeType[] {
    ...
    for (const [node, deps] of Object.entries(graph) as [NodeType, NodeType[]][]) {
      for (const _dep of deps) {
        // Node depends on dep, so we need to load dep first
        // This means node has an edge TO dep in our inverted graph
 ...
 */
export const simplePluginDiscovery = new SimplePluginDiscovery();