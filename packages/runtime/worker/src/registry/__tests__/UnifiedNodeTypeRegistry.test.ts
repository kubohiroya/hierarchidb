...
// Mock entity handler
class MockEntityHandler extends BaseEntityHandler<
  TestEntity,
  GroupEntity,
  TestEntity & WorkingCopyProperties
> {
  constructor() {
  ...
  async commitWorkingCopy(
    nodeId: NodeId,
    workingCopy: TestEntity & WorkingCopyProperties
  ): Promise<void> {
    // Mock implementation
  }
...
describe.skip('UnifiedNodeTypeRegistry', () => {
  ...
  describe('Plugin Registration', () => {
    it('should register a plugin successfully', () => {
      const plugin = createTestPlugin('testNode' as NodeType);

      registry.registerPlugin(plugin);

      const retrieved = registry.getPluginDefinition('testNode' as NodeType);
    ...
    it('should throw error for missing dependencies', () => {
      const plugin = createTestPlugin('testNode' as NodeType, ['missingDep']);
    ...
    it('should register plugin with satisfied dependencies', () => {
      const dependency = createTestPlugin('dependency' as NodeType);
      const plugin = createTestPlugin('testNode' as NodeType, ['dependency']);

      registry.registerPlugin(dependency);
      registry.registerPlugin(plugin);

      expect(registry.getPluginDefinition('testNode' as NodeType)).toBeDefined();
    });
  });
  ...
  describe('Plugin Retrieval', () => {
    ...
    it('should get plugin definition', () => {
      const definition = registry.getPluginDefinition('testNode' as NodeType);
      expect(definition).toBeDefined();
      expect(definition?.nodeType).toBe('testNode');
    });

    it('should get entity handler', () => {
      const handler = registry.getEntityHandler('testNode' as NodeType);
      expect(handler).toBeDefined();
      expect(handler).toBeInstanceOf(MockEntityHandler);
    });

    it('should get router action', () => {
      const action = registry.getRouterAction('testNode' as NodeType, 'view');
    ...
    it('should get available actions', () => {
      const actions = registry.getAvailableActions('testNode' as NodeType);
  ...
  describe('Plugin Search', () => {
    beforeEach(() => {
      const plugin1 = createTestPlugin('node1' as NodeType);
      const plugin2 = createTestPlugin('node2' as NodeType);
  ...
  describe('Dependency Management', () => {
    it('should get plugin dependencies', () => {
      ...
      const dep1 = createTestPlugin('dep1' as NodeType);
      const dep2 = createTestPlugin('dep2' as NodeType);
      registry.registerPlugin(dep1);
      registry.registerPlugin(dep2);

      // Then register the plugin with dependencies
      const plugin = createTestPlugin('testNode' as NodeType, ['dep1', 'dep2']);
      registry.registerPlugin(plugin);

      const deps = registry.getPluginDependencies('testNode' as NodeType);
    ...
    it('should validate dependencies correctly', () => {
      ...
      expect(registry.validatePluginDependencies('testNode' as NodeType)).toBe(true);
    });

    it('should sort plugins by dependencies', () => {
      const plugin1 = createTestPlugin('plugin1' as NodeType, ['plugin2']);
      const plugin2 = createTestPlugin('plugin2' as NodeType, ['plugin3']);
      const plugin3 = createTestPlugin('plugin3' as NodeType);
  ...
  describe('Backward Compatibility', () => {
    it('should register as NodeTypeConfig', () => {
      ...
      const config = registry.getNodeTypeConfig('testNode' as NodeType);
  ...
  describe('Clear and Reset', () => {
    it('should clear all registrations', () => {
      ...
      expect(registry.getPluginDefinition('testNode' as NodeType)).toBeUndefined();
