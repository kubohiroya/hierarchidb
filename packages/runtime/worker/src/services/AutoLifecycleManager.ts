...
export class AutoLifecycleManager {
   ...
   */
  async onNodeCreate(nodeId: NodeId, nodeType: NodeType): Promise<void> {
   ...
   */
  async onNodeDelete(nodeId: NodeId, nodeType: NodeType): Promise<void> {
   ...
   */
  async createWorkingCopies(nodeId: NodeId, nodeType: NodeType): Promise<WorkingCopySession> {
    const entities = this.registrationService.getEntitiesByNodeType(nodeType);
  ...
  async commitWorkingCopies(session: WorkingCopySession): Promise<void> {
    ...
    const relevantEntities = entities.filter((e) => session.getTableNames().includes(e.tableName));
  ...
  private async deleteEntityWithCleanup(nodeId: NodeId, entityMeta: EntityMetadata): Promise<void> {
    ...
    if (entityMeta.entityType === 'relational') {
      ...
      if (entityMeta.workingCopyConfig?.enabled) {
        ...
        await this.database.table(workingCopyStore).where('workingCopyOf').equals(nodeId).delete();
      }
    }
  }
   ...
   */
  private async cleanupRelationalReferences(
    nodeId: NodeId,
    entityMeta: EntityMetadata
  ): Promise<void> {
   ...
   */
  private async getNodeTypeFromSession(session: WorkingCopySession): Promise<NodeType | null> {
    ...
    for (const [nodeType, entityKeys] of (this.registrationService as any).nodeTypeToEntities) {
      const entities = entityKeys
        .map((key: string) => (this.registrationService as any).registrations.get(key))
        .filter(Boolean);
