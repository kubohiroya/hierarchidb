...
export class EntityRegistrationService {
  ...
  private nodeTypeToEntities = new Map<NodeType, string[]>();

  /**
   * Register entity metadata for a node type
   */
  registerEntity(nodeType: NodeType, entityKey: string, metadata: EntityMetadata): void {
   ...
   */
  getEntitiesByNodeType(nodeType: NodeType): EntityMetadata[] {
   ...
   */
  getEntitiesSortedByType(nodeType: NodeType, operation: 'delete' | 'commit'): EntityMetadata[] {
    const entities = this.getEntitiesByNodeType(nodeType);

    // Define sort order based on operation
    const sortOrder: EntityType[] =
      operation === 'delete' ? ['relational', 'group', 'peer'] : ['peer', 'group', 'relational'];
