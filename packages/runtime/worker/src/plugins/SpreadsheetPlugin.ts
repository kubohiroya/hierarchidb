import type { NodeId, NodeType, TreeId } from '@hierarchidb/common-core';
import {
  SpreadsheetDB,
  type SpreadsheetMetadata,
  type SpreadsheetRefEntity,
  type SpreadsheetChunk,
  type SpreadsheetMetadataId,
  ...
  return {
    ...
    routing: {
      actions: {
        ...
        'import-file': {
          action: async () => {
            const [nodeId, file, options] = arguments as unknown as [
              NodeId,
              File,
        ...
        'import-url': {
          action: async () => {
            const [nodeId, url, options] = arguments as unknown as [
              NodeId,
              string,
              SpreadsheetImportOptions?,
            ];
        ...
        'import-clipboard': {
          action: async () => {
            const [nodeId, text, options] = arguments as unknown as [
              NodeId,
              string,
              SpreadsheetImportOptions?,
            ];
        ...
        'get-data': {
          action: async () => {
            const [nodeId, options] = arguments as unknown as [
              NodeId,
