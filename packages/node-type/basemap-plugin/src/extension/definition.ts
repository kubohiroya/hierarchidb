...
export const BaseMapExtension = {
  ...
  extendedSteps: [
      ...
      validation: {
        validate: async (data: any) => {
          ...
          if (data.mapStyle.style === 'custom' && !data.mapStyle.customStyleUrl) {
            return {
              isValid: false,
              errors: ['Custom style URL is required when custom style is selected'],
            };
          }
    ...
    },
    {
      ...
      validation: {
        validate: async (data: any) => {
          // Preview is view-only, validation passes if we have required data
          if (!data.mapStyle || !data.viewport) {
            return {
              isValid: false,
              errors: ['Please complete configuration steps before preview'],
            };
          }
          return { isValid: true, errors: [] };
        },
  ...
  extendedFields: [
    ...
    },
    {
      ...
      type: 'array',
      required: false,
      label: 'Initial Position (ZXY)',
      description: 'Initial map position as [zoom, longitude, latitude]',
    },
    {
      name: 'mapStyle',
      type: 'object',
