...
West,90000,18000`;
    ...
    await csvApi.addTableReference(tableMetadata.id, 'stylemap-plugin-sales');
    ...
    await csvApi.addTableReference(tableMetadata.id, 'stylemap-plugin-profit');
    ...
    expect(sharedTable?.referencingPlugins).toContain('stylemap-plugin-sales');
    expect(sharedTable?.referencingPlugins).toContain('stylemap-plugin-profit');

    // Remove one plugin reference
    await csvApi.removeTableReference(tableMetadata.id, 'stylemap-plugin-sales');

    const updatedTable = await csvApi.getTableMetadata(tableMetadata.id);
    expect(updatedTable?.referenceCount).toBe(1);
    expect(updatedTable?.referencingPlugins).toEqual(['stylemap-plugin-profit']);

    // Remove the last reference - table should be deleted
    await csvApi.removeTableReference(tableMetadata.id, 'stylemap-plugin-profit');
