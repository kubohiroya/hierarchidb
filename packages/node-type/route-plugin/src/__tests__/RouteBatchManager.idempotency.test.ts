...
describe('RouteBatchManager idempotency', () => {
  it('returns the same sessionId for identical input payload', async () => {
    ...
    const cfg = { routeGeneration: { method: 'direct', parallel: true, maxConcurrent: 2, retryOnFailure: false, maxRetries: 0 } } as any;
    const routes = [ { startCoordinates: [0,0] as [number,number], endCoordinates: [1,1] as [number,number], method: 'direct' } ];
