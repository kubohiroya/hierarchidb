...
describe('RouteBatchManager pause/resume (smoke)', () => {
  it('creates cursor and toggles paused flag', async () => {
    ...
    const cfg = { routeGeneration: { method: 'direct', parallel: true, maxConcurrent: 1, retryOnFailure: false, maxRetries: 0 } } as any;
    const routes = new Array(3).fill(0).map((_, i) => ({ startCoordinates: [i, i] as [number,number], endCoordinates: [i+1, i+1] as [number,number], method: 'direct' }));
