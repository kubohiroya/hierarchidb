...
#### 1. **BatchSessionManager** (packages/node-type/shape-plugin/src/services/BatchSessionManager.ts)
...
#### 2. **DownloadWorker** (packages/node-type/shape-plugin/src/services/workers/DownloadWorker.ts)
- ✅ 既に完全実装済み
...
#### 3. **SimplifyWorker1** (packages/node-type/shape-plugin/src/services/workers/SimplifyWorker1.ts)
...
#### 4. **SimplifyWorker2** (packages/node-type/shape-plugin/src/services/workers/SimplifyWorker2.ts)
- ✅ 既に完全実装済み
  - タイルグリッド生成
  - 境界へのクリッピング
  - ズームレベル別簡略化
