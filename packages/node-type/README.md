...
### 📊 [@hierarchidb/node-type-shape-plugin](shape-plugin/)
**地理形状データ管理プラグイン**
...
### 📁 [@hierarchidb/node-type-folder-plugin](folder-plugin/)
**拡張可能フォルダープラグイン**
...
### 📈 [@hierarchidb/node-type-spreadsheet-plugin](spreadsheet-plugin/)
**スプレッドシート拡張プラグイン**
...
### 🎨 [@hierarchidb/node-type-stylemap-plugin](stylemap-plugin/)
**スタイルマップ設定プラグイン**
...
> = {
  extends: 'folder-plugin',  // 基底プラグイン指定
...
### プラグイン間依存関係とデータベースアクセス

#### 依存関係の定義
プラグインは他のプラグインに依存することができます。依存関係は自動的に検出され、適切な順序で初期化されます：

```typescript
// BaseMapプラグインがFolderプラグインに依存する例
export const BaseMapExtension = {
  extends: 'folder-plugin',  // 自動的にfolder依存として認識
  nodeType: 'basemap',
  // ...
};
...
#### 依存先データベースへのアクセス
依存するプラグインのデータベースにアクセスできます：

```typescript
// Worker側での依存データベースアクセス例
...
#### 依存関係の制約
- **循環依存の禁止**: A → B → A のような循環依存は許可されません
- **登録順序**: 依存先プラグインが先に登録される必要があります
- **自動削除防止**: 依存されているプラグインは、依存元が削除されるまで削除できません

```typescript
// 依存関係エラーの例
try {
  await registry.unregister('folder-plugin'); // folderを削除しようとする
} catch (error) {
  // Error: Cannot unregister plugin folder-plugin because it has dependents: basemap, spreadsheet-plugin
}
```
...
HierarchiDB v1.1.0以降では、プラグイン登録時にDexieデータベーススキーマが自動的に作成されます：

#### 基本機能
- **自動データベース作成**: プラグイン登録時に専用データベースを作成
- **バージョン管理**: スキーマバージョンによる自動マイグレーション
- **依存関係管理**: 依存プラグインのデータベースアクセス機能
- **クリーンアップ**: プラグイン削除時のデータベース清理・削除

#### Worker側での利用
```typescript
import { NodeDefinitionRegistry, PluginDatabaseManager } from '@hierarchidb/runtime-worker';

// プラグイン登録（データベースが自動作成される）
const registry = NodeDefinitionRegistry.getInstance();
await registry.registerDefinition(MyPluginDefinition);

// プラグインデータベースアクセス
const pluginDB = registry.getPluginDatabase('my-plugin');
const entityTable = pluginDB?.getEntityTable();

// データベースオプション付き削除
await registry.unregister('my-plugin', {
  clearData: true,      // データのみクリア
  dropDatabase: true    // データベース完全削除
});
```
...
#### データベース管理API
```typescript
// プラグインデータベースマネージャー直接操作
const dbManager = PluginDatabaseManager.getInstance();

// プラグインデータベース登録
await dbManager.registerPluginDatabase('plugin-name', databaseConfig, ['dependency1']);

// 依存データベースアクセス
const depDB = dbManager.getDependencyDatabase('plugin-name', 'dependency-plugin');
