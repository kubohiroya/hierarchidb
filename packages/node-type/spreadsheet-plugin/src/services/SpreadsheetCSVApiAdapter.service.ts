...
interface CSVProcessingConfig { [key: string]: any }
interface CSVFilterRule { field: string; op: string; value?: any }
interface CSVColumnInfo { name: string; type?: string; uniqueValues?: number }
interface CSVTableMetadata { id: string; name: string; rowCount?: number; columns?: CSVColumnInfo[] }
interface CSVDataResult { rows: any[]; total: number; columns?: CSVColumnInfo[] }
interface CSVSelectionConfig { filterRules?: CSVFilterRule[]; limit?: number; offset?: number }
interface PaginationOptions { offset: number; limit: number }
interface CSVTableListResult { tables: CSVTableMetadata[]; total: number }
interface CSVProcessingStatus { status: 'pending'|'processing'|'completed'|'error'; progress: number; error?: string; info?: string }
...
export class SpreadsheetCSVApiAdapter implements ICSVDataApi {
  constructor(_pluginId: string = 'spreadsheet') { void _pluginId }
  ...
  async getTableMetadata(_id: string): Promise<CSVTableMetadata | null> { return null; }
  ...
  async getProcessingStatus?(_id: string): Promise<CSVProcessingStatus | null> { return null; }
}
