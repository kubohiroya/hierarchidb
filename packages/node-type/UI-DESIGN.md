...
この文書は、packages/node-type/*-plugin に含まれる「ノード種別ごとのステッパーダイアログ（ウィザード含む）」の各ステップ仕様を整理したものです。
各ステップについて、以下を記載します。
- ステップタイトル / ステップ番号
- 画面構成（主要コンポーネント）
...
### バッチ処理 進捗確認（Route）
- 表示位置: ダイアログではなくパネル内（`packages/node-type/route-plugin/src/components/RoutePanel.tsx`）。
- コンポーネント
...
- 実装: `packages/node-type/shape-plugin/src/components/ShapeStepperDialog.tsx`
- ステップ構成コンポーネント:
...
### バッチ処理 進捗確認画面（Shape）
- コンポーネント: `packages/node-type/shape-plugin/src/components/BatchProcessingMonitorDialog.tsx`
- 構成
  - タイトル行:
...
- 実装: `packages/node-type/route-plugin/src/components/RouteDialog.tsx`
- ステップ構成コンポーネント:
...
- 実装: `packages/node-type/resolver-plugin/src/components/ResolverDialog.tsx`
- ステップ構成: 6 ステップ
  1) Basic Information
  2) Schema Selection
  3) Property Mapping
...
- 実装: `packages/node-type/basemap-plugin/src/extension/definition.ts`
...
- 実装: `packages/node-type/spreadsheet-plugin/src/extension/definition.ts`
...
- 実装: `packages/node-type/styler-plugin/src/extensions/StylerFolderExtension.tsx`
...
- 実装（現状の作成・編集）: `packages/node-type/location-plugin/src/components/LocationDialog.tsx`（単一画面）
...
- コンポーネント: `packages/node-type/location-plugin/src/components/batch/BatchProgressDialog.tsx`
- 構成
