...
export const generateVectorTilesFromFeatureCollection = async (
  ...
  const resolvedMaxZoom = Number.isFinite(config.maxZoom)
    ? Number(config.maxZoom)
    : fallbackMaxZoom;
  ...
  const fallbackLayerAdminLevel =
    typeof metadataContext.adminLevel === 'number' ? metadataContext.adminLevel : undefined;
  const featureGroups = new Map<string, FeatureLike[]>();
  // Compute collection bbox during the grouping pass to avoid a redundant iteration
  ...
  for (const feature of features) {
    ...
    // Accumulate bbox from each feature's geometry
    const stats = extractGeometryStats(feature?.geometry, geometryEngine);
    if (stats.bbox) {
