...
export const resolveTileLayerName = (
    feature: FeatureLike,
...
export const extractGeometryStats = (geometry: FeatureGeometry | undefined, engine: GeometryEngine): {
    vertexCount: number;
    polygonCount: number;
    ...
    if (type !== 'Polygon' && type !== 'MultiPolygon') {
        ...
        return { vertexCount: 0, polygonCount: 0, bbox: undefined, area: fallbackArea };
    }
...
export const normalizeFeatureCollection = async (decoded: unknown): Promise<FeatureCollectionLike | null> => {
...
export const decodeFeatureCollectionFromJsonBuffer = async (buffer: ArrayBuffer): Promise<FeatureCollectionLike | null> => {
    try {
