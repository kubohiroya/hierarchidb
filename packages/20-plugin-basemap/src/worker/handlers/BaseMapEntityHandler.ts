...
export class BaseMapEntityHandler {
  ...
  async createEntity(
    ...
    if (!isCreateBaseMapData(data)) {
      throw BaseMapErrorFactory.databaseOperation(
        "create",
  ...
  async createWorkingCopy(nodeId: NodeId): Promise<BaseMapWorkingCopy> {
    ...
    const workingCopy: BaseMapWorkingCopy = {
      ...
      nodeType: "basemap",
      id: nodeId,
      parentNodeId: "" as NodeId, // Will be set by caller
      name: entity.name,
  ...
  protected async beforeStore(entity: BaseMapEntity): Promise<BaseMapEntity> {
    ...
    if (entity.bearing === undefined)
      entity.bearing = DEFAULT_MAP_CONFIG.bearing!;
    if (entity.pitch === undefined) entity.pitch = DEFAULT_MAP_CONFIG.pitch!;
    if (!entity.displayOptions)
      entity.displayOptions = DEFAULT_MAP_CONFIG.displayOptions!;
