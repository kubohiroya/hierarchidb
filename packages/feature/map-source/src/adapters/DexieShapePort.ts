...
export class DexieShapePort implements MapSourcePort {
  ...
  constructor(dbName?: string) { this.db = new ShapeEphemeralDB(dbName); }
  ...
  async getMetadata(): Promise<{ bounds?: BBox; featureCount?: number; updatedAt?: number }> {
    const items = await this.db.rawBuffers.toArray();
    let fc = 0; let bounds: BBox | undefined; let updatedAt = 0;
    for (const it of items) {
...
function arrToBBox(a: [number, number, number, number]): BBox { return { minX: a[0], minY: a[1], maxX: a[2], maxY: a[3] } }
function union(a: BBox | undefined, b: BBox): BBox {
  if (!a) return b; return { minX: Math.min(a.minX, b.minX), minY: Math.min(a.minY, b.minY), maxX: Math.max(a.maxX, b.maxX), maxY: Math.max(a.maxY, b.maxY) };
}
