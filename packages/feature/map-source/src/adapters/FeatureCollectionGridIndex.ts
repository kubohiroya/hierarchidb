...
export class FeatureCollectionGridIndex implements MapSourcePort {
  ...
  constructor(cellSizeDeg: number = 1) { this.cellSizeDeg = cellSizeDeg; }

  build(fc: FeatureCollection): void {
    this.featureCount = fc.features.length;
    for (const f of fc.features) {
      const b = featureBBox(f); if (!b) continue;
      ...
      for (let i = minI; i <= maxI; i++) for (let j = minJ; j <= maxJ; j++) {
        const key = `${i},${j}`;
        let cell = this.grid.get(key); if (!cell) { cell = { feats: [] }; this.grid.set(key, cell); }
        cell.feats.push(f);
      }
    }
  }
  ...
  async queryByBBox(bbox: BBox, _zoom?: number): Promise<FeatureCollection> {
    ...
    const out: any[] = [];
    for (let i = minI; i <= maxI; i++) for (let j = minJ; j <= maxJ; j++) {
      const cell = this.grid.get(`${i},${j}`); if (!cell) continue;
  ...
  async queryByTile(tile: TileCoord): Promise<FeatureCollection> { return this.queryByBBox(tileToBBox(tile)); }
  async getMetadata(): Promise<{ bounds?: BBox; featureCount?: number; updatedAt?: number }> { return { bounds: this.bounds, featureCount: this.featureCount, updatedAt: Date.now() }; }
}
...
function featureBBox(f: any): BBox | undefined {
  const c = f?.geometry?.coordinates; if (!c) return undefined;
  let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
  const visit = (p: any) => {
    if (typeof p?.[0] === 'number') { const x = p[0], y = p[1]; if (!Number.isFinite(x) || !Number.isFinite(y)) return; minX = Math.min(minX, x); minY = Math.min(minY, y); maxX = Math.max(maxX, x); maxY = Math.max(maxY, y); }
...
function intersectsBBox(f: any, bbox: BBox): boolean {
  const fb = featureBBox(f); if (!fb) return false;
  return !(fb.minX > bbox.maxX || fb.maxX < bbox.minX || fb.minY > bbox.maxY || fb.maxY < bbox.minY);
}
function union(a: BBox | undefined, b: BBox): BBox { return !a ? b : { minX: Math.min(a.minX, b.minX), minY: Math.min(a.minY, b.minY), maxX: Math.max(a.maxX, b.maxX), maxY: Math.max(a.maxY, b.maxY) } }
