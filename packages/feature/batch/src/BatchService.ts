...
function isAsync<T>(it: any): it is AsyncIterable<T> { return !!it && typeof it[Symbol.asyncIterator] === 'function'; }
function toAsync<T>(it: Iterable<T>): AsyncIterable<T> {
  return {
    [Symbol.asyncIterator]: async function* () { for (const v of it) yield v; },
  } as AsyncIterable<T>;
}
