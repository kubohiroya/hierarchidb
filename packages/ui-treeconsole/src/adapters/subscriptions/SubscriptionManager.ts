...
export class SubscriptionManager {
  ...
  async subscribeToNode(
    ...
    try {
      ...
      const unsubscribe = await this.adapter.subscribeToNode(
        nodeId,
        callback,
        context
      );
  ...
  async subscribeToChildren(
    ...
    try {
      ...
      const unsubscribe = await this.adapter.subscribeToChildren(
        parentNodeId,
        callback,
        context
      );
   ...
   * @param nodeId ノードID
   */
  unsubscribeByNodeId(nodeId: TreeNodeId): void {
