...
describe('ThemedLoadingScreen Components', () => {
  ...
  const renderWithTheme = (
    ...
    const theme = createTheme({
      palette: {
        ...
        grey:
          themeMode === 'dark'
            ? { 900: '#121212', 100: '#f5f5f5' }
            : { 50: '#fafafa', 900: '#212121' },
      },
    });
  ...
  describe('正常系テストケース', () => {
    ...
    it('TC-02: Theme integration - テーマ統合', async () => {
      ...
      renderWithTheme(<ThemedLoadingScreen variant="circular" message="Loading..." />, 'dark');
  ...
  describe('異常系テストケース', () => {
    ...
    it('TC-06: Invalid props - 不正なprops値', () => {
      ...
      const { container } = renderWithTheme(
        <ThemedLoadingScreen variant={undefined as any} size={-10} />
      );
  ...
  describe('境界値テストケース', () => {
    ...
    it('TC-09: Message length boundaries - メッセージ長境界', () => {
      ...
      const messages = ['', 'A', 'A'.repeat(1000), 'Very long message with spaces '.repeat(100)];

      messages.forEach((message) => {
        renderWithTheme(<ThemedLoadingScreen variant="circular" message={message} />);
    ...
    it('TC-10: Children complexity - 複雑なchildren要素', () => {
      ...
      renderWithTheme(
        <ThemedLoadingScreen variant="circular">{complexChildren}</ThemedLoadingScreen>
      );
