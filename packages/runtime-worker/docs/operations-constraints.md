...
- Archive（holder 方式）
  - 目的: Archive 復元・競合・参照整合性の取り扱いを単純化。
  - 仕様: archiveRoot 直下の holder.name に `${originalParentId}\t${archiveedNodeId}` を埋め込み、子に実体ノード。
...
- フラグ運用
  - 主要機能（Tx ラップ / コマンドメトリクス / Policy C / Archive holder）は常時有効です。
