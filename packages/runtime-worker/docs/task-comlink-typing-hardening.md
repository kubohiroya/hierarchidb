...
- 共有契約（`@hierarchidb/worker-api` / `@hierarchidb/build-api`）に実装を厳密一致させ、コンパイル時に逸脱を検出する。
...
- `@hierarchidb/worker-api` / `@hierarchidb/build-api`（必要時、契約の明確化と型の更新）

実施手順
1) `WorkerService` が返す各 API の戻り値/引数が `@hierarchidb/worker-api` / `@hierarchidb/build-api` と一致しているか型チェック。
