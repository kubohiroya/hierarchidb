...
対象範囲
- Worker レイヤの Draft 周辺（EphemeralDB バックのサービス実装、および TreeNode ベースの WC 操作ユーティリティ）。
- 関連ファイル:
  - `src/services/DraftService.ts`
  - `src/services/EphemeralDB.ts`
  - `src/services/DraftTreeNodeOperations.ts`
  - `src/RuntimeWorkerService.ts`（`DraftAPI` の公開）
...
- 主な API:
  - `createDraftDraft(nodeType, parentId, initialData?)`: Ephemeral にドラフトを生成
  - `createDraftFromNode(nodeId)`: 既存ノードを Ephemeral に複製
  - `get/update/has/list/validate/hasUnsavedChanges`
  - `commitDraft(nodeId)`: 現状は Ephemeral 側を discard するだけ（Core 反映なし）
  - `commitMultipleWorkingCopies` / `createMultipleWorkingCopies` / `getDraftStats` / `cleanupOldWorkingCopies`
- 公開: `src/RuntimeWorkerService.ts` 経由で `DraftAPI` を WorkerAPI として公開済み

2) TreeNode ベースの WC ユーティリティ（CoreDB 直操作）
- ファイル: `src/services/DraftTreeNodeOperations.ts`
- モデリング: CoreDB 上に WC 用の「ホルダー」Node と中身の Node をペアで作成
  - `createNewDraftDraft(treeId, parentId, nodeType, baseName)`
  - `createDraftFromNode(treeId, nodeId)`
  - `commitDraft(draftNodeId, isDraft, onNameConflict)`
    - 名前重複時の `auto-rename`、楽観ロック（version 比較）に対応
  - `discardDraft([holderId, draftId])` / `getDraft(originalNodeId)` / `updateDraft()` / `checkDraftConflict()`
...
達成していること（仕様化に取り込みたい）
- EphemeralDB による軽量な WC 作成/編集/一覧/統計・クリーンアップ（`DraftService`）。
- CoreDB 直の WC コミット手順（名前重複の自動リネーム、楽観ロック検出）と破棄ロジック（`DraftTreeNodeOperations`）。
- WorkerAPI から `DraftAPI` を取得できる導線（`RuntimeWorkerService.ts`）。

未達/課題（仕様と実装の乖離）
- `commitDraft`（サービス実装）は CoreDB への反映をしておらず、破棄で代替している。
...
- `DraftService` の挙動を仕様に記述（Ephemeral 前提、commit は破棄ベース）。
- `DraftTreeNodeOperations` のルール（楽観ロック/リネーム/ホルダー構造）を仕様に記述し「将来の永続仕様」として位置づけ。
...
- `commitDraft` を CommandProcessor 経由とし、イベント記録・Undo を導入する計画をドラフト。
...
- [x] WorkerAPI から DraftAPI へのアクセス公開

## チェックリスト（未達/課題）
- [ ] サービス実装の `commitDraft` が Core 反映していない（破棄で代用）
- [ ] データモデルの二重化（Ephemeral Row vs Core Node + Holder ペア）
- [ ] 競合検出/自動リネームのサービス層移植方針
- [ ] CommandProcessor/監査/Undo 連携の有無
- [ ] 型の明確化（WC レコード必須項目、`draftOf` 等の扱い）
...
## 更新ログ（実装反映）
- 2025-09-02: holder.name を `${targetParentId}\t${targetNodeId}` に修正（ドラフトは targetNodeId を先行採番）。`getDraft` を holder 走査＋decode 方式へ切替（`draftOf` 参照を撤廃）。
...
- 冪等: `createDraft(...)` は冪等で、既存WCがあればそれを返す（`returnedExisting: true` 相当のメタで識別可能）。
