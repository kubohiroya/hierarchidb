...
export class PluginManagementService implements PluginManagementAPI, PluginLifecycleAPI {
  constructor(private nodeTypeRegistry: PluginRegistryFacade) {}
  ...
  async register(definition: any): Promise<PluginRegistrationResult> {
    ...
    try {
      // Registration is handled by the repository itself
      // await this.nodeTypeRegistry.registerPlugin(definition.nodeType);
  ...
  async unregister(nodeType: NodeType): Promise<UnregistrationResult> {
    ...
    try {
      ...
      // Unregistration handled by repository
      // await this.nodeTypeRegistry.unregister(nodeType);
  ...
  async resetPlugin(options: PluginResetOptions): Promise<PluginResetResult> {
    ...
    try {
      ...
      if (!(await this.nodeTypeRegistry.isPluginAvailable(nodeType as NodeType))) {
        return {
          success: false,
          nodeType,
          deletedEntities: {},
  ...
  async deletePlugin(nodeType: NodeType): Promise<PluginDeleteResult> {
    try {
      ...
      if (!(await this.nodeTypeRegistry.isPluginAvailable(nodeType))) {
        return {
          success: false,
          nodeType,
          error: {
      ...
      const warnings: string[] = [];
      const registeredNodeTypes = await this.nodeTypeRegistry.getAllPlugins();
      for (const registeredNodeType of registeredNodeTypes) {
        const pluginDefinition = await this.nodeTypeRegistry.getPlugin(registeredNodeType.nodeType);
