...
import type { CommandEnvelope, ObserveSubtreePayload, Timestamp, NodeId } from '@hierarchidb/common-type';
...
import { createMockCoreDB, type MockCoreDB, setupTestData } from './TreeSubscriptionService.setup';

/**
 * TreeSubscriptionService - 部分木監視機能のテスト
 *
 * subscribeSubtree メソッドの各種機能をテストします：
 * - 子孫ノード全体の変更監視
 * - 最大深度制限機能
 * - フィルタリング機能
 */
describe('TreeSubscriptionService - Subtree Observation', () => {
  let service: TreeSubscriptionService;
  let coreDB: MockCoreDB;

  beforeEach(() => {
    coreDB = createMockCoreDB();
    setupTestData(coreDB);
    service = new TreeSubscriptionService(coreDB as any);
  });
  ...
  describe('subscribeSubtree', () => {
    ...
    it('should observe changes in any descendant node', async () => {
      ...
      const observable = await service.subscribeSubtreeCommand(cmd);
    ...
    it('should handle depth calculation correctly for simple cases', async () => {
      ...
      setTimeout(() => {
        coreDB.updateNode('subfolder1' as NodeId, { name: 'Updated Subfolder' });
      }, 10);
