...
describe('AutoLifecycleManager', () => {
  ...
  const nodeType = 'test-node' as NodeType;
  ...
  describe('onNodeCreate', () => {
    ...
    it('should handle entities that do not auto-create', async () => {
      await expect(lifecycleManager.onNodeCreate(nodeId, nodeType)).resolves.toBeUndefined();
    ...
    it('should handle unknown node type gracefully', async () => {
      await expect(
        lifecycleManager.onNodeCreate(nodeId, 'unknown-type' as NodeType)
  ...
  describe('onNodeDelete', () => {
    ...
    it.skip('should delete entities in correct order (relational, group, peer)', async () => {
      ...
      const originalDelete = services.table.bind(services);
      vi.spyOn(services, 'table').mockImplementation((tableName: string) => {
      ...
      const peerEntity = await services.table('peerEntities').get(nodeId);
      expect(peerEntity).toBeUndefined();

      // Verify relational entity reference is updated but not deleted
      const relationalEntity = await services.table('relationalEntities').get('resource-1');
  ...
  describe('createWorkingCopies', () => {
    ...
    it('should handle entities without working copy config', async () => {
      ...
      registrationService.registerEntity('no-wc-type' as NodeType, 'no-wc', noWCMetadata);

      const session = await lifecycleManager.createWorkingCopies(nodeId, 'no-wc-type' as NodeType);
  ...
  describe('commitWorkingCopies', () => {
    ...
    it('should handle empty session', async () => {
      ...
      await expect(lifecycleManager.commitWorkingCopies(emptySession)).resolves.toBeUndefined();
    });
  });
  ...
  describe('error handling', () => {
    it('should handle database transaction errors gracefully', async () => {
      ...
      await expect(lifecycleManager.onNodeCreate(nodeId, nodeType)).rejects.toThrow(
        'Database error'
      );
    });
