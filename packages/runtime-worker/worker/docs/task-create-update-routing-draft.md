...
- 種別: 環境変数（`scripts/start-env.sh` から注入）
- 値: `"0" | "1"`（既定は `"0"`）
- 影響範囲: `TreeMutationService.createNode` / `updateNode` のみ。その他は非対象。
- 実装方針（後続PRで実施）:
  - 読み取りは 1 箇所に集約（例: `config/feature-flags.ts`）。
...
- `config/feature-flags.md`（ドキュメント）と `scripts/start-env.sh` の利用例更新
...
## 次アクション（ここで一旦停止）
- [ ] PR-1（ドキュメントのみ）: `docs/feature-flags.md` をベースに、モノレポ横断の記載整備と `start-env.sh` 追補の草案作成。
