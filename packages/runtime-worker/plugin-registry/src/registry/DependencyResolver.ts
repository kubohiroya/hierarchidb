...
export class PluginDependencyResolver {
   ...
   */
  registerPlugin(metadata: PluginIntegrated): void {
   ...
   */
  registerPlugins(plugins: PluginIntegrated[]): void {
  ...
  getAllDependencies(nodeType: NodeType): NodeType[] {
    ...
    const collect = (current: NodeType) => {
      ...
      const directDeps = [
        ...(plugin.dependencies || []),
        ...(plugin.extends ? [plugin.extends] : []),
      ];

      for (const dep of directDeps) {
        if (!dependencies.includes(dep as NodeType)) {
          dependencies.push(dep as NodeType);
          dependencies.push(dep as NodeType);
        }
        collect(dep as NodeType);
      }
    };
  ...
  private buildDependencyGraph(): DependencyGraph {
    ...
    for (const [nodeType, metadata] of this.plugins) {
      ...
      if (metadata.dependencies) {
        metadata.dependencies.forEach((dep) => deps.add(dep as NodeType));
      }

      // Add inheritance dependency
      if (metadata.extends) {
        deps.add(metadata.extends as NodeType);
      }
  ...
  private calculateTopologicalOrder(
    nodes: Map<NodeType, PluginIntegrated>,
  ...
  /**
   * Generate plugin definition automatically from available plugins
   */
  generateDefinition(): PluginDefinition[] {
