 ...
 */
export class ManifestFileDiscovery {
  private static readonly PLUGIN_PATTERN = /^@hierarchidb\/node-type-(.+)-plugin$/;
  private pluginManifests: Record<NodeType, PluginManifest> = {};
  ...
  extractPluginName(packageName: string): string | null {
    const match = packageName.match(ManifestFileDiscovery.PLUGIN_PATTERN);
...
export const PLUGIN_LOAD_ORDER = ${JSON.stringify(loadOrder, null, 2)};`;
   ...
   */
  setPluginManifests(manifests: Record<NodeType, PluginManifest>): void {
 ...
 */
export const manifestFileDiscovery = new ManifestFileDiscovery();