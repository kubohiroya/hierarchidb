...
export class DynamicPluginLoader {
  ...
  private manifests: Map<NodeType, PluginManifest> = new Map();
  ...
  private async collectAllPlugins(requested: NodeType[]): Promise<Set<NodeType>> {
    ...
    while (queue.length > 0) {
      ...
      if (manifest.extends) {
        ...
        if (!visited.has(manifest.extends)) {
   ...
   */
  private async loadManifest(nodeType: NodeType): Promise<PluginManifest | null> {
    // Check cache
   ...
   */
  private generateManifest(nodeType: NodeType): PluginManifest {
    return {
      nodeType,
      name: this.humanize(nodeType),
      version: '1.0.0',
      priority: 500,
      exports: {
        definition: `${this.capitalize(nodeType)}Definition`,
      },
    };
  }
   ...
   * Get plugin manifest
   */
  getManifest(nodeType: NodeType): PluginManifest | undefined {
    return this.manifests.get(nodeType);
  }
