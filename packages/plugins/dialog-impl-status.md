...
本資料は、`packages/plugins/*-plugin` における「作成/編集ダイアログ」内の各ステップについて、
...
- 入力済み判定
  ...
  - 参照: `packages/plugins/folder-plugin/src/components/ExtensibleFolderDialog.tsx`（約 L206, L318 付近）、`packages/plugins/folder-plugin/src/base/BaseFolderPlugin.ts`（`createDialogStep`）。
- ステップ遷移判定
...
- 参照: `packages/plugins/basemap-plugin/src/extension/definition.ts`。
...
- 参照: `packages/plugins/shape-plugin/src/extension/definition.ts`。

### spreadsheet-plugin
- 入力済み判定: Step2（データソース）/Step3（フィルタ）に `validation.validate(data)`（2 は必須、3 は常に true）。
- ステップ遷移判定: `dependsOn` なしで順次。
- 参照: `packages/plugins/spreadsheet-plugin/src/extension/definition.ts`。

### styler-plugin（spreadsheet の拡張）
- 入力済み判定: Step5/Step6 に `validation.validate(data)`（Step5 は対象プロパティ/値列/数値範囲、Step6 は true）。
- ステップ遷移判定: 順次。
- 参照: `packages/plugins/styler-plugin/src/extension/definition.ts`, `components/steps/StylerStep5.tsx`, `components/steps/StylerStep6.tsx`。

### styler-plugin（folder 拡張のサンプル）
- 入力済み判定: `BaseDialogPlugin#createDialogStep` で渡す `validation.validate(data)` を利用。
- ステップ遷移判定: コード上 `dependsOn` 相当はコメントアウトされており未配線。結果として順次遷移。
- 参照: `packages/plugins/styler-plugin/src/extensions/StylerDialogExtension.tsx`, `packages/plugins/folder-plugin/src/base/BaseFolderPlugin.ts`。
...
- 参照: `packages/plugins/resolver-plugin/src/components/ResolverDialog.tsx`。
...
- 参照: `packages/plugins/location-plugin/src/entities/LocationEntityHandler.ts`（`getStepCapabilities`）。

### linker-plugin（ホスト合成ダイアログに移行中）
- 現状: ホスト `PluginDialog` 合成へ移行し、`steps-provider.tsx` で Step2〜Step4（ResourcePicker / AggregatedList / MapPreview）を登録済み。
- 入力済み判定: Step2 は `likedNodeIdSet` の非空チェックを追加。Step3/Step4 は現状 true。
- 祖先/子孫の Linker 集約: QueryAPI 移行（listAncestors/listDescendants）に追随済み。Linker エンティティ API が未整備のため、他 Linker の likedNodeIdSet 取り込みは未着手（自己選択のみ反映）。
- 参照: `packages/plugins/linker-plugin/src/ui/steps-provider.tsx`, `src/ui/steps/*`。
