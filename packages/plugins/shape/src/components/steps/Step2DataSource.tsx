...
// @hierarchidb/app-datasource パッケージのコンポーネントを活用（想定）
// 実際のimportは、app-datasourceパッケージの実装に依存
import type { StepProps } from '~/types';
import { DATA_SOURCE_CONFIGS } from '~/mock/data';

/**
 * Step 2: Data Source Selection
 * Uses @hierarchidb/app-datasource components for data source selection
 */
export const Step2DataSource: React.FC<StepProps> = ({ workingCopy, onUpdate, disabled }) => {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* TODO: Replace with actual DataSourceSelector from @hierarchidb/app-datasource */}
      <Box sx={{ mt: 3 }}>
        {Object.values(DATA_SOURCE_CONFIGS).map((source) => (
          <Box 
            ...
            sx={{
              ...
              bgcolor: workingCopy.dataSourceName === source.name ? 'action.selected' : 'background.paper',
