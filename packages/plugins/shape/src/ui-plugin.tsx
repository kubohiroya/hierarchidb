...
import type { 
  ...
  CreateDialogProps, 
  EditDialogProps 
} from '@hierarchidb/10-ui-core/src/plugins/types';
 ...
 */
const ShapeForm: React.FC<{ nodeId: string }> = ({ nodeId }) => (
  <div>Shape Form for {nodeId}</div>
);
...
export function registerShapeUIPlugin(): void {
  try {
    ...
    import('@hierarchidb/10-ui-core/src/plugins/registry/UIPluginRegistry').then(({ getUIPluginRegistry }) => {
