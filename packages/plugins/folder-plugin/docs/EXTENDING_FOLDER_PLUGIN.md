...
>
> プラグインが公開する TSX 関数/コンポーネントの戻り値は、必ず `JSX.Element`（必要なら `| null`）を明示してください。これは TypeScript の d.ts ロールアップ時に推論戻り値が `jsx-runtime` に依存してポータブルでなくなる（TS2742）問題を防ぐための必須ルールです。詳細は `packages/plugins/README.md` の「Plugin Dev MUSTs」を参照してください。
