...
import type { ContinentCode, Country } from '~/types/Country';
...
const normalizeContinent = (location: string | undefined): ContinentCode => {
  ...
  if (includesAny(lower, ['africa']) || includesAny(trimmed, ['\u30a2\u30d5\u30ea\u30ab']))
    return 'AF';
  if (
    includesAny(lower, ['asia', 'middle east']) ||
    includesAny(trimmed, ['\u30a2\u30b8\u30a2', '\u4e2d\u6771'])
  )
    return 'AS';
  if (
    includesAny(lower, ['europe']) ||
    includesAny(trimmed, ['\u30e8\u30fc\u30ed\u30c3\u30d1', '\u6b27\u5dde'])
  )
    return 'EU';
  if (
    includesAny(lower, ['south america']) ||
    includesAny(trimmed, ['\u5357\u30a2\u30e1\u30ea\u30ab', '\u4e2d\u5357\u30a2\u30e1\u30ea\u30ab'])
  )
    return 'SA';
  if (
  ...
  if (
    ...
    return 'OC';
  if (
    includesAny(lower, ['antarctica']) ||
    includesAny(trimmed, ['\u5357\u6975', '\u5357\u6975\u5927\u9678'])
  )
    return 'AN';
...
export function useIsoCountries(options: UseIsoCountriesOptions = {}) {
  ...
  return state.status === 'ready'
    ? { status: 'ready', countries: readyCountries }
    : { ...state, countries: [] };
}
