  ...
  return {
    ...
    columnStats: columns.map((column) => {
      ...
      return {
        ...
        percentage:
          selectedCountries.length > 0 ? (stats.selected / selectedCountries.length) * 100 : 0,
      };
    }),
  };
}
 ...
 */
export function mergeSelections(...selectionSets: MatrixSelection[][]): MatrixSelection[] {
...
export function filterSelections(
  ...
  return selections.filter((selection) => {
    ...
    // Filter by columns
    if (criteria.columns) {
      const relevantSelections = criteria.columns.reduce(
        (acc, colId) => {
