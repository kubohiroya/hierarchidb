...
export class UseMapLibreFeatureState {
  ...
  private log(message: string, level: 'info' | 'error' = 'info'): void {
    if (this.config.debug) {
      const prefix = '[UseMapLibreFeatureState]';
      if (level === 'error') {
...
let clientBootstrapper: UseMapLibreFeatureState | null = null;

export async function connectToWorker(
  config?: ClientBootstrapConfig,
): Promise<WorkerAPI> {
  if (!clientBootstrapper) {
    clientBootstrapper = new UseMapLibreFeatureState(config);
  }
