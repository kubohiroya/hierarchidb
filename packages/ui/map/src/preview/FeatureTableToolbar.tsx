...
export const FeatureTableToolbar: React.FC<FeatureTableToolbarProps> = ({
  ...
  const { handleClearSearch, handleSearchChange, handleSearchKeyDown } =
    useFeatureTableToolbarView(search);
    ...
    <Box sx={{ p: 1.5, display: 'flex', flexDirection: 'column', gap: 1 }}>
      ...
      {search ? (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          />
          {toolbarActions ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>{toolbarActions}</Box>
          ) : null}
