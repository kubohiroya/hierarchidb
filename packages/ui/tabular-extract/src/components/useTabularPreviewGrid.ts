...
export const useTabularPreviewGrid = ({
  ...
  const numericCols = useMemo(() => {
    ...
    detectedColumns.forEach((col) => {
      const allNumeric =
        sample.length > 0 &&
        sample.every((r) => typeof r?.[col] === 'number' && Number.isFinite(r?.[col] as number));
      if (allNumeric) {
        set.add(col);
      }
    });
  ...
  const [gridHeight, setGridHeight] = useState<number>(
    Math.min(defaultHeight, 48 + rowHeight * maxVisible)
  );
  ...
  const openFilterMenu = useCallback(
    (event: { clientX: number; clientY: number }, columnId: string, value: unknown) => {
      setMenuState({
        open: true,
  ...
  const createFilterFromMenu = useCallback(
    (operator: TabularFilterOperator) => {
