...
export const useBuildSessionTransition = <Phase extends string>({
  ...
  const [buildSessionTransition, setBuildSessionTransition] = useState<
    BuildSessionTransitionState<Phase>
  >({
    active: false,
    phase: 'idle',
    startedAt: 0,
  ...
  const emitBuildSessionTransitionLog = useCallback(
    (level: BuildSessionTransitionLogLevel, event: string, extra?: Record<string, unknown>) => {
  ...
  const pushBuildSessionTransitionNotification = useCallback(
    (level: BuildSessionTransitionNotificationLevel, message: string) => {
      if (!onNotify) return;
  ...
  const beginBuildSessionTransition = useCallback(
    (phase: Phase, options?: TransitionMessageOptions) => {
      const now = Date.now();
      setBuildSessionTransition({ active: true, phase, startedAt: now });
      emitBuildSessionTransitionLog('info', 'build session transition begin', {
        phase,
        ...(options?.extra ?? {}),
      });
      if (options?.message) {
  ...
  const advanceBuildSessionTransitionPhase = useCallback(
    (phase: Phase, options?: TransitionMessageOptions) => {
      ...
      emitBuildSessionTransitionLog('info', 'build session transition phase', {
        phase,
        ...(options?.extra ?? {}),
      });
      if (options?.message) {
