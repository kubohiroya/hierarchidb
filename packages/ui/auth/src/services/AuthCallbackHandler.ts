  ...
  try {
    ...
    if (token) {
      ...
      tokenData = {
        access_token: token,
        token_type: 'Bearer',
        expires_in: Number.parseInt(hashParams.get('expires_in') || '3600', 10),
      };
    } else if (code) {
      // Exchange code for token
      tokenData = await exchangeCodeForToken(code, provider);
    } else {
      throw new Error('No authentication data found');
    }

    // Fetch user info
    const user = await fetchUserInfo(tokenData.access_token, provider);
