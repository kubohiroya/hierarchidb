 ...
 * :
 */
const validatedConfigs = new WeakMap<AuthConfig, boolean>();

const validateRequiredFields = (config: AuthConfig): void => {
...
const validateUrls = (config: AuthConfig): void => {
  ...
  if (!isValidOrigin(config.authOrigin)) {
    throw new Error(`設定エラー: authOriginの形式が不正です: ${config.authOrigin}`);
  }
};

const validateAuthParams = (config: AuthConfig): void => {
  const validResponseTypes = ['code', 'token', 'id_token'];
  if (!validResponseTypes.includes(config.responseType) && import.meta.env.DEV) {
    console.warn(`非標準のresponseType: ${config.responseType}`);
  }

  if (!config.scope.includes('openid') && import.meta.env.DEV) {
    console.warn('OpenID Connectを使用する場合、scopeに"openid"を含めることを推奨します');
  }
};

const validateAuthConfig = (config: AuthConfig): void => {
  if (validatedConfigs.has(config)) {
    return;
  }

  validateRequiredFields(config);
  validateUrls(config);
  validateAuthParams(config);

  validatedConfigs.set(config, true);
};

const freezeConfig = (config: AuthConfig): Readonly<AuthConfig> => Object.freeze({ ...config });
