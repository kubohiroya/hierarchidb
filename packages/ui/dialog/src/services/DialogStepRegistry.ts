...
// Local type definitions until common-core exports are fixed
      ...
      if (step.dependsOn) {
        for (const dep of step.dependsOn) {
          ...
          if (dep >= step.stepNumber) {
            errors.push(
              `Step ${step.stepNumber} cannot depend on later step ${dep}`
            );
          }
        }
      }
      ...
      if (step.validation) {
        ...
        if (!result.isValid && step.dependsOn) {
          ...
          const dependentSteps = steps.filter(s => 
            s.dependsOn?.includes(step.stepNumber)
          );
