...
export function useModelessDialogFrameLogic<TData>({
  ...
  const headerComponent = useMemo(() => {
    ...
    WrappedHeader.displayName =
      OriginalHeader?.displayName ?? OriginalHeader?.name ?? 'FramelessHeader';
  ...
  const defaultFrameSx = useMemo(
    () => (activeTheme: Theme) => {
      const height = fullScreen ? '100%' : isMinimized ? collapsedHeight : size.height;
      ...
      return {
        ...
        maxWidth: fullScreen
          ? '100%'
          : `calc(100vw - ${FRAME_CONSTANTS.NON_STANDARD_MARGIN * 2}px)`,
        maxHeight,
        display: 'flex',
  ...
  const combinedFrameSx = useMemo<SxProps<Theme>>(
    () => (frameSx ? ([defaultFrameSx, frameSx] as SxProps<Theme>) : defaultFrameSx),
