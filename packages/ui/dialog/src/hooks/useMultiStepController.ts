...
export function useMultiStepController<TData = DialogData>(
  opts: ControllerOptions<TData>
): Controller {
  const {
    steps,
    currentData: data,
  ...
  const setStepError = useCallback((index: number, msg?: string) => {
    setStepErrors((prev) => {
      ...
      if (!msg) next.delete(index);
      else next.set(index, msg);
      return next;
    });
  }, []);
  ...
  const canSubmit = useMemo(() => {
    ...
    const requiredOk = Array.isArray(filled)
      ...
      : steps.every((s, i) =>
          s.optional ? true : i === currentStep ? !stepErrors.has(i) : completedSteps.has(i)
        );
  ...
  const validateCurrentStep = useCallback(async () => {
    ...
    try {
      ...
      if (!ok) setStepError(currentStep, '');
      else clearStepError(currentStep);
      return !!ok;
    } catch (e) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const allRequired = steps.every((s, i) =>
      s.optional ? true : i === currentStep ? true : completedSteps.has(i)
    );
