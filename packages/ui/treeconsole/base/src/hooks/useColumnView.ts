...
export function useColumnView({
  ...
  const expandedSet = useMemo(() => new Set(state.expandedPath), [state.expandedPath]);

  const getIsExpanded = useCallback((nodeId: NodeId) => expandedSet.has(nodeId), [expandedSet]);

  const getCanExpand = useCallback((nodeId: NodeId) => hasChildren(nodeId), [hasChildren]);
