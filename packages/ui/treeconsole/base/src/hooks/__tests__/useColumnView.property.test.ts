 ...
 */
function generateTree(depth: number, branching: number, childCounts: number[]): GeneratedTree {
 ...
 */
function generateValidSelectionSequence(tree: GeneratedTree, selectionChoices: number[]): string[] {
...
describe('Feature: treeconsole-view-modes, Property 4: Column path reflects selection hierarchy', () => {
  ...
  it('selecting leaf truncates path (no columns beyond leaf level)', () => {
    fc.assert(
      fc.property(depthArb, branchingArb, childCountsArb, (depth, branching, childCounts) => {
        const tree = generateTree(depth, branching, childCounts);
        ...
        expect(newState.expandedPath[newState.expandedPath.length - 1]).toBe(rootNode as NodeId);
