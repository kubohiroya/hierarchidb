...
import { useMemo, useCallback, useState, useEffect, KeyboardEvent, MouseEvent } from 'react';
...
export function TreeTableWithPlugins({
  ...
  return (
    <PluginProvider plugins={plugins} debugMode={debugPlugins} onPluginEvent={onPluginEvent}>
...
function PluginEnhancedTreeTable(props: TreeTableCoreProps) {
      ...
      switch (event.key) {
        case 'F2':
          ...
          const selectedNodeIds = props.controller?.rowSelection
            ? Object.keys(props.controller.rowSelection).filter(
                (id) => props.controller?.rowSelection?.[id]
              )
            : [];
  ...
  useEffect(() => {
    if (props.controller?.rowSelection) {
      const selectedIds = Object.keys(props.controller.rowSelection).filter(
        (id) => props.controller?.rowSelection?.[id]
      );
  ...
  // セル描画の拡張
  const enhancedRenderCell = useCallback(
    (cellContext: any) => {
...
export function InlineEditableTreeTable(props: TreeTableWithPluginsProps) {
  ...
  return <TreeTableWithPlugins {...props} plugins={defaultPlugins} enableInlineEditing={true} />;
}
...
export function KeyboardNavigableTreeTable(props: TreeTableWithPluginsProps) {
  ...
  return (
    <TreeTableWithPlugins {...props} plugins={defaultPlugins} enableAdvancedKeyboardNav={true} />
  );
}
