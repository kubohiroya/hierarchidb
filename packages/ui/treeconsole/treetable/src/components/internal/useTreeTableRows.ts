...
export function useTreeTableRows({
  ...
  const getFallbackRowState = useCallback((nodeId: NodeId, nodeDepth?: number): FallbackRowState => {
    const inheritedSelection = hasSelectedAncestor(nodeId);
    const visuallyChecked = selectAll || visualSelectionSet.has(nodeId);
    const disableCheckbox = selectAll || inheritedSelection || (!!pageNodeId && !selectAllHydrated);
  ...
  }, [depthOffset, hasSelectedAncestor, pageNodeId, selectAll, selectAllHydrated, useArchiveColumns, visualSelectionSet]);

  const handleFallbackCheckboxChange = useCallback((nodeId: NodeId, checked: boolean, disabled: boolean) => {
    if (disabled) return;
  ...
  const getRowRenderState = useCallback((nodeId: string): RowRenderState => {
  ...
  const createRowDragHandlers = useCallback((nodeId: string) => {
    return {
      onDragStart: (event: React.DragEvent<HTMLTableRowElement>) => {
        ...
        try {
          event.dataTransfer?.setData('application/hdb-node-descendants', JSON.stringify(Array.from(descendants)));
        } catch {
  ...
  }, [controller, disableDragAndDrop, forbiddenTargets, getDescendants, setForbiddenTargets, setHoverDropTargetId]);

  const formatDateValue = useCallback((value: unknown): { dateLabel: string; timeLabel: string } | null => {
