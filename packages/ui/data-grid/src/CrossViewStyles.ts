...
const emptySets = () => ({
  ...
  disabled: new Set<Id>(),
  dragging: new Set<Id>(),
  dropTarget: new Set<Id>(),
});
function makeState(): ChannelState {
  return {
    rows: emptySets(),
    features: emptySets(),
    mapping: { rowToFeatures: new Map(), featureToRows: new Map() },
    registry: { styles: new Map(), rowAssignments: new Map(), featureAssignments: new Map() },
    listeners: new Set(),
    focusListeners: new Set(),
    focus: null,
  };
}
function getCh(id: DatasetId) {
...
export const CrossViewStyles = {
  ...
  setMapping(datasetId: DatasetId, pairs: Array<{ rowId: Id; featureIds: Id[] }>) {
    const ch = getCh(datasetId);
    ch.mapping.rowToFeatures.clear();
    ch.mapping.featureToRows.clear();
    for (const { rowId, featureIds } of pairs) {
      ch.mapping.rowToFeatures.set(rowId, new Set(featureIds));
      for (const fid of featureIds) {
        const s = ch.mapping.featureToRows.get(fid) || new Set<Id>();
        s.add(rowId);
        ch.mapping.featureToRows.set(fid, s);
      }
    }
  ...
  setState(datasetId: DatasetId, which: 'rows' | 'features', state: VisualState, ids: Set<Id>) {
    const ch = getCh(datasetId);
    ch[which][state] = new Set(ids);
    if (which === 'rows' && (state === 'hovered' || state === 'selected' || state === 'matched')) {
      const fids = new Set<Id>();
      ids.forEach((rid) => ch.mapping.rowToFeatures.get(rid)?.forEach((fid) => fids.add(fid)));
      ch.features[state] = fids;
    }
    if (
      which === 'features' &&
      (state === 'hovered' || state === 'selected' || state === 'matched')
    ) {
      const rids = new Set<Id>();
      ids.forEach((fid) => ch.mapping.featureToRows.get(fid)?.forEach((rid) => rids.add(rid)));
      ch.rows[state] = rids;
    }
    ch.listeners.forEach((f) => f());
  },
   ...
   */
  setStyles(datasetId: DatasetId, styles: Map<StyleId, StyleSpec>) {
    const ch = getCh(datasetId);
    ch.registry.styles = styles;
    ch.listeners.forEach((f) => f());
  },
  hasStyles(datasetId: DatasetId) {
    const ch = getCh(datasetId);
    return (ch.registry.styles?.size || 0) > 0;
  },
  assignRows(datasetId: DatasetId, styleId: StyleId, ids: Set<Id>) {
    const ch = getCh(datasetId);
    ch.registry.rowAssignments.set(styleId, new Set(ids));
    ch.listeners.forEach((f) => f());
  },
  assignFeatures(datasetId: DatasetId, styleId: StyleId, ids: Set<Id>) {
    const ch = getCh(datasetId);
    ch.registry.featureAssignments.set(styleId, new Set(ids));
    ch.listeners.forEach((f) => f());
  },
  ...
  getRowSets(datasetId: DatasetId) {
    const ch = getCh(datasetId);
    return {
      hovered: new Set(ch.rows.hovered),
      selected: new Set(ch.rows.selected),
      matched: new Set(ch.rows.matched),
      disabled: new Set(ch.rows.disabled),
      dragging: new Set(ch.rows.dragging),
      dropTarget: new Set(ch.rows.dropTarget),
    };
  },
  ...
  resolveRowStyle(datasetId: DatasetId, rowId: Id) {
    ...
    ch.registry.rowAssignments.forEach((ids, sid) => {
      if (!ids.has(rowId)) return;
      const st = ch.registry.styles.get(sid);
      if (st?.row) specs.push({ pr: st.priority ?? 0, spec: st });
    });
    // Add default atoms-driven styles if defined
    if (ch.rows.matched.has(rowId)) {
      const st = ch.registry.styles.get('match');
      if (st?.row) specs.push({ pr: st.priority ?? 5, spec: st });
    }
    if (ch.rows.hovered.has(rowId)) {
      const st = ch.registry.styles.get('hover');
      if (st?.row) specs.push({ pr: st.priority ?? 10, spec: st });
    }
    if (ch.rows.selected.has(rowId)) {
      const st = ch.registry.styles.get('select');
      if (st?.row) specs.push({ pr: st.priority ?? 20, spec: st });
    }
    ...
    for (const s of specs) {
      ...
      merged.className =
        [merged.className, r.className].filter(Boolean).join(' ').trim() || undefined;
    }
    return merged;
  },
  ...
  getDeckAccessors(datasetId: DatasetId) {
    ...
    const pick = (fid: Id) => {
      ...
      ch.registry.featureAssignments.forEach((ids, sid) => {
        ...
        const st = ch.registry.styles.get(sid);
        if (st?.map) specs.push({ pr: st.priority ?? 0, spec: st });
      });
      // Add default atoms-driven styles if defined
      if (ch.features.matched.has(fid)) {
        const st = ch.registry.styles.get('match');
        if (st?.map) specs.push({ pr: st.priority ?? 5, spec: st });
      }
      if (ch.features.hovered.has(fid)) {
        const st = ch.registry.styles.get('hover');
        if (st?.map) specs.push({ pr: st.priority ?? 10, spec: st });
      }
      if (ch.features.selected.has(fid)) {
        const st = ch.registry.styles.get('select');
        if (st?.map) specs.push({ pr: st.priority ?? 20, spec: st });
      }
      specs.sort((a, b) => a.pr - b.pr);
      if (specs.length === 0) return undefined;
      const shouldMerge = specs.some((s) => (s.spec.composeMode ?? 'override') === 'merge');
      ...
      for (const s of specs) {
        ...
        if (m.featureState)
          merged.featureState = { ...(merged.featureState || {}), ...m.featureState };
      }
      return merged;
    };
    return {
      getFillColor: (d: any) => pick(d.id)?.fillColor ?? [64, 128, 200, 160],
      getLineColor: (d: any) => pick(d.id)?.lineColor ?? [32, 64, 100, 200],
      getLineWidth: (d: any) => pick(d.id)?.lineWidth ?? 1,
      getElevation: (d: any) => pick(d.id)?.elevation ?? 0,
    };
  },
    ...
    ch.registry.featureAssignments.forEach((ids, sid) => {
      ...
      ids.forEach((fid) => {
        try {
          ...
          logCrossViewWarning(
            `Failed to apply registry feature state for id ${String(fid)}`,
            error
          );
        }
      });
  ...
  emitFocus(datasetId: DatasetId, payload: FocusEventPayload) {
    const ch = getCh(datasetId);
    ch.focus = payload;
    ch.focusListeners.forEach((f) => f(payload));
  },
  emitBlur(datasetId: DatasetId) {
    const ch = getCh(datasetId);
    ch.focus = null;
    ch.focusListeners.forEach((f) => f(null));
  },
};
