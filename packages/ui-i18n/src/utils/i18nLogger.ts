...
const t = (key: string, _options?: any): string => {
...
export const i18nLog = isDev
  ? (key: string, options?: any, ...args: unknown[]) => {
...
export const i18nPerf = (labelKey: string, fn: () => void, options?: any) => {
  if (!isDev) {
    fn();
    return;
  }
...
export const i18nLogIf = (condition: boolean, key: string, options?: any, ...args: unknown[]) => {
  if (isDev && condition) i18nLog(key, options, ...args);
};

export const i18nWarnIf = (condition: boolean, key: string, options?: any, ...args: unknown[]) => {
  if (isDev && condition) i18nWarn(key, options, ...args);
};

export const i18nErrorIf = (condition: boolean, key: string, options?: any, ...args: unknown[]) => {
