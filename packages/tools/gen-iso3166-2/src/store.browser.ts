...
export async function ensureIso3166CountryNamesI18n(options: EnsureIsoOptions = {}) {
  ...
  if (options.countryNamesI18nText) {
    const parsed = JSON.parse(options.countryNamesI18nText) as Record<
      string,
...
export function getLocalizedCountryName(code: string, locale: string): string | null {
  ...
  if (
    fallbackEn &&
    typeof fallbackEn[codeKey] === 'string' &&
    fallbackEn[codeKey].trim().length > 0
  ) {
  ...
  if (
    fallbackJa &&
    typeof fallbackJa[codeKey] === 'string' &&
    fallbackJa[codeKey].trim().length > 0
  ) {
