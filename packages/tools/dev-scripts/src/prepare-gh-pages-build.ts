 ...
 * Invocation: run via `pnpm --filter @hierarchidb/tools-dev-scripts run prepare-gh-pages-build` after `pnpm -C app build`.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error('[prepare-gh-pages-build] Unable to read dist/index.html:', error);
    process.exitCode = 1;
    return;
  }
