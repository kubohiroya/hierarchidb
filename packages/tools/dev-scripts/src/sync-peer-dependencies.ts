 ...
 * Invocation: run via `pnpm --filter @hierarchidb/tools-dev-scripts run sync-peer-dependencies`.
...
function main(): void {
  ...
  const touched: Array<{ name: string; dir: string; peers: boolean }> = [];
  ...
  for (const dir of packageDirs) {
    ...
    if (peersChanged) {
      touched.push({ name: pkgJson.name ?? path.basename(dir), dir, peers: peersChanged });
    }
  }

  console.log(`sync-peer-dependencies: peers=${peerAdjustments}`);
  for (const entry of touched) {
    console.log(`- ${entry.name}: peers=${entry.peers ? 'yes' : 'no'}`);
  }
}
