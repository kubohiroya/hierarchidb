...
const runtimeWorkerGeneratedDir = path.dirname(runtimeWorkerMetadataFile);
...
function relativeImportFrom(baseDir: string, repoRelativePath: string | null): string | null {
...
function formatUiModuleSpecifierEntries(summaries: ManifestSummary[], baseDir: string): string {
  return summaries
    .filter((summary) => summary.hasUI)
    .map((summary) => {
      const relative = relativeImportFrom(baseDir, summary.uiSourceEntry);
      const specifier = relative ?? `${summary.packageName}/ui`;
      return `  ${JSON.stringify(summary.nodeType)}: ${JSON.stringify(specifier)},`;
    })
    .join('\n');
}

function formatUiLoaderEntries(summaries: ManifestSummary[], baseDir: string): string {
  return summaries
    .filter((summary) => summary.hasUI)
    .map((summary) => {
      const relative = relativeImportFrom(baseDir, summary.uiSourceEntry);
      const specifier = relative ?? `${summary.packageName}/ui`;
      return `  ${JSON.stringify(summary.nodeType)}: () => import('${specifier}'),`;
    })
    .join('\n');
}

function formatWorkerModuleSpecifierEntries(summaries: ManifestSummary[], baseDir: string): string {
  return summaries
    .filter((summary) => summary.hasWorker)
    .map((summary) => {
      const relative = relativeImportFrom(baseDir, summary.workerSourceEntry);
      const specifier = relative ?? `${summary.packageName}/worker`;
      return `  ${JSON.stringify(summary.nodeType)}: ${JSON.stringify(specifier)},`;
    })
    .join('\n');
}

function formatWorkerSourceEntries(summaries: ManifestSummary[]): string {
  return summaries
    .filter((summary) => summary.hasWorker && summary.workerSourceEntry)
    .map((summary) => `  ${JSON.stringify(summary.nodeType)}: ${JSON.stringify(summary.workerSourceEntry!)},`)
    .join('\n');
}
...
async function collectManifests(): Promise<ManifestSummary[]> {
  ...
  for (const pkgName of pluginPackages) {
    ...
    let workerSourceEntry: string | null = null;
    if (hasWorker) {
      for (const candidate of WORKER_ENTRY_BASENAMES) {
        const candidatePath = path.join(pkgDir, candidate);
        if (await fileExists(candidatePath)) {
          const relative = path.relative(repoRoot, candidatePath).split(path.sep).join('/');
          workerSourceEntry = relative;
          break;
        }
      }
    }

    let uiSourceEntry: string | null = null;
    if (hasUI) {
      for (const candidate of UI_ENTRY_BASENAMES) {
        const candidatePath = path.join(pkgDir, candidate);
        if (await fileExists(candidatePath)) {
          const relative = path.relative(repoRoot, candidatePath).split(path.sep).join('/');
          uiSourceEntry = relative;
          break;
        }
      }
    }
...
function generateWorkerLoader(summaries: ManifestSummary[]): string {
  const specEntries = formatWorkerModuleSpecifierEntries(summaries, outDir);

  return `/* auto-generated by tools-dev-scripts */
export const workerPluginModuleSpecifiers: Record<string, string> = {
${specEntries}
};
`;
}
...
function generateUiLoader(summaries: ManifestSummary[]): string {
  const specEntries = formatUiModuleSpecifierEntries(summaries, outDir);
  const loaderEntries = formatUiLoaderEntries(summaries, outDir);
...
function generateRegistryIndex(summaries: ManifestSummary[]): string {
  ...
  const uiModuleEntries = formatUiModuleSpecifierEntries(summaries, registryGeneratedDir);
  const uiLoaderEntries = formatUiLoaderEntries(summaries, registryGeneratedDir);
  const workerModuleEntries = formatWorkerModuleSpecifierEntries(summaries, registryGeneratedDir);
  const workerSourceEntries = formatWorkerSourceEntries(summaries);
  return `/* auto-generated by tools-dev-scripts */
...
function generateRuntimeWorkerAmbientDeclarations(summaries: ManifestSummary[]): string {
  ...
  for (const summary of summaries) {
    if (summary.exportPaths.includes('worker')) {
      lines.push(
        `declare module '${summary.packageName}/worker' { const moduleValue: Record<string, unknown>; export = moduleValue; }`,
      );
    }
  }
...
function generateRuntimeWorkerMetadata(summaries: ManifestSummary[]): string {
  const registryEntries = formatPluginRegistryEntries(summaries);
  const workerModuleEntries = formatWorkerModuleSpecifierEntries(summaries, runtimeWorkerGeneratedDir);
  const uiModuleEntries = formatUiModuleSpecifierEntries(summaries, runtimeWorkerGeneratedDir);
  const workerSourceEntries = formatWorkerSourceEntries(summaries);
