...
function formatUiModuleSpecifierEntries(summaries: ManifestSummary[]): string {
  return summaries
    .filter((summary) => summary.hasUI)
    .map((summary) => `  ${JSON.stringify(summary.nodeType)}: ${JSON.stringify(`${summary.packageName}/ui`)},`)
    .join('\n');
}

function formatUiLoaderEntries(summaries: ManifestSummary[]): string {
  return summaries
    .filter((summary) => summary.hasUI)
...
function formatWorkerModuleSpecifierEntries(summaries: ManifestSummary[]): string {
  return summaries
    .filter((summary) => summary.hasWorker)
    .map((summary) => `  ${JSON.stringify(summary.nodeType)}: ${JSON.stringify(`${summary.packageName}/worker`)},`)
    .join('\n');
}
...
function generateWorkerLoader(summaries: ManifestSummary[]): string {
  const specEntries = formatWorkerModuleSpecifierEntries(summaries);
...
function generateUiLoader(summaries: ManifestSummary[]): string {
  const specEntries = formatUiModuleSpecifierEntries(summaries);
  const loaderEntries = formatUiLoaderEntries(summaries);
...
function generateRegistryIndex(summaries: ManifestSummary[]): string {
  ...
  const uiModuleEntries = formatUiModuleSpecifierEntries(summaries);
  const uiLoaderEntries = formatUiLoaderEntries(summaries);
  const workerModuleEntries = formatWorkerModuleSpecifierEntries(summaries);
