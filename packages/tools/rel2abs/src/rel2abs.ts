...
function isWorkerUrlOrComlinkContext(node: ts.Node): boolean {
  ...
  const isDirectArgumentNode = (
    target: ts.Node,
    candidates?: ts.NodeArray<ts.Expression>
  ): boolean => {
...
function rewriteSourceFile(sourceText: string, filePath: string, rootDir: string): FileChange {
  ...
  const pushRewrite = (node: ts.StringLiteral): void => {
    ...
    const legacyAlias = isLegacy ? toLegacyAbsoluteAlias(moduleSpecifier, filePath, rootDir) : null;
...
async function run(): Promise<void> {
  ...
  console.log(`${changedImports} specifier replacements in ${changedFiles} file(s).`);
}
