...
import { resolveSpecifierExpression, resolveTargetSpecifier } from './entry-resolver.ts';
...
function formatModulesLiteral(summary: ManifestSummary, mode: PluginSpecifierMode): string {
  ...
  const rootSpecifier = resolveSpecifierExpression(summary, summary.packageName, summary.rootDistEntry, mode);
  entries.push(`    root: ${formatModuleInfo(rootSpecifier)}`);

  if (summary.hasUI && (summary.uiSourceEntry || summary.uiDistEntry)) {
    const specExpr = resolveSpecifierExpression(summary, `${summary.packageName}/ui`, summary.uiDistEntry, mode);
    entries.push(`    ui: ${formatModuleInfo(specExpr, { source: summary.uiSourceEntry })}`);
  }

  if (hasWorkerModule(summary, mode)) {
    const specExpr = resolveSpecifierExpression(summary, `${summary.packageName}/worker`, summary.workerDistEntry, mode);
  ...
  if (summary.hasDatabaseModule && (summary.databaseSourceEntry || summary.databaseDistEntry)) {
    const specExpr = resolveSpecifierExpression(summary, `${summary.packageName}/database`, summary.databaseDistEntry, mode);
  ...
  if (summary.commonSourceEntry || summary.commonDistEntry) {
    const specExpr = resolveSpecifierExpression(summary, `${summary.packageName}/common`, summary.commonDistEntry, mode);
  ...
  if (summary.iconComponent) {
    ...
    const iconSpecExpr = resolveSpecifierExpression(summary, icon.specifier, icon.distEntry ?? null, mode);
...
export function generateDatabaseLoadersSource(summaries: ManifestSummary[], mode: PluginSpecifierMode): string {
  const databaseLoaderEntries = summaries
    .map((summary) => {
      ...
      if (hasLoader) {
        const moduleSpecifierExpr = resolveSpecifierExpression(summary, moduleSpecifier, summary.databaseDistEntry, mode);
      ...
      if (hasPrewarm) {
        ...
        for (const target of summary.databasePrewarmTargets) {
          const targetSpecifierExpr = resolveTargetSpecifier(summary, target.specifier, mode);
          lines.push('      {');
