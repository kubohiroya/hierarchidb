...
function formatModulesLiteral(summary: ManifestSummary, mode: PluginSpecifierMode): string {
  ...
  const rootSpecifier = resolveSpecifierExpression(summary.packageName, summary.rootDistEntry, mode);
  entries.push(`    root: ${formatModuleInfo(rootSpecifier)}`);

  if (summary.hasUI && (summary.uiSourceEntry || summary.uiDistEntry)) {
    const specExpr = resolveSpecifierExpression(`${summary.packageName}/ui`, summary.uiDistEntry, mode);
    entries.push(`    ui: ${formatModuleInfo(specExpr, { source: summary.uiSourceEntry })}`);
  }

  if (hasWorkerModule(summary, mode)) {
    const specExpr = resolveSpecifierExpression(`${summary.packageName}/worker`, summary.workerDistEntry, mode);
  ...
  if (summary.hasDatabaseModule && (summary.databaseSourceEntry || summary.databaseDistEntry)) {
    const specExpr = resolveSpecifierExpression(`${summary.packageName}/database`, summary.databaseDistEntry, mode);
  ...
  if (summary.commonSourceEntry || summary.commonDistEntry) {
    const specExpr = resolveSpecifierExpression(`${summary.packageName}/common`, summary.commonDistEntry, mode);
...
export function generateRegistrySource(summaries: ManifestSummary[], mode: PluginSpecifierMode): string {
...
export function generateUiLoadersSource(summaries: ManifestSummary[], mode: PluginSpecifierMode): string {
  const entries = summaries
    .map((summary) => {
      if (!summary.hasUI || (!summary.uiSourceEntry && !summary.uiDistEntry)) {
        return null;
...
export function generateWorkerLoadersSource(summaries: ManifestSummary[], mode: PluginSpecifierMode): string {
  ...
  const preloadEntries = summaries
    .map((summary) => {
      if (!hasWorkerModule(summary, mode) || !summary.workerPreloadExports || summary.workerPreloadExports.length === 0) {
        return null;
      }
...
export function generateDatabaseLoadersSource(summaries: ManifestSummary[], mode: PluginSpecifierMode): string {
  const databaseLoaderEntries = summaries
    .map((summary) => {
      ...
      if (hasLoader) {
        const moduleSpecifierExpr = resolveSpecifierExpression(moduleSpecifier, summary.databaseDistEntry, mode);
