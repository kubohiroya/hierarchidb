 ...
 * Invocation: run via `pnpm --filter @hierarchidb/tools-codemods run codemod:esm-ext`
 * or execute the bundled output in `dist/` directly.
...
function resolveIndexIfDir(absFrom: string, rel: string): string | null {
  ...
  try {
    ...
    logCodemodWarning(`Failed to resolve directory at ${abs}`, error);
  }
  return null;
}
