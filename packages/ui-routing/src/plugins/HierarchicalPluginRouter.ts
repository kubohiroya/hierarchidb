...
export interface RouteParams {
  ...
  /** ページツリーノードID */
  readonly pageTreeNodeId: string;
  /** ターゲットツリーノードID */
  readonly targetTreeNodeId: string;
  /** ツリーノードタイプ（プラグイン名） */
  readonly treeNodeType: string;
}
...
export function parseHierarchicalUrl(url: string): RouteParams {
  ...
  // 【構造分解】: マッチ結果から各要素を抽出
  const [, treeId, pageTreeNodeId, targetTreeNodeId, treeNodeType] = match;

  // 【セキュリティ検証】: プラグイン名の安全性確認
  if (!treeNodeType) {
    throw new Error('treeNodeType is required');
  }
  validatePluginNameSecurity(treeNodeType);
...
export class HierarchicalPluginRouter {
  ...
  static async resolveRoute(params: RouteParams): Promise<unknown> {
    ...
    try {
      ...
      const plugin = PluginRegistry.get(params.treeNodeType);
      if (!plugin) {
        const message = getErrorMessage('PLUGIN_NOT_FOUND');
        throw new Error(typeof message === 'function' ? message(params.treeNodeType) : message);
      }
