...
describe('階層的プラグインルーティングシステム', () => {
  ...
  describe('URL解析機能', () => {
    test('階層的URLパラメータを正常に解析する', () => {
      ...
      expect(params.pageTreeNodeId).toBe('node-456'); // 【確認内容】: ページノードIDが正しく抽出されることを確認 🟢
      expect(params.targetTreeNodeId).toBe('node-789'); // 【確認内容】: ターゲットノードIDが正しく抽出されることを確認 🟢
      expect(params.treeNodeType).toBe('basemap'); // 【確認内容】: プラグインタイプが正しく抽出されることを確認 🟢
    ...
    test('最小URLパターンでの解析処理', () => {
      ...
      expect(params.pageTreeNodeId).toBeUndefined(); // 【確認内容】: オプショナルパラメータが未定義になることを確認 🟢
      expect(params.targetTreeNodeId).toBeUndefined(); // 【確認内容】: オプショナルパラメータが未定義になることを確認 🟢
      expect(params.treeNodeType).toBeUndefined(); // 【確認内容】: オプショナルパラメータが未定義になることを確認 🟢
        ...
        actions: {
          view: {
            component: React.lazy(() =>
              import('@hierarchidb/plugin-basemap').then((m) => ({ default: m.BaseMapView }))
            ),
            permissions: ['basemap:view'],
          },
          edit: {
            component: React.lazy(() =>
              import('@hierarchidb/plugin-basemap').then((m) => ({ default: m.BaseMapEditor }))
            ),
  ...
  describe('動的プラグインロード機能', () => {
    test('プラグインコンポーネントの動的インポート成功', async () => {
      ...
      const component = await loadPluginComponent(pluginParams.treeNodeType, pluginParams.action);
    ...
    test('存在しないプラグインアクションでの404エラー', async () => {
      ...
      await expect(
        loadPluginComponent(invalidParams.treeNodeType, invalidParams.action)
