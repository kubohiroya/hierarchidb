...
export function parseIdeGsmCsv(
  ...
  nodeId: NodeId,
): { lineStrings: RouteLineString[]; errors: IdeGsmCsvError[] } {
...
function buildRoutePoint(
  ...
  const pointId =
    (location as { pointId?: LocationPointId }).pointId
    ?? ((location as { pid?: string }).pid as LocationPointId | undefined)
