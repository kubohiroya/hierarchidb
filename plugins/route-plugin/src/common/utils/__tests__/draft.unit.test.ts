...
describe('route draft utilities', () => {
  ...
  it('extracts route entity only from draftData', () => {
    const extracted = extractRouteEntity({
      treeNodeId: 'route-1',
      draftMetadata: { name: '', description: '', tags: [] },
      draftData: { processingStatus: 'completed' },
    } as RouteUpdaterPayload);
    expect(extracted.processingStatus).toBe('completed');
  });
