...
- dependencies: @hierarchidb/util、@hierarchidb/table-metadata、必要に応じて @hierarchidb/features/*
...
```typescript
class SpreadsheetCSVApiDriver implements TabularDataApi {
  // チャンク設定
...
- `createSpreadsheetCSVApi()`（`src/ui/facade/RuntimeWorkerService.ts`）: UI から `TabularDataApi` を取得し、ファイルアップロードや URL 指定に応じて取り込みを実行します。内部で `SpreadsheetCSVApiAdapter` を動的 import します。
