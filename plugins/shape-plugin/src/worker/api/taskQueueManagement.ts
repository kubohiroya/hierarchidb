...
const readTaskVersionStrict = (task: BuildTaskRecordLike): number => {
    ...
    if (typeof version !== 'number' || !Number.isFinite(version) || version < 1) {
        throw new Error(`[taskQueueManagement] invalid task version: ${String(version)} (taskId=${task.taskId})`);
    }
...
export const seedTaskQueueFromBuildTasks = async (nodeId: NodeId): Promise<void> => {
    ...
    console.warn('[taskQueueManagement] seed task queue from build tasks', JSON.stringify({
        nodeId,
        scannedCount,
        queuedCount,
    }));
...
const purgeLegacyTaskQueue = async (nodeId: NodeId, taskQueue: VtTaskQueueDb): Promise<number> => {
    ...
    console.warn('[taskQueueManagement] purged legacy task queue records', JSON.stringify({
        nodeId,
...
export const ensureTaskQueueSeeded = async (nodeId: NodeId, taskQueue: VtTaskQueueDb): Promise<void> => {
