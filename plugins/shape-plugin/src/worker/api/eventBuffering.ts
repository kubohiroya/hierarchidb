...
class UnconditionalEventStreamer {
    // nodeId -> eventType -> Set of callbacks
    private subscribers = new Map<NodeId, Map<string, Set<(event: any) => void>>>();
    ...
    private notifySubscribers(nodeId: NodeId, eventType: string, payload: unknown): void {
        const callbacks = this.subscribers.get(nodeId)?.get(eventType);
        if (!callbacks) return;
    ...
    subscribe(nodeId: NodeId, eventType: string, callback: (event: any) => void): () => void {
        ...
        const nodeMap = this.subscribers.get(nodeId) as Map<string, Set<(event: any) => void>>;
        if (!nodeMap.has(eventType)) {
            nodeMap.set(eventType, new Set());
        }

        const callbacks = nodeMap.get(eventType) as Set<(event: any) => void>;
