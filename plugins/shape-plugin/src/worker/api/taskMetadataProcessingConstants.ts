...
import { isGeometryStage, isSourceStage, isTileEmitStage } from './taskQueueManagement.js';
...
const buildFetchDetailFromGeometryInput = (
  task: TaskQueueRecord
...
export const sanitizeTaskMetadataForSummary = (
  ...
  const tileEmitParentInputSummary = pickRecordMetadataField(
    metadata,
...
export const buildPreviewMetadataFromTask = (
  task: TaskQueueRecord
): Record<string, unknown> | null => {
  const metadata = asRecord(task.metadata);
  ...
  if (isSourceStage(task.stage)) {
    ...
    const sourceCacheId =
      readString(preview?.sourceCacheId) ??
  ...
  if (isGeometryStage(task.stage)) {
    ...
    const geometryCacheId =
      readString(preview?.geometryCacheId) ??
    ...
    return {
      ...
      sourceCacheId: sourceCacheId ?? null,
      sourceCacheFormat:
        readString(preview?.sourceCacheFormat) ??
        readString(input?.sourceCacheFormat) ??
        'flatgeobuf',
      sourceCacheCompression:
        readString(preview?.sourceCacheCompression) ??
        readString(input?.sourceCacheCompression) ??
        'none',
...
export const resolveTaskMetadataText = (
  metadata: Record<string, unknown> | undefined
): string | null => resolveTaskMetadataMessage(metadata);
