...
export const resolveToleranceByBand = (
  ...
  return clampTolerance(
    typeof values[safeIndex] === 'number' ? values[safeIndex] : fallback,
    fallback
  );
};
  ...
  const rawNormalized = Array.from({ length: bandCount }, (_, index) =>
    index < reps.length
      ? resolveNumericToleranceValue(
          ...
          index < resolvedFallbackAnchors.length
            ? (resolvedFallbackAnchors[index] ?? fallback)
            : fallback
        )
      : fallback
  );
...
export const buildToleranceByBandFromToneCurveAnchors = (
  ...
  const normalizedAnchors = [...anchors]
    .filter(
      (anchor): anchor is ToneCurveAnchor => Number.isFinite(anchor.x) && Number.isFinite(anchor.y)
    )
  ...
  if (normalizedAnchors.length === 1) {
    return Array.from({ length: bandCount }, () =>
      clampTolerance(normalizedAnchors[0]?.y ?? fallback, fallback)
    );
  }
