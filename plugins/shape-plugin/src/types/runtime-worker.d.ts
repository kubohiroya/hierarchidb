...
declare module '@hierarchidb/runtime-worker' {
  ...
  export interface RuntimeWorkerSimplifyApi {
    simplifyStage1: (inputBufferId: string, options: { tolerance: number; minArea?: number }) => Promise<void>;
    simplifyStage2: (inputBufferId: string, options: { zoomLevels: number[]; tileSize: number }) => Promise<void>;
  }

  export interface RuntimeWorkerVectorTileApi {
    generateTiles: (
      inputBufferId: string,
      options: { format: 'mvt'; compression: 'gzip' | 'none'; tileSize: number },
    ) => Promise<void>;
    listTiles?: (
      fileId: string,
    ...
    getSummary?: (fileId: string) => Promise<{ tiles: number; totalBytes: number; zoomMin?: number; zoomMax?: number }>;
  }

  export interface RuntimeWorkerStageClient {
    download?: RuntimeWorkerDownloadApi;
    simplify?: RuntimeWorkerSimplifyApi;
    vectortile?: RuntimeWorkerVectorTileApi;
    [key: string]: unknown;
  }
  ...
  export function createStageWorkerClient(): Promise<RuntimeWorkerStageClient | null>;
