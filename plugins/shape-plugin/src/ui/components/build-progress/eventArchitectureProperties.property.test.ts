        ...
        fc.property(
          ...
          ({ nodeId, subscriberCount, eventCount }) => {
            ...
            const unsubscribers = receivedEventsBySubscriber.map((events) =>
              unconditionalEventStreamer.subscribe(node, 'task-progress', (event) =>
                events.push(event)
              )
            );

            for (let i = 0; i < eventCount; i++) {
              unconditionalEventStreamer.emitEvent(node, 'task-progress', {
                eventIndex: i,
              } as unknown as import('../../../worker/api/eventBuffering').EventPayload);
            }
  ...
  describe('Property 19: Loss-Free Event Buffering', () => {
    ...
    it('should maintain FIFO ordering within notification types', () => {
      fc.assert(
        fc.property(
          fc.record({
            notificationType: fc.constantFrom('session-state', 'stage-snapshot') as fc.Arbitrary<
        ...
        fc.property(
          fc.record({
            nodeIds: fc.uniqueArray(
              fc.string({ minLength: 2, maxLength: 10 }).filter((value) => value.trim().length > 0),
              {
                minLength: 1,
                maxLength: 3,
