...
import { buildGeometryTaskOutcomeSummary } from '../taskOutcomeSummaryBuilderUtils';
...
describe('Bug Condition Exploration: i18n Hardcoded Text', () => {
  ...
  it('uses translate() labels for completed tasks', () => {
    ...
    expect(mockTranslate).toHaveBeenCalledWith(
      'task.details.effectiveTolerance',
      'Effective Tolerance'
    );
    expect(mockTranslate).toHaveBeenCalledWith('task.details.retryCount', 'Retry Count');
    expect(mockTranslate).toHaveBeenCalledWith(
      'task.details.finalDataSize',
      'Final Data Size (F/Pol/V)'
    );
    expect(mockTranslate).toHaveBeenCalledWith(
      'task.details.originalDataSize',
      'Original Data Size (F/Pol/V)'
    );
    expect(mockTranslate).toHaveBeenCalledWith(
      'task.details.vertexReductionRate',
      'Vertex Reduction Rate'
    );
  ...
  it('uses translate() labels for failed tasks', () => {
    ...
    expect(result.detailLines).toEqual(
      expect.arrayContaining([expect.stringContaining('Failure Reason:')])
    );
