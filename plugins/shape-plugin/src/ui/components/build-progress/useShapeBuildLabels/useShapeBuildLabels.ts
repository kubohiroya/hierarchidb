...
export const useShapeBuildLabels = ({
  ...
  const stageLabel = useMemo(() => {
    if (displayStageId) {
      return stages.find((stage) => stage.id === displayStageId)?.title ?? displayStageId;
    }
  ...
  const taskLabel = useMemo(() => {
    const progressTaskMessage =
      formatTaskDisplayMessage(effectiveProgress?.progressTaskDisplay, t) ??
    ...
    if (buildStatus !== 'running') {
      if (buildStatus === 'idle' && rawDisplayCounts.total > 0) {
        const doneCount =
          rawDisplayCounts.completed + rawDisplayCounts.failed + rawDisplayCounts.skipped;
    ...
    return (
      progressTaskMessage ??
      (resolvedTaskType
        ? (stages.find((stage) => stage.id === resolvedTaskType)?.title ?? resolvedTaskType)
        : undefined) ??
