...
const generateDistributedSeqNum = (workerIndex: number, eventCount: number, totalWorkers: number): number => {
    return workerIndex + (eventCount * totalWorkers);
};

const parseDistributedSeqNum = (seqNum: number, totalWorkers: number): { workerIndex: number; eventCount: number } => {
...
class MockEventBufferManager implements EventBufferManager {
    ...
    getNextExpectedSeqNum(): number {
        const allBuffers = [
            this.sessionStateBuffer,
            this.stageSnapshotBuffer,
            this.taskProgressBuffer
        ];
