...
export const useShapeBuildTaskSyncScheduling = ({
  ...
  const flushTasks = useCallback(
    (next: ShapeBuildTaskSummary[], dirty: boolean) => {
      if (!dirty) {
        markTaskSnapshotProgressSynchronized?.();
        return;
      }
      committedTasksRef.current = next;
      const shouldUpdateTasks =
        next.length === 0 || !areTaskListsEquivalentForView(tasksRef.current, next);
  ...
  const scheduleFlush = useCallback(
    (next: ShapeBuildTaskSummary[], dirty = true) => {
  ...
  const mergeTaskWithBase = useCallback(
    (task: ShapeBuildTaskSummary, baseList: ShapeBuildTaskSummary[]) => {
  ...
  const bufferTaskUpdate = useCallback(
  ...
  const applyBufferedEvents = useCallback(() => {
    ...
    if (bufferedSnapshot) {
      tasksMapRef.current = new Map(
        nextList.map((task: ShapeBuildTaskSummary) => [task.taskId, task])
      );
      const nextCompletedMap = new Map(completedTasksRef.current);
  ...
  return {
    ...
    scheduleFlush,
    syncTasksRef,
    syncLoadingRef: useCallback(
      (loading: boolean) => {
        isLoadingRef.current = loading;
      },
      [isLoadingRef]
    ),
    syncErrorRef: useCallback(
      (error: Error | null) => {
