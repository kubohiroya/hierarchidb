...
const isTerminalTaskLike = (task: ShapeBuildTaskSummary): boolean => isTerminalTask(task);
...
export const useShapeBuildTaskSyncEventHandlers = ({
  ...
  const { tasksMapRef, errorRef, isLoadingRef, bufferedSnapshotRef, sessionNodeId } = refs;
  ...
  const handleSnapshot = useCallback(
    (next: RawTaskSummary[]) => {
      ...
      try {
        ...
        const errObj =
          error instanceof Error ? error : new Error('Failed to resolve task snapshot');
        setError(errObj);
        return;
      }
