...
export const useShapeBuildProgressPanelControllerBaseStateData = ({
  ...
  const stageLoadingState = useMemo<StageMetadataMap<boolean>>(() => {
    const next = stages.reduce<StageMetadataMap<boolean>>((acc, stage: { id: string }) => {
  ...
  const stageConcurrencyIndicatorAriaLabels = useMemo<StageMetadataMap<string>>(() => {
    const next = stages.reduce<StageMetadataMap<string>>((acc, stage: { id: string; title: string }) => {
