...
const resolveTargetFromPosition = (
  segments: BuildSessionStageProgressBarSegment[],
  position: number
) => {
  let offset = 0;
...
const resolveNearestInteractiveSegment = (
  segments: BuildSessionStageProgressBarSegment[],
  index: number
) => {
...
export const useBuildSessionStageProgressBarInteraction = ({
  ...
  const updateScrollTargetFromClientX = useCallback(
  ...
  const onActivateTaskSegment = useCallback(
