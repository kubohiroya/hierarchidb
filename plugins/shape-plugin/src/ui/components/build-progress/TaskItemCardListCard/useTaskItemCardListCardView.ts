...
} from '~/ui/components/build-progress/TaskItemCard/taskOutcomeSummaryBuilderUtils';
...
export const useTaskItemCardListCardView = ({
  ...
  const stages = useShapeBuildStages({
    t: (key, fallback): string => String(t(key, fallback ?? key)),
  });
  const stageIconById = useMemo(
    () => new Map(stages.map((stage) => [stage.id, stage.icon])),
    [stages]
  );
  const resolveStageIcon = useCallback(
    (taskStageId: string): ReactNode | null => stageIconById.get(taskStageId) ?? null,
    [stageIconById]
  );
  const resolveSummaryBuilder = useCallback(
    (taskStageId: string): TaskOutcomeSummaryBuilder => {
  ...
  const resolveDetailSelection = useCallback(
    (detailId: string | null): TaskDetailSelection | null => {
  ...
  const resolveTaskCardView = useCallback(
    (task: ShapeBuildTaskSummary) => {
      ...
      const isDetailHoverPreviewActive =
        !selectedTaskDetailId && hoveredTaskDetailId === currentTaskDetailId;
  ...
  const createTaskCardStyle = useCallback(
    (start: number, size: number): CSSProperties => ({
      position: 'absolute',
      top: 0,
      left: 0,
      width: '100%',
