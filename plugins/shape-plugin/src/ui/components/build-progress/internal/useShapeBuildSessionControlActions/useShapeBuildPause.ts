...
} from '~/ui/components/build-progress/internal/useShapeBuildSessionHelpers/constants';
import {
  runWithTimeout,
  waitForSessionStateSync,
} from '~/ui/components/build-progress/internal/useShapeBuildSessionHelpers/elapsedConstants';
...
export const useShapeBuildPause = ({
  ...
  const handlePause = useCallback(
    async (reason: ShapeBuildPauseReason = 'user-pause'): Promise<void> => {
      if (!activeNodeId) {
      ...
      const shouldCancelQueued =
        buildSessionTransitionActive && buildStatus !== 'running' && runtimeStatus !== 'running';
      ...
      try {
        ...
        if (!sessionSyncSuccess) {
          ...
          console.warn(
            '[ShapeBuildPause] Session state sync timed out, but pause command was sent successfully'
          );
        } else {
