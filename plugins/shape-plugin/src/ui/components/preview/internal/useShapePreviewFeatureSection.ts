...
export const useShapePreviewFeatureSection = ({
  ...
  const { featureListRows, rowIdToMembers, featureToCountryKey, countryGroupMembers } =
    useMemo(() => {
  ...
  const baseErrorSummaryById = useMemo(() => {
    const summary = new Map<
      string,
      { errorCount: number; repairCount: number; count: number; messages: string[] }
    >();
    featureListRows.forEach((row) => {
      const id = row.featureId ?? row.id;
      if (!id) return;
      const key = String(id);
      const metadataErrorCount =
        typeof (row as { errorCount?: number }).errorCount === 'number'
          ? Math.max(0, (row as { errorCount?: number }).errorCount ?? 0)
          : 0;
      const metadataRepairCount =
        typeof (row as { repairCount?: number }).repairCount === 'number'
  ...
  const errorSummaryById = useMemo<MapPreviewErrorSummaryById>(() => {
    ...
    featureListRows.forEach((row) => {
      const memberFeatureIds = (row as ShapePreviewFeatureRow & { memberFeatureIds?: string[] })
        .memberFeatureIds;
  ...
  const toggleRecyclingForSelection = useCallback(
    async (selectedFeatureIds: string[]) => {
      ...
      try {
        ...
        const updatedRowsMap = new Map(
          updatedRows.map((row) => [String(row.featureId ?? row.id), row])
        );
