...
import { calculateEstimatedFeatures, calculateEstimatedSize, formatBytes, formatNumber } from '~/common/utils/estimateUtils';
...
export const useShapeCountrySelectionStepSelectionState = ({
  ...
  const availabilityByCountryCode = useMemo(() => buildAvailabilityByCode(availability), [availability]);
  ...
  const baseCountries = useMemo(() => (
    countries.map((country, countryIndex) => {
      ...
      const normalizedLevels = Array.from(
        new Set(
          resolvedLevels.filter((level) => Number.isFinite(level) && level >= 0 && level <= resolvedMaxAdminLevel),
        ),
      ).sort((a, b) => a - b);
      return {
        country: {
  ...
  useEffect(() => {
    ...
    baseCountries.forEach((entry) => {
      ...
      nextSelection[entry.country.code] = Array.from(
        { length: resolvedMaxAdminLevel + 1 },
        (_, colIndex) => (entry.availableAdminLevels.includes(colIndex) ? Boolean(row[colIndex]) : false),
      );
    });
  ...
  const isCellEnabled = useCallback((countryCode: string, columnId: string) => {
