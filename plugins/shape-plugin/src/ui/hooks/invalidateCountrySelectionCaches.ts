 ...
 */
export async function invalidateCountrySelectionCaches(
  dataSource: string,
  nodeId: NodeId
): Promise<void> {
  ...
  const jsonStore = createShapeChunkStore(
    jsonSerializer,
    jsonDeserializer
  ) as DexieChunkStore<unknown>;
  const textStore = createShapeChunkStore(
    textSerializer,
    textDeserializer
  ) as DexieChunkStore<unknown>;
  ...
  if (normalized === 'geoboundaries' || normalized === 'geoboundaries-topojson') {
    tasks.push(
      jsonStore.deleteForNode(
        nodeId,
        buildShapeCacheKey('geoboundaries:metadata:all', GEOBOUNDARIES_ALL_METADATA_URL)
      )
    );
  ...
  if (normalized === 'gadm') {
    tasks.push(textStore.deleteForNode(nodeId, buildShapeCacheKey('gadm:maps', GADM_MAPS_URL)));
