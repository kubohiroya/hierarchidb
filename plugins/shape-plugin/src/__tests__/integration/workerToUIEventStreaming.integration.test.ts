...
describe('Worker-to-UI Event Streaming Integration Tests', () => {
  ...
  describe('Process Boundary Event Delivery', () => {
    it('should deliver events from Worker thread to main thread', async () => {
      ...
      eventsToEmit.forEach(({ type }) => {
        const unsubscribe = unconditionalEventStreamer.subscribe(nodeId, type, (sequencedEvent) => {
  ...
  describe('SeqNum Generation and Distribution', () => {
    ...
    it('should verify seqNum monotonicity within notification types', () => {
      ...
      const unsubscribers = notificationTypes.map((type) =>
        unconditionalEventStreamer.subscribe(nodeId, type, (sequencedEvent) => {
    ...
    it('should handle seqNum reset on Worker restart', () => {
      ...
      const unsubscribe1 = unconditionalEventStreamer.subscribe(nodeId, 'session-state', (event) =>
        firstSessionSeqNums.push(event.seqNum)
      );
      ...
      const unsubscribe2 = unconditionalEventStreamer.subscribe(nodeId, 'session-state', (event) =>
        secondSessionSeqNums.push(event.seqNum)
      );
  ...
  describe('AbortController Integration', () => {
    it('should terminate event streaming within 500ms', async () => {
      ...
      let eventCount = 0;
      const unsubscribe = unconditionalEventStreamer.subscribe(nodeId, 'task-progress', () => {
        eventCount++;
      });
    ...
    it('should handle AbortSignal in event emission', async () => {
      ...
      const unsubscribe = unconditionalEventStreamer.subscribe(nodeId, 'session-state', (event) =>
    ...
    it('should cleanup event streams on Worker termination', async () => {
      ...
      const unsubscribers = Object.keys(receivedEventsByType).map((type) =>
        unconditionalEventStreamer.subscribe(nodeId, type, (event) =>
          receivedEventsByType[type]!.push(event)
        )
      );
  ...
  describe('Event Delivery Reliability', () => {
    it('should maintain event ordering under Worker load', async () => {
      ...
      for (let workerIndex = 0; workerIndex < concurrentWorkers; workerIndex++) {
        unconditionalEventStreamer.configureDistributedSeqNum(
          nodeId,
          workerIndex,
          concurrentWorkers
        );

        const unsubscribe = unconditionalEventStreamer.subscribe(nodeId, 'task-progress', (event) =>
          allWorkerEvents.push(event)
        );
