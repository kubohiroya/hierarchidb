    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ runId, nodeId, bandIndex, sourceKey, dataSize }) => {
          await ephemeralDB.geometryCache.delete(
            `geom-valid-${runId}-${nodeId}-${bandIndex}-${sourceKey}`
          );
          await ephemeralDB.geometryCacheMeta.delete(
            `geom-valid-${runId}-${nodeId}-${bandIndex}-${sourceKey}`
          );
  ...
  it('should treat geometry entries with only data write as valid (meta auto-mirrored)', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ runId, nodeId, bandIndex, sourceKey, dataSize }) => {
          await ephemeralDB.geometryCache.delete(
            `geom-invalid-${runId}-${nodeId}-${bandIndex}-${sourceKey}`
          );
          await ephemeralDB.geometryCacheMeta.delete(
            `geom-invalid-${runId}-${nodeId}-${bandIndex}-${sourceKey}`
          );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ runId, nodeId, sourceKey, dataSize }) => {
          ...
          await ephemeralDB.sourceCacheMeta.delete(
            `source-invalid-${runId}-${nodeId}-${sourceKey}`
          );
