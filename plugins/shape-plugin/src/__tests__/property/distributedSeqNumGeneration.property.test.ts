...
const makePayload = (eventType: NotificationType): EventPayload => {
    switch (eventType) {
        ...
        case 'stage-snapshot':
            return { nodeId: 'n' as NodeId, sessionId: 's', stageId: 'st', tasks: [] } as unknown as EventPayload;
        case 'task-progress':
            return { nodeId: 'n' as NodeId, sessionId: 's', taskId: 't', value: 0 } as unknown as EventPayload;
        case 'worker-log':
            return { nodeId: 'n' as NodeId, sessionId: 's', level: 'info', message: 'x' } as unknown as EventPayload;
        case 'critical-error':
                ...
                fc.property(
                    fc.record({
                        ...
                        eventType: fc.constantFrom<NotificationType>(
                            'session-state', 'stage-snapshot', 'task-progress', 'worker-log', 'critical-error',
                        ),
                ...
                fc.property(
                    ...
                    ({ nodeId, preEventCount, postEventCount }) => {
                        ...
                        for (let k = 0; k < preEventCount; k++) {
                            unconditionalEventStreamer.emitEvent(node, 'session-state', payload as Exclude<EventPayload, import('~/common/types/session-events').SessionHeartbeatEvent>);
                        }
                ...
                fc.property(
                    ...
                    ({ nodeId, beforeCount, afterCount }) => {
                        ...
                        for (let k = 0; k < beforeCount; k++) {
                            unconditionalEventStreamer.emitEvent(node, 'worker-log', payload as Exclude<EventPayload, import('~/common/types/session-events').SessionHeartbeatEvent>);
                        }
            ...
            fc.assert(
                fc.property(
                    ...
                    ({ nodeId, eventCount }) => {
                        ...
                        for (let k = 0; k < eventCount; k++) {
                            unconditionalEventStreamer.emitEvent(node, 'critical-error', payload as Exclude<EventPayload, import('~/common/types/session-events').SessionHeartbeatEvent>);
                        }
            ...
            fc.assert(
                fc.property(
                    ...
                    ({ nodeId, eventCount }) => {
                        ...
                        for (let k = 0; k < eventCount; k++) {
                            unconditionalEventStreamer.emitEvent(node, 'session-state', sessionPayload as Exclude<EventPayload, import('~/common/types/session-events').SessionHeartbeatEvent>);
                        }
            ...
            fc.assert(
                fc.property(
                    ...
                    ({ nodeIdA, nodeIdB, eventCount }) => {
                        ...
                        const payload = makePayload('session-state');
                        for (let k = 0; k < eventCount; k++) {
                            unconditionalEventStreamer.emitEvent(nodeA, 'session-state', payload as Exclude<EventPayload, import('~/common/types/session-events').SessionHeartbeatEvent>);
                        }
