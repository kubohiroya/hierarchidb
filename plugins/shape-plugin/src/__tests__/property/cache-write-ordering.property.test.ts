...
describe('Property 6: Cache Write Ordering', () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ nodeId, bandIndex, sourceKey, dataSize }) => {
          ...
          await writeGeometryCacheWithTracking(id, nodeId, bandIndex, sourceKey, data, operations);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ nodeId, sourceKey, dataSize }) => {
          ...
          await writeSourceCacheWithTracking(id, nodeId, sourceKey, data, operations);
   ...
   */
  it(
    'should maintain write ordering for concurrent geometry cache writes',
    { timeout: 15000 },
    async () => {
      await fc.assert(
        fc.asyncProperty(
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ nodeId, bandIndex, sourceKey, dataSize }) => {
          ...
          await Promise.all([
            writeGeometryCacheWithTracking(geomId, nodeId, bandIndex, sourceKey, data, operations),
            writeSourceCacheWithTracking(sourceId, nodeId, sourceKey, data, operations),
          ]);
...
describe('Property 11: Cache Type Consistency', () => {
   ...
   */
  it(
    'should apply consistent timestamp-based validity rules across cache types',
    { timeout: 15000 },
    async () => {
      await fc.assert(
        fc.asyncProperty(
   ...
   */
  it(
    'should apply consistent cleanup behavior across cache types',
    { timeout: 15000 },
    async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ runId, nodeId, invalidCount, validCount }) => {
            ...
            const allGeomData = await db.geometryCache.where('nodeId').equals(nodeId).toArray();
            ...
            const allSourceData = await db.sourceCache.where('nodeId').equals(nodeId).toArray();
