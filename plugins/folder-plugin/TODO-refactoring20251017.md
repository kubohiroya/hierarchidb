...
- folder-plugin の `src/ui/preconnect.ts` / `src/ui/components/folder-host.tsx` は新方式に沿っているが、`src/ui/plugin.ts` や `src/ui/components/FolderUIPlugin.tsx` が未整理のまま残っている。
...
3. 他プラグイン（shape/location/route 等）で同様の旧 `components` 利用が残っていないか横断調査し、必要なら共通のToDoへ登録する。
   ...
   - 2025-10-17 着手: 上記2ファイルを削除し、`src/ui/preconnect.ts` の副作用 import を `./components/steps-provider.js` へ更新。
