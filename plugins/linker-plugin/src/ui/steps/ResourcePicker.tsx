...
const injected =
  typeof window !== 'undefined'
    ? (window as { __HDB_TreeConsolePanel?: React.FC<TreeConsolePanelProps> })
        .__HDB_TreeConsolePanel
    : undefined;
...
export const ResourcePicker: React.FC<ResourcePickerProps> = ({ value, onChange, notice }) => {
  ...
  const setsRef = useRef<{
    ancestor: Set<string>;
    self: Set<string>;
    selfClosure: Set<string>;
    descendant: Set<string>;
  }>({ ancestor: new Set(), self: new Set(), selfClosure: new Set(), descendant: new Set() });
  ...
  const computeSelectionSets = useCallback(
    async (query: TreeQueryAPI) => {
      try {
        const nodeApi = query;
        ...
        setsRef.current.selfClosure = new Set<string>(
          [...closure].filter((id) => !selfSet.has(id))
        );
      } catch {
        setsRef.current = {
          ancestor: new Set(),
  ...
  const applySearchAndBadges = useCallback(
    (rows: Row[]) => {
