...
export const parseWorldPortIndexCsv = async (
  ...
  const mapped = await Promise.all(
    rows.map(async (row: string[]) => {
      ...
      const rawCountryName = getColumnValue(
        row,
        headerIndex,
        'country',
        'country_name',
      ...
      const admin0Code = rawCountryCode && rawCountryCode.length === 2 ? rawCountryCode : undefined;
      const admin0Name =
        rawCountryName && (!admin0Code || rawCountryName.length > 2) ? rawCountryName : undefined;
