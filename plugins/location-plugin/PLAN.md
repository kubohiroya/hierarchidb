# Location Plugin リファクタリング計画（build/download 再利用）

最終更新: 2025-09-06

本ドキュメントは、`location-plugin` を `shape-plugin` から抽出された共通フィーチャ（`@hierarchidb/build`, `@hierarchidb/download`）を再利用して、ダウンロード→ビルド処理→ベクトルタイル生成を行う流れにリファクタリングするための実行計画です。
...
- `shape-plugin` で整理した共通コンポーネント（build, download）を横展開し、実装重複を排除するとともに、信頼性/再現性/保守性を高める。
- 計算/タイル化の共通基盤は現状未整備のため、当面は `location-plugin` 側の実装で対応する。
- 目標は「データソースからのダウンロード→ビルド分割→計算（変換/タイル化）→保存→UI で進捗/再開/失敗時再試行」ができる一連のパイプラインを `location-plugin` に組み込むこと。

## 成果物（DoD）

- ビルド実行の API 群（開始/状態/キャンセル/再開）が `location-plugin` 経由で提供される。
- ダウンロード/計算/タイル生成の各ステージが `@hierarchidb/{build,download}` を経由して実行される（計算/タイル化は当面 plugin 側実装）。
...
- 機能フラグは既定 OFF（`LOCATION_BUILD_VTILES_V1`）。

## スコープ

- IN: ダウンロード→ビルド分割→計算（座標系/属性正規化）→ベクトルタイル生成→保存→UI 進捗表示。
- OUT: 高度なスタイリング/UI 編集機能、地図表示コンポーネントの刷新、外部サービスへの公開/配信。

## 依存関係

- `@hierarchidb/build`: セッション/タスク/進捗/キャンセル/リトライ。
...
- Worker 側に Location 用の軽量オーケストレータ（`LocationBuildOrchestrator`）を追加。
- 3 ステージの明示:
  1) `download`（ソース分割/取得/一時保存）
  2) `compute`（ETL: 座標/属性整形、タイル化）
  3) `finalize`（成果物の格納/インデックス更新）
- すべて `BuildService` でセッション/タスク化。`mapChunks` ベースでチャンク並列、上限は `concurrency` に集約。
...
1) `startLocationBuild` 呼び出し（UI/CLI）
   - 入力: `source`, `area`（任意: bbox/tileset）、`zRange`, `concurrency`, `retryPolicy`。
   - セッション作成 → チャンク生成 → `BuildService` へエンキュー。
2) `download` タスク
...
4) `finalize` タスク
   - インデックス更新、不要キャッシュ削除。
5) 進捗・再開
   - `getBuildStatus` でセッション集計、`resumeBuild` で未完/失敗タスクのみ再投入。

## Worker API 仕様（案）

```ts
// location-plugin worker API (型は _obsolate_common-type 参照)
startLocationBuild(nodeId: NodeId, cfg: LocationBuildConfig): Promise<SessionId>;
getLocationBuildStatus(sessionId: SessionId): Promise<LocationBuildStatus>;
resumeLocationBuild(sessionId: SessionId): Promise<boolean>;
cancelLocationBuild(sessionId: SessionId): Promise<boolean>;
getVectorTile(nodeId: NodeId, z: number, x: number, y: number): Promise<Uint8Array | null>;
```

- `LocationBuildConfig` には `source`, `area`, `zRange`, `concurrency`, `retryPolicy` など。
- 返却の `LocationBuildStatus` はタスク数/成功/失敗/ETA/カレントステージを含む。

## 再利用コンポーネント適用ポイント

- `@hierarchidb/build`
  - `BuildService.createSession`, `mapChunks`, `retryWithBackoff`, `resume/cancel` をそのまま利用。
...
1. feat/location/build-skeleton（基盤）
   - Orchestrator/DB スキーマのスケルトン、型のみ。フラグ `LOCATION_BUILD_VTILES_V1` 追加（既定 OFF）。
...
3. feat/location/start-download
   - `startLocationBuild` と `download` タスク実装。`DownloadService` 統合。キャッシュ保存と重複回避（`payloadHash`）対応。
4. feat/location/compute-tiles
   - `compute` タスク実装（正規化→タイル化→保存）。`getVectorTile` 最小実装。スモークテスト追加。
5. feat/location/finalize-and-resume
   - `finalize` 実装、`resume/cancel` 実装、`getLocationBuildStatus` 集計ロジック。
6. ui/location/build-controls (任意/最小)
...
- `LOCATION_BUILD_VTILES_V1`（既定 OFF）
...
- `LocationBuildOrchestrator` (worker)
- `startLocationBuild`, `getLocationBuildStatus`, `resumeLocationBuild`, `cancelLocationBuild`, `getVectorTile`
