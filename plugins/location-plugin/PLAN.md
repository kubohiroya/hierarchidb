# Location Plugin リファクタリング計画（batch/download/compute 再利用）

最終更新: 2025-09-06

本ドキュメントは、`location-plugin` を `shape-plugin` から抽出された共通フィーチャ（`@hierarchidb/batch`, `@hierarchidb/download`, `@hierarchidb/compute`）を再利用して、ダウンロード→バッチ処理→ベクトルタイル生成を行う流れにリファクタリングするための実行計画です。
...
- `shape-plugin` で整理した共通コンポーネント（batch, download, compute）を横展開し、実装重複を排除するとともに、信頼性/再現性/保守性を高める。
...
- ダウンロード/計算/タイル生成の各ステージが `@hierarchidb/{batch,download,compute}` を経由して実行される（自前実装の排除）。
...
- `@hierarchidb/compute`: 座標変換/属性正規化/タイル化（VT creation pipeline）。
...
3) `compute` タスク
   - `ComputeService` で正規化→タイル化。結果を `vectorTiles` または `cache` に保存。
4) `finalize` タスク
   - インデックス更新、不要キャッシュ削除。
5) 進捗・再開
