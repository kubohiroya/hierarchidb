...
function analyzeUiPackages() {
  ...
  if (
    duplicateFunctions.some(
      ([name]) => name === 'devLog' || name === 'devError' || name === 'devWarn'
    )
  ) {
  ...
  if (duplicateFunctions.some(([name]) => name === 'validateExternalURL')) {
    recommendations.push({
      ...
      solution:
        'Consolidate validation functions in @hierarchidb/core or create @hierarchidb/ui-utils',
    });
  }
  ...
  const report = {
    ...
    details: {
      ...
      commonImports: commonImports.map(([imp, packages]) => ({
        import: imp,
        packages: [...packages],
      })),
    },
