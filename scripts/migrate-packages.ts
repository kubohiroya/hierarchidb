...
const PACKAGE_MAPPINGS: PackageMapping[] = [
  ...
  { 
    ...
    newPath: 'packages/30-app',
    oldName: '@hierarchidb/app',
    newName: '@hierarchidb/30-app'
  },
  // 40-level: Backend
  { 
    ...
    try {
      ...
      for (const depType of depTypes) {
        if (packageJson[depType]) {
          ...
          for (const [dep, version] of Object.entries(packageJson[depType] as Record<string, string>)) {
