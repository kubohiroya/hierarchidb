...
function main(): void {
  ...
  const auditRules: readonly Rule[] = [primaryExportRule, roleSuffixRule, viewPatternRule];
  ...
  progressDone(
    `Filtered to ${String(thresholdResults.length)} files above ${String(options.threshold)} lines`
  );

  // Apply batch limit to threshold results if specified
  const batchedResults =
    options.batch !== null ? thresholdResults.slice(0, options.batch) : thresholdResults;
  ...
  for (let i = 0; i < totalFiltered; i++) {
    ...
    progress(
      `Analyzing structure... (${String(i + 1)}/${String(totalFiltered)}) ${result.file.relativePath}`
    );
