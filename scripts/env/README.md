...
```
# CommandProcessor 経由で create/update を有効化
export WORKER_USE_CMDPROC_CREATE_UPDATE="1"

# CommandProcessor 経由で move/remove を有効化（Phase 2）
export WORKER_USE_CMDPROC_MOVE_REMOVE="1"

# Working Copy commit を V2 フローに切替（戻り: ok/CONFLICT/NAME_CONFLICT）
...
CIや単発実行では、コマンド先頭に付けることもできます。

```
WORKER_USE_CMDPROC_CREATE_UPDATE=1 pnpm --filter @hierarchidb/runtime-worker test -r
WORKER_USE_CMDPROC_MOVE_REMOVE=1 pnpm --filter @hierarchidb/runtime-worker typecheck
