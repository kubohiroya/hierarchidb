...
function sanitizeManifest(manifest, { packageDescription } = {}) {
...
export async function generatePluginRegistry() {
  ...
  for (const name of pluginPkgs) {
    ...
    const packageDescription = typeof json?.description === 'string' ? json.description : undefined;
    const manifest = sanitizeManifest(manifestRaw ?? pluginMeta, { packageDescription });
...
`;
  ...
  const workerTypesHeader = `// AUTO-GENERATED FILE. DO NOT EDIT.\n// Generated by scripts/generate-plugin-loader.mjs\n`;
  const workerTypesBody = `export type EntitiesOverrideFactory =\n  | PeerEntitiesDBAdapter\n  | (() => PeerEntitiesDBAdapter | Promise<PeerEntitiesDBAdapter | undefined> | undefined)\n  | (() => Promise<PeerEntitiesDBAdapter | undefined>);\n\nexport type EntitiesOverrideRegistry = Record<string, EntitiesOverrideFactory>;\n\ndeclare global {\n  var __HDB_PLUGIN_ENTITY_OVERRIDES__: EntitiesOverrideRegistry | undefined;\n}\n\nexport { type PeerEntitiesDBAdapter, type EntitiesOverrideRegistry };`;

  const registryAmbientChanged = await writeFileIfChanged(
    runtimeWorkerTypesFile,
    `${workerTypesHeader}${workerTypesBody}\n`,
  );
