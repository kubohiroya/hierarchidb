...
#
# Requirements:
# - git installed and configured with a remote named "origin"
# - One of:
#   - GitHub CLI `gh` (preferred), authenticated
#   - GITHUB_TOKEN env var (repo scope) for REST fallback
#
# Usage examples:
#   scripts/create-pr.sh -t "feat(worker): entity lifecycle skeleton"       # base=main, draft=false
#   scripts/create-pr.sh -t "draft: peer impl" -B develop --draft            # base=develop, draft PR
#   scripts/create-pr.sh -t "bulk ops" -d ./pr-body.md                       # body from file
#
...
if [[ -z "${PR_TITLE}" ]]; then
  # fallback to last commit subject
...
# Fallback to REST API using GITHUB_TOKEN
