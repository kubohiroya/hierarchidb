...
async function checkExternalLicenses() {
  try {
    ...
    await writeFile(
      join(rootDir, 'docs/licenses.json'),
      JSON.stringify(externalLicenses, null, 2)
    );
...
`;
    ...
    await writeFile(
      join(rootDir, 'docs/licenses-summary.txt'),
      summaryText
    );
    ...
    for (const [pkg, info] of Object.entries(externalLicenses)) {
      const [name, version] = pkg.lastIndexOf('@') > 0 ? 
    ...
    await writeFile(
      join(rootDir, 'docs/licenses.csv'),
      csvContent
    );
