...
import { analyzeStructure, buildDependencyGraph } from '../structureAnalyzer.js';
...
describe('buildDependencyGraph', () => {
  ...
  it('extracts const variables as const kind', () => {
    const { project } = createProjectWithSource('test.ts', `export const MAX_SIZE = 100;`);
    const sf = project.getSourceFileOrThrow('test.ts');
    const graph = buildDependencyGraph(sf);
  ...
  it('detects intra-file references between symbols', () => {
    const { project } = createProjectWithSource(
      'test.ts',
      ['function helper() { return 42; }', 'export function main() { return helper(); }'].join('\n')
    );
    const sf = project.getSourceFileOrThrow('test.ts');
    const graph = buildDependencyGraph(sf);
...
describe('analyzeStructure', () => {
  ...
  it('handles files with no exports', () => {
    const project = new Project({ useInMemoryFileSystem: true });
    const content = ['function internal() { return 1; }', 'const SECRET = 42;'].join('\n');
