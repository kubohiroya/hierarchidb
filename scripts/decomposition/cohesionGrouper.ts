...
function inferGroupRole(symbols: readonly SymbolNode[]): GroupRole {
  ...
  if (symbols.every((s) => s.kind === 'const' && UPPER_SNAKE_RE.test(s.name))) {
    return 'constants';
  }

  // If ALL symbols are non-exported pure functions → 'utils'
  if (symbols.every((s) => !s.isExported && (s.kind === 'function' || s.kind === 'local'))) {
    return 'utils';
  }
  ...
  if (symbols.some((s) => s.kind === 'function' && PASCAL_CASE_RE.test(s.name))) {
    return 'component';
  }
...
export function groupByCohesion(graph: DependencyGraph): CohesionGroup[] {
  ...
  for (let i = 0; i < sccs.length; i++) {
    ...
    const lineCount = symbols.reduce((sum, s) => sum + (s.endLine - s.startLine + 1), 0);
