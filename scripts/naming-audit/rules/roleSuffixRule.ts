...
export function hasCorrectSuffix(stem: string, role: SingleRole): boolean {
  ...
  // Also accept singular form for hyphenated patterns (e.g. "id-util" for utils)
  const singularStandalone = standalone.replace(/s$/, '');
  return (
    stem.endsWith(suffix) ||
    stem === standalone ||
    stem.endsWith(`-${standalone}`) ||
    stem === singularStandalone ||
    stem.endsWith(`-${singularStandalone}`)
  );
}
