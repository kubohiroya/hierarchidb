  ...
  lifecycle: {
    hooks: {
      ...
      onEnable: async (context) => {
        // WebWorkerの初期化
        await context.workerRegistry?.registerWorker('shapes', '/workers/ShapesWorker.js');
      },
      onDisable: async (context) => {
        await context.workerRegistry?.unregisterWorker('shapes');
      },
...
export class ShapesEntityHandler extends EntityHandler<ShapesEntity, never, ShapesWorkingCopy> {
  ...
  async deleteEntity(nodeId: TreeNodeId): Promise<void> {
    // カスケード削除
    await Promise.all([
      this.coreDB.table('shapes').delete(nodeId),
...
import { FileDropzone } from '@hierarchidb/ui-core/components/FileDropzone';
...
import { ColorPicker } from '@hierarchidb/ui-core/components/ColorPicker';
import type { ShapesEntity } from '../types';
