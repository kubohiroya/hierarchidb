...
- Holder-based Archive: deletion and recover operations flow through “archive holders” under the per-tree archive root. See `adr-draft-archive-unification`.
...
- Move to Archive and Remove commands also rely on worker-side checks; UI doesn’t guess. We only optimistically disable actions when selection is empty.
...
- Table-state persistence (e.g., column widths) is stored per page (node). These entries are removed only on permanent deletion from Archive (empty-all or single permanent delete), not on move-to-archive.
...
- `packages/runtime/worker/docs/adr/adr-draft-archive-unification.md`
