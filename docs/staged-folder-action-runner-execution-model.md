...
2. CLI runner は config を parse/validate し、Worker / IndexedDB progress に `validating-config` を報告する。
   - Phase 0 CLI は `--dry-run` validation host として提供し、manifest / CLI option validation と normalized execution plan の JSON 出力を行う。Phase 1 初期では `runStagedFolderActionCli()` が optional execution host injection を受け付け、`--dry-run` なしの実行を注入済み host に委譲できる。bundled entrypoint はまだ host を注入しないため、実 Worker / IndexedDB / browser profile に接続して runner を起動する標準 CLI bridge は後続 phase とする。
...
Phase 0 では service-level tests で IndexedDB persistence、generic `running-action`、`auth-required` の保持、BuildSessionRuntimeRecord 投影、subscribe 初期 snapshot、active run deletion guard を固定する。runtime-worker bootstrap は staged-folder-action adapter を既存 `CanonicalBuildRuntimeAdapterRegistry` に登録する。UI 統合時も専用 route や別 progress SSOT を追加してはならない。
...
Phase 2 child #1590 の UI 表示は runner behavior を変更しない。TreeTable / Breadcrumb / node info panel は `@hierarchidb/build-api` resolver の `status` / `reason` / `details[]` を表示用 helper に渡し、disabled Build entry の理由と diagnostics entry を表示する。Phase 3 child #1608 では、これらの UI surface は同じ resolver input として `BuildDependencyAvailabilitySummary` と plugin prerequisite failure diagnostics を受け取り、dependency error、schema error、orphaned edge、unsupported participant、plugin prerequisite failure を `build-not-required` と混同しない。UI は dependency store を直接読まず、runtime-worker / build-api 境界から渡された summary だけを表示入力として扱う。runner は引き続き resolver の contract violation を fail-fast として扱い、UI 表示の有無で build target collection や action failure 判定を変えてはならない。
...
Phase 2 child #1602 では、`export-csv` / `export-xlsx` の shared export file action runner factory を `@hierarchidb/staged-folder-action/export-file-host` に追加する。Node 専用 host は browser app が import する root entrypoint から再 export しない。factory は `outputBasePath`、Step2 adapter 由来の `materializeRows` port、file writer port、optional XLSX writer port を明示 input とする。相対 output path は `outputBasePath` 基準の絶対 path に解決し、host 境界でも絶対 path、NUL、空 segment、`.` segment、`..` segment を fail-fast する。CSV writer は shared host が提供し、XLSX writer は host boundary の外から注入する。shared host は location/route 固有の effective data resolution や Step2 schema mapping を推測せず、plugin adapter が materialized rows と canonical columns を返す。manifest の `columns` が指定された場合、host は canonical columns に含まれない column を fail-fast する。row cell は string、finite number、boolean、null、undefined のみを許可し、object/array/NaN/Infinity は fail-fast する。`export-csv` / `export-xlsx` action result は runner record の `actionResults` に保存し、CLI success JSON は record の typed result (`outputPath`、`entityType`、`rowCount`、XLSX の `sheetName`) をそのまま返す。runtime-worker は injected runner result の `type` と `entityType` が実行中 action と一致しない場合、contract violation として run を失敗させる。
