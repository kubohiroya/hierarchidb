...
- Route canonical progress and commands use the Worker-owned canonical API. Location
  command ownership may remain separate during migration, but its canonical event
  source emits the same `source`, `geometry`, and `tileEmit` stages for the
  LocationPoint SSOT + derived MVT pipeline.
...
## Location Event Examples

Location uses the same four canonical events as Shape and Route. The stage ids are
`source`, `geometry`, and `tileEmit`; consumers must not special-case Location as a
single-stage producer.

### Location `sessionStatusUpdated`

When Location starts the derived MVT build, the active stage is `source`:
...
After source completion and geometry start, `stageId` changes to `geometry`. After
geometry completion and tile emission start, `stageId` changes to `tileEmit`.
`stageStartedAt` and `stageInactiveMs` remain required whenever `stageId` is present.

### Location `stageSnapshotUpdated`

The source stage reports authoritative tasks for data-source × country × location-type
planning. The geometry stage reports LOD / tile-relation planning tasks. The tileEmit
stage reports `z/x/y` tile tasks.
...
```typescript
{
  ...
  payload: {
    ...
    tasks: [
      {
        taskId: 'location:geometry:z5-8:airport:shard-0001',
        status: 'running',
        progress: 45,
        version: 3,
      },
...
All task progress values must be finite numbers in `[0, 100]`. A missing
`renderRank`, `importance`, `iconKey`, `labelClass`, or `minZoom` in the LocationPoint
input is not represented as skipped progress; it is a contract failure that reaches
the session as `failed`.

### Location `taskProgressUpdated`

Tile emission progress is per tile task and uses per-task monotonic versions:

```typescript
{
  type: 'taskProgressUpdated',
  payload: {
    taskId: 'location:tile:z8:x223:y101',
    version: 7,
    stageId: 'tileEmit',
    value: 80,
    message: 'encoded location_points',
    metadata: {
...
The metadata object is diagnostic event metadata only. It does not relax the MVT
feature property schema and must not carry arbitrary LocationPoint metadata.
...
- Route and Location emit the `source`, `geometry`, and `tileEmit` stages. Neither
  emits snapshots for unstarted stages.
