...
### 4.6.1 UIモジュールの分割構成
UIは機能別に番号プレフィックス付きパッケージに分割されている：

- **@hierarchidb/10-ui-core**: 基本UIコンポーネント
  - Material UI（MUI）コンポーネント
  - テーマシステムと共通スタイル
  - 通知システム（Toast、Notification）
  - アイコンとアニメーション

- **@hierarchidb/10-ui-auth**: 認証関連
  - OAuth2/OIDC認証コンポーネント
  - 認証コンテキストとフック
  - ユーザー情報表示

- **@hierarchidb/10-ui-routing**: ルーティング
  - React Routerベースのナビゲーション
  - ルート定義とURLヘルパー

- **@hierarchidb/10-ui-i18n**: 国際化
  - i18next設定と言語切り替え
  - 翻訳リソース管理

- **@hierarchidb/11-ui-xxx**: 拡張UIモジュール群
  - 各種専門的なUIコンポーネント
...
**重要**：本プロジェクトでは、Viteのエイリアス設定を使用せず、Turbo watchモードによる自動ビルドを採用している。
...
この方法により：
- vite.config.tsの複雑なエイリアス設定が不要
- TypeScriptの型チェックが確実に動作
- 新しいパッケージ追加時も設定変更不要
