...
プロジェクトでは、BaseMap（基図管理）、Styler（スタイル設定）、Shape（地理情報処理）、Spreadsheet（表データ管理）、Project（プロジェクト管理）などの各プラグインも、ここで定義される共通ガイドラインに従って開発されています。特にプラグイン開発者は、4.6節のUIモジュール分割構成と4.7節のWorker/DB設計規約を重点的に確認してください。
