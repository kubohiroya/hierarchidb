...
**参照した設計文書**: packages/core/src/types/entity-types.ts（エンティティ基底インターフェース群）
...
### 3.3 互換性要件
- 🟢 既存プラグイン（basemap, styler）の後方互換性維持
...
- **型定義**: packages/core/src/types/entity-types.ts（全インターフェース）
...
- **実装可能性**: 確実（既存のbasemap/stylerで部分実装済み、段階的移行計画あり）
