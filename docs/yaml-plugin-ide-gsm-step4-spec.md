...
- page loadではstorageを読まず、exact single `mode=pre | post | recovery-pre | recovery-post`とユーザーのbutton click 1回でread-only inspectionを正確に1回だけ開始する。自動実行、poll、retry、reload、root navigation、localStorage/sessionStorage/cookie/clipboard write、network送信を禁止する。
