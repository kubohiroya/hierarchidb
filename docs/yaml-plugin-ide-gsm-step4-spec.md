...
- YAML dialogのproduction connectorをYAML save / save-draftの唯一のrouteとし、canonical writerが発行するmetadataとdataを1つの`TreeNodeUpdater` requestとしてcommitする。汎用hostからyaml-fileだけvalidationを迂回して直接writeせず、port failure後にlegacy dialog writerへ戻さない。
