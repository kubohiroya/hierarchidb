...
## Removed IDE-GSM mounted filesystem projection

The IDE-GSM mounted filesystem projection, mounted project command UI, mounted node ID encoding, and `VITE_MOUNTED_IDE_GSM_COMMAND_UI_ENABLED` startup flag have been removed. YAML Step 4 remains the only YAML-specific IDE-GSM command flow described by this document.

Current IDE-GSM project browsing, CSV materialization, command execution, and disconnected read behavior are defined by [IDE-GSM Project Node Specification](./idegsm-project-node-spec.md). New records must reject `mountKind`, `mountId`, `sourceKind`, `originMountId`, `originSourceKind`, scalar `projectId`, endpoint, credential, absolute path, and raw content fields instead of copying them into synchronized records.
