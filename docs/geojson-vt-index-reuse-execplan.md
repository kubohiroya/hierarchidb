...
- Decision: geojson-vt index は `packages//src/EphemeralShapeDB.ts` の新テーブルに保存する。
...
4) vectortile ステージで index を復元してタイルを生成し、保存する経路を追加する。
   ...
   - 必要なら `packages//src/vectorTiles.ts` に tile エンコード補助関数を追加する。
