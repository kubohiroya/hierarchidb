...
| `effective-staged` | staged-folder-action が action input として読む view。copy-on-write と `patchData` を反映し、Phase 4 child #1638 以降は完了済み `import-mount` action result の mount record が reference/dependency resolver input へ渡される。resolver は mounted content が必要な action で mount record を検証できる |
...
Phase 4 child #1638 では、runner が `resolveReferences` input に完了済み `actionResults` と `importMounts` を渡す。これにより、`import-mount` 後の build/export/capture action は resolver 境界で mount record の有無を検証できる。呼び出し側で独自に mount record を混ぜたり、`mountedContentApplied: false` を mounted content 適用済みとして扱ったりしてはならない。
