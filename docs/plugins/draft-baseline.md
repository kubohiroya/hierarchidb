...
- **メタ情報は最小限**: `DraftBase<TEntity>` が提供する `treeNodeId` / `createdAt` / `updatedAt` / `originalVersion?` のみを保持する。`isDraft` や `schemaVersion`、独自 ID などは追加しない。
...
| `updatedAt` | `Timestamp` | 直近で WC を更新した時刻。`markDraftUpdated` で更新する。 |
| `originalVersion?` | `number` | 既存エンティティから派生した場合のオリジナル版数。新規作成時は省略可能。 |

## 推奨ヘルパーの使い方
```ts
import { createDraftDraftBase, markDraftUpdated } from '@hierarchidb/plugin-loader-base-plugin';

const draft = createDraftDraftBase<MyEntity>({
  draft: {
    name: '',
...
const updated = markDraftUpdated(draft, { name: 'New name' });
```

- `createDraftDraftBase` の `meta.treeNodeId` は必須。新規ノードの場合もツリー側で採番した ID を渡す。
- `markDraftUpdated` は `draft` とメタ情報を同時に更新する唯一の経路とする。

## 実装指針（Worker 側）
- `BaseEntityHandler` / `HierarchicalEntityHandler` を継承するクラスでは以下のメソッド契約を守る。
  - `createDraft(nodeId: NodeId)` は必ず `createDraftDraftBase` を経由して WC を生成し、Dexie の `workingCopies` テーブルに `treeNodeId` をキーとして保存する。
  - `createNewDraftDraft(nodeId: NodeId)` も同様にヘルパーを利用し、未入力フィールドは `draft` のみで表現する。
  - `commitDraft` / `discardDraft` は `draft` の内容を元にエンティティを更新・削除する。UI 専用フィールドに依存しない実装にする。
- Dexie スキーマでは WC テーブルに余計なインデックスを追加せず、`treeNodeId` を Primary Key とする。

## 実装指針（UI 側）
- WC の読み書きには `@hierarchidb/ui-core` の `useDraft` 等の既存フックを用い、`setDraft` 時は `markDraftUpdated` と同様の構造を維持する。
...
- `schemaVersion` / `isDraft` / `draftId` といった冗長なフィールドを追加すること。

## DoD チェックリスト
- [ ] プラグインごとの `Draft` 型が `DraftDraft<TEntity>` で定義されている。
- [ ] Worker Handler が `createDraftDraftBase` と `markDraftUpdated` を利用している。
- [ ] UI コンポーネントが WC の `draft` を前提に実装され、UI 固有状態は別管理になっている。
- [ ] `pnpm --filter @hierarchidb/*-plugin typecheck` と関連ユニットテストがグリーン。
- [ ] `docs/plugins/draft-initial-payloads.md` の内容と実装が一致している。

## 関連ドキュメント
- `docs/plugins/draft-initial-payloads.md`: プラグインごとの初期値例。
