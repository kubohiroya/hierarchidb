...
🎯 座標データを処理する
🎉 対象task/importが契約違反として失敗する
🎉 最初に検出したfeature/polygon/coordinateの位置と不正値がエラー詳細に記録される
🎉 ユーザーに修正可能なエラーメッセージが表示される
🎉 元データと既存artifactは変更されない
🚫 経度・緯度をclamp、丸め、wrapして処理を継続しない
🚫 invalid geometry filteringによるdrop + warningへ変換しない
```
