...
- **テスト名**: 経度±180度、緯度±90度の境界値処理
  ...
  - **境界値での動作保証**: 経度 `[-180, 180]`、緯度 `[-90, 90]` は受理し、その外側は契約違反として拒否
- **入力値**:
  ```json
  {
    "coordinates": [
...
- **期待される結果**:
  - payload 内の範囲外座標 `[181.0, 91.0]` を検出して対象task/import全体を失敗させる
  - feature/polygon/coordinateの位置と不正値を含むエラーを表示する
  - `[180.0, 90.0]` と `[-180.0, -90.0]` を含む入力だけなら値を変更せず受理する
  - **境界での正確性**: clamp、丸め、wrapを行わない
  - **一貫した動作**: 失敗payloadから部分artifactを保存せず、既存artifactを変更しない
- **テストの目的**: EDGE-101の境界値処理確認
  - **堅牢性の確認**: 不正な座標値を可視なcontract errorとして扱い、システム全体は継続動作する
