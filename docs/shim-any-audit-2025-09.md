...
- 2025-09-18: App の `@hierarchidb/ui-theme` / `@hierarchidb/ui-auth` / `@hierarchidb/ui-treeconsole-toolbar` / `@hierarchidb/node-type-folder-plugin` シムを削除し、正式 d.ts へ移行。`@hierarchidb/common-type` の ambient からも `ui-theme` シムを除去済み。
...
- 2025-09-19 23:05: RouteBatchManager / RouteBatchSession / RouteEntitiesDB の Dexie 操作から `as any` を撤廃し、進捗通知のレガシー互換 I/F を型安全に再実装。`packages/node-type/route-plugin` の `as any` 件数を 93 → 75 まで削減。
- 2025-09-19 23:32: SearouteEngine / OsrmEngine / download registry / config / net ポート周辺を型安全化し、環境変数解決・共有ダウンロードサービス連携を見直し。`packages/node-type/route-plugin` の `as any` は 44 件まで縮減。
- 2025-09-19 23:58: RouteBatchOrchestrationService / SourceOrchestrator / CSV・GeoJSON 戦略を正式型で再構築し、UI LaunchForm まで型付けを連携。`packages/node-type/route-plugin` の `as any` は 18 件まで減少、ワークスペース総数は 747。
...
- 2025-09-20 01:20: Shape plugin の UI hooks / ダイアログ / auth 連携から `as any` を撤廃し、MUI props を公式型に合わせて整備。`packages/node-type/shape-plugin` の `as any` は 30 件まで減少し、ワークスペース総数は 697。
- 2025-09-20 01:35: Shape plugin の Dexie GroupStore とバッチ起動 API を型安全化し、`packages/node-type/shape-plugin` は 26 件、ワークスペース総数は 693 に減少。
- 2025-09-20 01:55: Shape plugin の UI パネル／Map preview／Worker API まわりを調整し、`packages/node-type/shape-plugin` は 11 件、ワークスペース総数は 678 に減少（残りはテスト／モックのみ）。
...
- 2025-09-20 02:45: Location plugin のダイアログ/パネル/バッチ UI を公式型へ揃え、公開アダプタからの `as any` を撤廃。`pnpm --filter @hierarchidb/node-type-location-plugin typecheck` グリーン、ワークスペース総数は 644（location-plugin は 41 件）。
- 2025-09-20 03:05: Location plugin の Dexie ベース stores を型安全化し、peer/group/relation ストアから `as any` を排除。`pnpm --filter @hierarchidb/node-type-location-plugin typecheck` / `pnpm as-any:report` で location-plugin 33 件 / ワークスペース 636 件を確認。
- 2025-09-20 03:25: Location plugin のダウンロード戦略とバッチ制御（registry/Overpass/Nominatim/SessionManager）を型整備し、`as any` を実装コードから 10 件まで削減。`pnpm --filter @hierarchidb/node-type-location-plugin typecheck` / `pnpm as-any:report` で location-plugin 10 件 / ワークスペース 613 件を確認。
- 2025-09-20 03:35: Location plugin の LocationVectorTileService / SessionController / Custom search ルートを型安全化し、実装コードの `as any` を 0 件へ。`pnpm --filter @hierarchidb/node-type-location-plugin typecheck` / `pnpm as-any:report` で location-plugin 0 件 / ワークスペース 603 件を確認。
- 2025-09-20 19:05: TreeConsole breadcrumb / timeline plugin から react-router / react-transition-group シムを撤去し、`packages/common/types/src/@types/react-transition-group` で Node16 対応の公式型を再定義。`pnpm --filter @hierarchidb/ui-treeconsole-breadcrumb typecheck && build` と `pnpm --filter @hierarchidb/node-type-timeline-plugin typecheck && build` を実行し問題なし。
...
- 2025-09-20 07:55: StylerEntityHandler の基底ハンドラ戻り値を型安全にアンラップし、spreadsheetMetadataId の参照漏れを防止。`pnpm --filter @hierarchidb/node-type-styler-plugin typecheck` / `pnpm as-any:report` で styler-plugin 18 件 / ワークスペース 421 件を確認。
...
| ~~`packages/node-type/timeline-plugin/src/types/rtg-bridge.d.ts`~~ | `react-transition-group` | **削除済 (2025-09-20)** | `@types/react-transition-group` を導入済み。tsconfig から typeRoots を撤去し公式型でビルド/型検証が通ることを確認 |
...
| `packages/node-type/feature/map-source/src/types/dexie.d.ts` など | 外部ライブラリ | 公式型未提供 | ライブラリの型確認／導入 or コメント付き維持 |
...
| `packages/node-type/spreadsheet-plugin` | 30 | CSV ドライバ / フィルタユーティリティ |
| `packages/ui/core` | 25 | UI foundation（入力フォーム/ダイアログ） |
| `packages/node-type/styler-plugin` | 18 | UI steps・dialog |
...
| `packages/node-type/basemap-plugin` | 10 | Map preview / Store 旧ロジック |
| `packages/node-type/resolver-plugin` | 10 | ResolverDialog UI |
| `packages/ui/data-grid` | 10 | InMemoryDataProvider など旧モデル依存 |
| `packages/ui/i18n` | 9 | i18n 初期化（lazy load） |
| `packages/node-type/linker-plugin` | 8 | リソース選択ステップ |
| `packages/feature/map-adapter` | 6 | MapLibre アダプタ周辺 |
| `packages/feature/tabular` | 6 | タブラー書き出しユーティリティ |
...
top packages:
  ...
  packages/node-type/shape-plugin             52
  packages/node-type/location-plugin          48
  packages/ui/treeconsole                     42
  packages/node-type/styler-plugin            37
  packages/ui/auth                            32
...
top packages:
  ...
  packages/node-type/styler-plugin            37
  packages/ui/auth                            32
  packages/node-type/shape-plugin             30
  packages/node-type/spreadsheet-plugin       30
  packages/ui/core                            25
  packages/backend/bff                        16
  packages/node-type/folder-plugin            16
```
...
top packages:
  ...
  packages/backend/bff                        16
  packages/node-type/folder-plugin            16
  packages/feature/download                   13
  packages/feature/tabular-store              12
...
top packages:
  ...
  packages/runtime-worker/worker             127
  packages/ui/treeconsole                     42
  packages/node-type/location-plugin          41
  packages/node-type/styler-plugin            37
  packages/ui/auth                            32
  packages/node-type/spreadsheet-plugin       30
  packages/ui/core                            25
...
top packages:
  ...
  packages/ui/i18n                             9
  packages/node-type/linker-plugin             8
  packages/feature/import-export               7
  packages/node-type/base-plugin               7
```
...
top packages:
  ...
  packages/runtime-ui/plugin-dialog           12
  packages/node-type/basemap-plugin           10
  packages/node-type/location-plugin          10
  packages/node-type/resolver-plugin          10
  packages/ui/data-grid                       10
```
...
top packages:
  ...
  packages/ui/treeconsole                     42
  packages/node-type/styler-plugin            37
  packages/node-type/location-plugin          33
  packages/ui/auth                            32
  packages/node-type/spreadsheet-plugin       30
  packages/ui/core                            25
  packages/backend/bff                        16
...
top packages:
  ...
  packages/node-type/styler-plugin            18
  packages/runtime-worker/worker              14
  packages/node-type/spreadsheet-plugin       12
  packages/ui.treeconsole                     12
  packages/backend/bff                        10
  packages/node-type.basemap-plugin           10
  packages/node-type.resolver-plugin          10
