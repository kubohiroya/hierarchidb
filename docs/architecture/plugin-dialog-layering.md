...
| **Runtime Host UI** | `packages/plugin-ui-host/src` | `PluginDialogShell`, `PluginDialogFooter`, `PluginDialogHeader` などホストアプリ向け UI コンポーネントとラッパー | `react`, `@mui/*`, `@tanstack/react-router`, `@hierarchidb/runtime-client` |
...
| **App Host Wiring** | `app/src/router/routes/tree/PluginDialogRoute.tsx` | React Router との接続、WorkingCopy ID リダイレクト、完了時ナビゲーション<br>`PluginDialogShell` を直接利用し、ホスト固有オプションを注入 | `@tanstack/react-router`, `@hierarchidb/runtime-client`, `@hierarchidb/plugin-ui-host` |

### 1.1 現状の問題点
- ヘッドレスロジックが `plugin-ui-sdk` と `ui/plugin-dialog` の双方に散在し、責務境界が不明瞭。
- `PluginDialogShell` がアプリ固有のルーティング／WorkingCopy 処理を一部抱え、ホストごとの再利用が難しい。
...
- `@hierarchidb/plugin-ui-host` は UI コンポーネント（シェル、フッター、ステッパー）とホストラッパーに集中させる。
