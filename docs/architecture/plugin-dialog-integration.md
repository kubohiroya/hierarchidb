...
* `@hierarchidb/ui-worker-client` provides `registerWorkerClientHook` / `getWorkerClientHook`. `root.tsx` registers the app’s `useWorkerAPIClient` hook, making it discoverable for all plugin packages at runtime.【F:packages/runtime/worker-bootstrap/src/ui/workerClientHook.ts†L1-L18】【F:app/src/root.tsx†L58-L79】
