...
- Dev proxy: set `VITE_BFF_BASE_URL` for `/auth` proxy in dev (see `app/vite.config.ts`).
- Build: `pnpm build` (runs license analysis in prebuild).

## For AI Agents

- Start with `AGENTS.md` for repository‑specific rules and pitfalls.
- Then see `CONTRIBUTING.md` for packaging/export policies and PR conventions.
