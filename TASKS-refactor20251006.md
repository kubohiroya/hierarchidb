...
- **共通設計フィードバック適用（docs/plugins/common-working-copy-refactor-feedback.md）**
  - [x] 1. WorkingCopyDraft 構造の厳格化: base-plugin の型/ヘルパー更新＆ Location プラグイン型定義の追従完了（他プラグイン展開は別タスクで管理）。
    ...
    - 2025-10-06 23:25 done: Route プラグインの `tsconfig.json` へローカルな `@hierarchidb/plugins-base-plugin` パスを追加し、`RouteEntity` / WorkingCopy ヘルパーの型を整備。`pnpm --filter @hierarchidb/plugins-route-plugin typecheck` が sandbox 環境でもグリーンになった。
