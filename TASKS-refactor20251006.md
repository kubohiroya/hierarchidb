...
- **共通設計フィードバック適用（docs/plugins/common-working-copy-refactor-feedback.md）**
  - [x] 1. WorkingCopyDraft 構造の厳格化: base-plugin の型/ヘルパー更新＆ Location プラグイン型定義の追従完了（他プラグイン展開は別タスクで管理）。
    ...
    - 2025-10-06 23:07 blocked: `pnpm --filter @hierarchidb/plugins-route-plugin typecheck` を実行したが、sandbox 上で `@hierarchidb/plugins-base-plugin` の型参照欠如（TS2307）と既知の `metadata/customFields` フィールド未定義により失敗。ユーザー環境で依存パッケージの dist-types を解決した後に再実行が必要。
