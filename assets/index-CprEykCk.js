...
import { Suspense, createContext, lazy, useCallback, useEffect, useMemo, useRef, useState } from "react";
import { Box, IconButton, InputAdornment, List, ListItem, ListItemText, Stack, TextField, Tooltip, Typography } from "@mui/material";
import { jsx, jsxs } from "react/jsx-runtime";
import "comlink";
import { DualKeyMap } from "@hierarchidb/util";
import { ArrowBack, ArrowForward, ExpandLess, ExpandMore, Search } from "@mui/icons-material";
...
//#region ../../packages/ui/worker-provider/src/provider/WorkerSingletonProvider.tsx
const WorkerContext = createContext(null);

//#endregion
//#region ../../packages/ui/worker-provider/src/ui/workerClientHook.ts
let currentHook = null;
function getWorkerClientHook() {
	if (!currentHook) throw new Error("Worker client hook is not registered. Please ensure registerWorkerClientHook is called at app startup.");
	return currentHook;
}
...
var en_default = {
	steps: {
		"resources": { "label": "Select Resources" },
		"aggregated": { "label": "Aggregated Paths" },
	...
	aggregated: { "description": "Aggregated resources from ancestors/self/descendants ({{count}} items)" },
	preview: {
		"description": "Preview of aggregated resources on the map (generalized).",
...
var ja_default = {
	steps: {
		"resources": { "label": "リソース選択" },
		"aggregated": { "label": "集約パス" },
		"preview": { "label": "プレビュー" }
	},
	resourcePicker: {
		"title": "リソース",
		"notice": "コンソールからリソースを選択してください。複数選択できます。データは読み取り専用です。",
		"columns": { "name": "名前" }
	},
	aggregated: { "description": "祖先/自身/子孫から集約したリソース（{{count}} 件）" },
	preview: {
		"description": "集約リソースのマッププレビュー（簡易表示）。",
...
const useTranslation = (ns = "linker-plugin") => {
...
//#endregion
//#region src/ui/steps/ResourcePicker.tsx
const TreeConsolePanel = (typeof window !== "undefined" ? window.__HDB_TreeConsolePanel : void 0) ?? (() => null);
const ResourcePicker = ({ value, onChange, notice }) => {
	const { t } = useTranslation();
	const [selected, setSelected] = useState(value || /* @__PURE__ */ new Set());
	...
	const setsRef = useRef({
		ancestor: /* @__PURE__ */ new Set(),
		self: /* @__PURE__ */ new Set(),
		selfClosure: /* @__PURE__ */ new Set(),
		descendant: /* @__PURE__ */ new Set()
	});
	...
	const toRows = (nodes, depth) => (nodes || []).map((node) => ({
		id: String(node.id),
	...
	const toNodeId = (value$1) => value$1;
	const toTreeId = (value$1) => value$1;
	const ensureChildren = useCallback(async (parentId) => {
		if (cacheRef.current.has(parentId)) return;
		try {
			const hook = getWorkerClientHook() ?? null;
			const ref = hook ? hook() : null;
			if (!ref) return;
			let query = null;
			try {
				query = await ref.getAPI().getQueryAPI();
			} catch {
				return;
			}
			if (!query) return;
	...
	const computeSelectionSets = useCallback(async (query) => {
		try {
			const nodeApi = query;
			const selfSet = new Set(Array.from(value || /* @__PURE__ */ new Set()));
			setsRef.current.self = selfSet;
			const closure = /* @__PURE__ */ new Set();
			for (const id of selfSet) try {
				(await nodeApi.listDescendants(toNodeId(id)))?.forEach((d) => closure.add(String(d.id)));
			} catch {}
			const ancestorLike = /* @__PURE__ */ new Set();
			const descendantLike = /* @__PURE__ */ new Set();
			setsRef.current.ancestor = ancestorLike;
			setsRef.current.descendant = descendantLike;
			setsRef.current.selfClosure = new Set([...closure].filter((id) => !selfSet.has(id)));
		} catch {
			setsRef.current = {
				ancestor: /* @__PURE__ */ new Set(),
				self: new Set(value || []),
	...
	const applySearchAndBadges = useCallback((rows) => {
	...
	const panelProps = useMemo(() => ({
		...
		treeId: "r",
		data: treeData,
		pageNodeId: breadcrumb.length ? String(breadcrumb[breadcrumb.length - 1]?.id) : void 0,
		nodeIndex: (() => {
			const index = new DualKeyMap();
			const parentId = breadcrumb.length ? breadcrumb[breadcrumb.length - 1]?.id : "root";
			treeData.forEach((row) => {
				const primary = row.id;
		...
		columns: [{
			id: "name",
			label: t("resourcePicker.columns.name", "Name")
		}],
		...
		onNodeExpand: (id, isExp) => {
			const next = new Set(expanded);
			isExp ? next.add(id) : next.delete(id);
			setExpanded(next);
		},
		onSearchChange: (t$1) => setSearchTerm(t$1),
		onSearchClear: handleSearchClear,
		onExpandAll: handleExpandAll,
		onCollapseAll: handleCollapseAll,
		onBreadcrumbNavigate: async (nodeId) => {
			...
			setTreeData(applySearchAndBadges(cacheRef.current.get(String(nodeId)) || []));
		},
		onNavigateBack: async () => {
			const prev = backStack.current.pop();
			if (!prev) return;
			const last = breadcrumb[breadcrumb.length - 1];
			if (last) fwdStack.current.push({
				id: String(last.id),
				path: [...breadcrumb]
			});
			setBreadcrumb(prev.path);
			await ensureChildren(String(prev.id));
			setTreeData(applySearchAndBadges(cacheRef.current.get(String(prev.id)) || []));
		},
		onNavigateForward: async () => {
			const next = fwdStack.current.pop();
			if (!next) return;
			const last = breadcrumb[breadcrumb.length - 1];
			if (last) backStack.current.push({
				id: String(last.id),
				path: [...breadcrumb]
			});
			setBreadcrumb(next.path);
			await ensureChildren(String(next.id));
			setTreeData(applySearchAndBadges(cacheRef.current.get(String(next.id)) || []));
		},
		onContextMenuAction: () => {},
		onCreate: () => {},
		onEdit: () => {},
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				const rootId = tree?.rootId ? String(tree.rootId) : void 0;
				if (!rootId) return;
				await computeSelectionSets(query);
				const rows = toRows(await query.listChildren(toNodeId(rootId)), 1);
	...
	return /* @__PURE__ */ jsxs(Box, {
		sx: { p: 2 },
		children: [
			notice && /* @__PURE__ */ jsx(Typography, {
				variant: "body2",
			...
			/* @__PURE__ */ jsx(Typography, {
				...
				children: "Legend: ● Self, ○ Self’s descendants, ▲ Ancestors’ liked, ■ Descendants’ liked"
			}),
			/* @__PURE__ */ jsxs(Stack, {
				direction: "row",
				spacing: 1,
				...
				children: [
					...
					/* @__PURE__ */ jsx(Tooltip, {
						...
						children: /* @__PURE__ */ jsx("span", { children: /* @__PURE__ */ jsx(IconButton, {
							size: "small",
							children: /* @__PURE__ */ jsx(ArrowForward, { fontSize: "small" })
						}) })
					}),
					/* @__PURE__ */ jsx(Tooltip, {
						title: "Expand all",
						children: /* @__PURE__ */ jsx("span", { children: /* @__PURE__ */ jsx(IconButton, {
							...
							children: /* @__PURE__ */ jsx(ExpandMore, { fontSize: "small" })
						}) })
					}),
					/* @__PURE__ */ jsx(Tooltip, {
						title: "Collapse all",
						children: /* @__PURE__ */ jsx("span", { children: /* @__PURE__ */ jsx(IconButton, {
							...
							children: /* @__PURE__ */ jsx(ExpandLess, { fontSize: "small" })
						}) })
					}),
			...
			/* @__PURE__ */ jsx(TreeConsolePanel, { ...panelProps })
		]
	});
};

//#endregion
//#region src/ui/steps/AggregatedList.tsx
function resolveWorkerClient() {
	const hook = getWorkerClientHook() ?? null;
	const ref = hook ? hook() : null;
	if (!ref) return null;
	try {
...
const AggregatedList = ({ selfNodeId, selected }) => {
	const { t } = useTranslation();
		...
		(async () => {
			try {
				...
				if (client && selfNodeId) {
					...
					const ancestorsAll = await query.listAncestors(selfNodeId);
					const descendantsAll = await query.listDescendants(selfNodeId);
					const linkerAnc = (ancestorsAll || []).filter((n) => n?.nodeType === "linker");
					const linkerDesc = (descendantsAll || []).filter((n) => n?.nodeType === "linker");
					const collectFrom = async (nodes) => {
						for (const ln of nodes) try {
							const liked = (await query.getNode(ln.id))?.data?.likedNodeIdSet;
							const arr$1 = Array.isArray(liked) ? liked : liked instanceof Set ? Array.from(liked) : [];
							for (const id of arr$1) {
								const s = { nodeId: String(id) };
								base.set(s.nodeId, s);
							}
						} catch {}
					};
	...
	return /* @__PURE__ */ jsxs(Box, {
		...
		children: [/* @__PURE__ */ jsx(Typography, {
			...
			color: "text.secondary",
			sx: { mb: 1 },
			children: t("aggregated.description", "Aggregated resources from ancestors/self/descendants ({{count}} items)", { count: useMemo(() => items.length, [items]) })
		}), /* @__PURE__ */ jsx(List, {
			dense: true,
			children: items.map((it) => /* @__PURE__ */ jsx(ListItem, { children: /* @__PURE__ */ jsx(ListItemText, {
				primary: it.name || it.nodeId,
				secondary: it.nodeType
			}) }, it.nodeId))
		})]
	});
};
...
const MapPreview = ({ items: _items }) => {
	...
	return /* @__PURE__ */ jsxs(Box, {
		...
		children: [/* @__PURE__ */ jsx(Typography, {
			...
			color: "text.secondary",
			sx: { mb: 1 },
			children: t("preview.description", "Preview of aggregated resources on the map (generalized).")
		}), /* @__PURE__ */ jsx(Box, {
			sx: {
				border: "1px solid",
				borderColor: "divider",
			...
			children: /* @__PURE__ */ jsx(Suspense, {
				fallback: /* @__PURE__ */ jsx(Box, {
					...
					children: /* @__PURE__ */ jsx(Typography, {
						variant: "caption",
						color: "text.secondary",
						children: t("preview.loading", "Loading map preview…")
					})
				}),
...
//#endregion
//#region src/ui/components/hooks/useLinkerSteps.ts
const useLinkerSteps = () => {
	...
	const toResourceSummaries = (value) => Array.from(value).map((id) => ({ nodeId: String(id) }));
	return {
		ensureDraft,
		toSelectionSet,
		toResourceSummaries
...
const ResourcesStepWrapper = (props) => {
	const { ensureDraft, toSelectionSet } = useLinkerSteps();
	const draft = ensureDraft(props.data);
	const { t } = useTranslation$1();
	return /* @__PURE__ */ jsx(ResourcePicker, {
		value: toSelectionSet(props.data?.draftData?.linkedNodeIds),
		onChange: (nextSet) => props.onChange({
			...draft ?? {},
			draftData: {
				...draft?.draftData ?? {},
				linkedNodeIds: Array.from(nextSet)
			}
		}),
		notice: t("resourcePicker.notice", "Select resources from the console. Multiple selection is allowed. Data is read-only.")
	});
};
const AggregatedStepWrapper = (props) => {
	const { toSelectionSet, toResourceSummaries } = useLinkerSteps();
	const selected = toResourceSummaries(toSelectionSet(props.data?.draftData?.linkedNodeIds));
	return /* @__PURE__ */ jsx(AggregatedList, {
		selfNodeId: String(props.nodeId ?? ""),
		selected
	});
};
const PreviewStepWrapper = (props) => {
	const { toSelectionSet, toResourceSummaries } = useLinkerSteps();
	return /* @__PURE__ */ jsx(MapPreview, { items: toResourceSummaries(toSelectionSet(props.data?.draftData?.linkedNodeIds)) });
};
const createLinkerStepConfigs = () => {
	const { t } = useTranslation();
	return [
		{
			id: "resources",
			label: t("steps.resources.label", "Select Resources"),
			componentFactory: (props) => /* @__PURE__ */ jsx(ResourcesStepWrapper, { ...props }),
			validate: (data) => Array.isArray(data?.draftData?.linkedNodeIds) && data.draftData.linkedNodeIds.length > 0
		},
		{
			id: "aggregated",
			label: t("steps.aggregated.label", "Aggregated Paths"),
			componentFactory: (props) => /* @__PURE__ */ jsx(AggregatedStepWrapper, { ...props }),
			validate: () => true
		},
		{
			id: "preview",
			label: t("steps.preview.label", "Map Preview"),
			componentFactory: (props) => /* @__PURE__ */ jsx(PreviewStepWrapper, { ...props }),
			validate: () => true
		}
	];
};
