...
import { f as summarizeCheckboxState, i as createAdapterFromProgressSubscribe, l as mergeProcessingConfig, o as useBatchProgress, p as validateProcessingConfig, t as isShapePreviewMetadataEnabled, u as normalizeDataSourceName } from "../previewFlags.js";
import { i as DEFAULT_PROCESSING_CONFIG, n as getEphemeralShapeDB } from "../EphemeralShapeDB.js";
import { t as metadataLoader } from "../MetadataLoader.js";
...
import { Accordion, AccordionDetails, AccordionSummary, Alert, Box, Button, Chip, CircularProgress, FormControl, FormControlLabel, FormGroup, FormHelperText, FormLabel, Grid, LinearProgress, Paper, Radio, RadioGroup, Rating, Slider, Snackbar, Stack, Switch, Tab, Tabs, TextField, Typography } from "@mui/material";
...
function resolveProcessingConfigInvalidation(prevConfig, nextConfig) {
...
const DownloadConfigSection = ({ config, draft, disabled, onChange }) => {
	...
	const deleteLabel = counts.raw > 0 ? t("processing.download.deleteDownloadedFilesWithCount", "Delete Downloaded Files ({{count}} files)", { count: counts.raw }) : t("processing.download.deleteDownloadedFiles", "Delete Downloaded Files");
	useEffect(() => {
		let cancelled = false;
		if (!sessionId) {
			setCounts({
	...
	const handleDeleteStage = useCallback(async (stage) => {
		if (!sessionId) return notify.warning("SessionId is missing.");
		await db.clearStage(sessionId, stage);
		const [next1, next2] = await Promise.all([db.simplifiedBuffers.where({
			sessionId,
			stage: "simplify1"
		}).count(), db.simplifiedBuffers.where({
			sessionId,
			stage: "simplify2"
		}).count()]);
		setCounts((prev) => ({
	...
	const update = (partial) => {
		onChange(mergeProcessingConfig({
			...config,
			...partial
		}));
	};
...
const SimplificationConfigSection = ({ config, disabled, onChange }) => {
	...
	const baseHybridConfig = baseSimplificationConfig?.hybridFilterConfig ?? DEFAULT_PROCESSING_CONFIG.simplificationConfig?.hybridFilterConfig ?? {
								...
								children: /* @__PURE__ */ jsxs(Stack, {
									...
									children: [/* @__PURE__ */ jsx(Typography, {
										...
										/* @__PURE__ */ jsx(Typography, {
											variant: "caption",
											color: "text.secondary",
											children: t("processing.filter.quantizeSelected", "Selected: {value}", { value: quantizeOptions[quantizeIndex].toLocaleString() })
										}),
										/* @__PURE__ */ jsx(Typography, {
											variant: "caption",
											color: "text.secondary",
...
const TileConfigSection = ({ config, disabled, onChange }) => {
	...
	const zoomRange = minZoom <= maxZoom ? [minZoom, maxZoom] : [maxZoom, minZoom];
	return /* @__PURE__ */ jsxs(Accordion, {
		defaultExpanded: true,
		children: [/* @__PURE__ */ jsx(AccordionSummary, {
			expandIcon: /* @__PURE__ */ jsx(ExpandMore, {}),
...
const normalizeCountryCodeFromMetadata = (country, index) => {
	const primary = ([
		...
		country.iso2,
		country.iso3
	].filter((value) => Boolean(value && value.trim()))[0] ?? `country-${index}`).trim().toUpperCase();
...
const ShapeCountrySelectionStep = ({ data, onChange }) => {
	...
	const checkboxMatrix = useMemo(() => {
		if (Array.isArray(draftData.checkboxState)) return draftData.checkboxState.map((row) => {
	...
	const applySelections = useCallback((nextSelections) => {
		...
		if (!isMatrixEqual(Array.isArray(draftData.checkboxState) ? draftData.checkboxState : void 0, nextMatrix)) {
			const sessionId = resolveShapeSessionId(draftData);
			if (sessionId) clearStagesIfPresent(sessionId, FULL_INVALIDATION_STAGES);
		}
...
const ShapePreviewStep = ({ data }) => {
	...
	useEffect(() => {
		...
		setMetadataError(null);
		getShapeTileMetadataDB().then(async (db) => db.featureMetadata.where("sessionId").equals(String(sessionId)).toArray()).then((rows) => {
			if (!cancelled) setMetadataRows(rows);
		}).catch((error) => {
			if (!cancelled) setMetadataError(error instanceof Error ? error.message : "Failed to load metadata.");
		}).finally(() => {
	...
	useEffect(() => {
		...
		setMatchedIds(metadataRows.filter((row) => {
			return [
				row.countryName,
				row.countryCode,
				row.adminName,
	...
	useEffect(() => {
		...
		setSelectionContext(selectedRows.every((row) => row.countryCode === first.countryCode && row.adminLevel === first.adminLevel) && first.countryCode != null && first.adminLevel != null ? {
	...
	const hoveredIdSet = useMemo(() => hoveredId ? new Set([hoveredId]) : /* @__PURE__ */ new Set(), [hoveredId]);
	...
	const selectionForFeature = useCallback((row, current) => {
		...
		return {
			nextLevel,
			selected: metadataRows.filter((item) => item.countryCode === countryCode && item.adminLevel === nextLevel).map((item) => item.featureId)
		};
	}, [metadataRows]);
	const handleMapIdentify = useCallback((result) => {
		...
		if (nextLevel == null || !row.countryCode || selected.length === 0) setSelectionContext(null);
		else setSelectionContext({
			countryCode: row.countryCode,
	...
	useEffect(() => {
		...
		const ensureLayer = (id, color, opacity) => {
		...
		const handleIdle = () => {
			...
			ensureLayer(`${baseId}-matched`, theme.palette.secondary.light, .45);
			ensureLayer(`${baseId}-selected`, theme.palette.primary.main, .5);
			ensureLayer(`${baseId}-hovered`, theme.palette.action.hover, .6);
		};
	...
	useEffect(() => {
		...
		const handleMouseMove = (...args) => {
			const feature = args[0]?.features?.[0];
			setHoveredId((feature ? String(feature.id ?? feature.properties?.id ?? "") : "") || null);
		};
...
function toStatus(info, fallback) {
...
function useShapeProgress(sessionId, options = {}) {
	...
	const { progress: unifiedProgress, subscribed, subscribe: sharedSubscribe, unsubscribe: sharedUnsubscribe } = useBatchProgress(useMemo(() => {
		...
		return createAdapterFromProgressSubscribe((eventCallback) => bridgeRef.current.subscribeBatchProgress(SHAPE_NODE_TYPE$1, sessionId, eventCallback).then((unsubscribe$1) => {
			setError(null);
			return unsubscribe$1;
		}).catch((err) => {
			setError(err instanceof Error ? err : /* @__PURE__ */ new Error("Failed to subscribe to shape batch progress"));
			return () => {};
		}));
	}, [sessionId]), {
		autoSubscribe,
		poll: useMemo(() => {
			if (!sessionId || !enablePollingFallback) return void 0;
			return async () => {
				try {
					const nextStatus = await bridgeRef.current.getBatchSessionStatus(SHAPE_NODE_TYPE$1, sessionId);
					setStatus(nextStatus);
					return statusToUnified(nextStatus);
				} catch (err) {
					setError(err instanceof Error ? err : /* @__PURE__ */ new Error("Failed to fetch shape batch status"));
					return null;
				}
			};
	...
	return {
		...
		status: toStatus(unifiedProgress, status),
...
const ShapeBuildProgressStep = ({ data, onChange }) => {
	const { t } = useTranslation();
	...
	const effectiveStatus = Boolean(status?.status === "processing" && !data?.batchSessionId) && status ? {
		...status,
		status: "paused"
	} : status;
	...
	const stageLabel = stages.find((stage) => stage.id === currentStage)?.title ?? currentStage ?? t("build.progress.unknownStage", "processing");
	const taskLabel = progress?.currentTask ?? status?.error ?? t("build.progress.working", "Working...");
	...
	const stageProgress = useMemo(() => {
		...
		stages.forEach((stage, idx) => {
			if (stageIndex < 0) map[stage.id] = 0;
			else if (idx < stageIndex) map[stage.id] = 100;
			else if (idx === stageIndex) map[stage.id] = Math.min(100, Math.max(0, overallProgress));
			else map[stage.id] = 0;
		});
		return map;
	}, [
	...
	const paneProgress = useMemo(() => {
		return stages.map((stage) => {
			...
			const progressValue = taskCount > 0 ? Math.round(stageTasks.reduce((sum, task) => sum + (task.progress ?? 0), 0) / taskCount) : stageProgress[stage.id] ?? overallProgress;
			return {
				paneId: stage.id,
			...
			children: [
				...
				stageTasks.length === 0 ? /* @__PURE__ */ jsx(Typography, {
					...
					children: stageTasks.map((task) => {
						...
						const statusLabel$1 = resolveStatusLabel(statusValue);
						...
						return /* @__PURE__ */ jsxs(Box, {
							...
							children: [
								...
								task.message ? /* @__PURE__ */ jsx(Typography, {
									variant: "caption",
									color: "text.secondary",
	...
	const isProcessingValid = useMemo(() => validateProcessingConfig(mergeProcessingConfig(data?.processingConfig ?? DEFAULT_PROCESSING_CONFIG)).isValid, [data?.processingConfig]);
	const hasSelection = summarizeCheckboxState(data?.checkboxState).hasSelection;
	const hasDataSource = Boolean(data?.dataSourceName);
	const canStartOrResume = buildStatus !== "completed" && hasDataSource && hasSelection && isProcessingValid;
	...
	return /* @__PURE__ */ jsxs(Box, {
		...
		children: [/* @__PURE__ */ jsx(Box, {
		...
		}), hasProgressData ? /* @__PURE__ */ jsxs(Paper, {
			variant: "outlined",
			sx: {
				p: 2,
				display: "flex",
			...
			children: [
				/* @__PURE__ */ jsx(Typography, {
					variant: "subtitle2",
					children: t("build.progress.title", "Batch progress")
				}),
				sessionId ? /* @__PURE__ */ jsxs(Typography, {
					variant: "body2",
					color: "text.secondary",
					children: [
						t("build.progress.session", "Session"),
						": ",
						sessionId
					]
				}) : null,
...
const canStartShapeBatch = (data) => {
	const hasSelection = summarizeCheckboxState(data?.checkboxState).hasSelection;
	const hasDataSource = Boolean(data?.dataSourceName);
	const processingValid = validateProcessingConfig(mergeProcessingConfig(data?.processingConfig ?? DEFAULT_PROCESSING_CONFIG)).isValid;
...
//#endregion
export { useShapeAPI, useShapeAPIGetter, useShapeBatchCommand, useShapeBatchTasks, useShapeProgress };
