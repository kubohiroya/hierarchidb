...
const buildBatchSessionConfig = (batchConfig, draft) => {
	const downloadConfig = batchConfig.downloadConfig ?? DEFAULT_PROCESSING_CONFIG.downloadConfig;
	const simplificationConfig = batchConfig.simplificationConfig ?? DEFAULT_PROCESSING_CONFIG.simplificationConfig;
	...
	const resolvedDataSource = batchConfig.dataSource ?? toDataSourceName(draft?.draftData?.batchConfig?.dataSource ?? "naturalearth");
	return {
...
const shapePluginAPI = {
	...
	startBatchProcessing: async (draftId, batchConfig, urlMetadata, progressCallback) => {
		if (!batchConfig?.dataSource) throw new Error("Data source is required to start batch processing");
		...
		const downloadConfig = batchConfig.downloadConfig ?? DEFAULT_PROCESSING_CONFIG.downloadConfig;
		const baseConfig = buildBatchSessionConfig(batchConfig, { draftData: draftLike ?? void 0 });
		const processConfig = {
			...baseConfig,
