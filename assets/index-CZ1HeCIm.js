...
import { CloudDownload, ExpandMore, Filter, FilterAlt, Layers } from "@mui/icons-material";
...
//#endregion
//#region src/ui/hooks/build/useBuildStages.ts
...
const useBatchSessionActions = ({ nodeType, nodeId, sessionId, data, onChange, buildStatus }) => {
	...
	const saveDraftBeforeBatch = useCallback(async () => {
		...
		if (!nextUrlMetadata || nextUrlMetadata.length === 0) {
			const dataSource = normalizeDataSourceName(data?.batchConfig?.dataSource ?? data?.dataSourceName);
			if (!dataSource) {
			...
			try {
				...
				const fallbackMetadata = metadata.length ? metadata : SAMPLE_COUNTRIES;
				nextUrlMetadata = deriveUrlMetadataFromSelection({
					dataSource,
					checkboxState: data?.checkboxState,
					metadata: fallbackMetadata
				});
				if (!nextUrlMetadata.length) {
					notify.warning("No URL metadata is available for the current selection.");
					return false;
				}
				onChange({ urlMetadata: nextUrlMetadata });
			} catch (error) {
				console.error("[ShapeBuildProgressStep] urlMetadata generation failed", error);
				notify.error("Failed to build urlMetadata for the current selection.");
				return false;
			}
		}
		try {
	...
	return {
		handleStartOrResume: useCallback(async () => {
			...
			if (!await saveDraftBeforeBatch()) return;
			try {
				await bridgeRef.current.initialize();
				...
				if (statusResult?.sessionId) onChange({ batchSessionId: statusResult.sessionId });
			} catch (error) {
