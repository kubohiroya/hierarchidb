...
import { a as LocationBatchSessionManager, c as deleteLocationPoints, i as isLocationBatchAPIV2Enabled, l as listLocationPoints, n as UnifiedLocationBatchManager, o as appendLocationPoints, r as createLocationBatchManager, s as clearLocationPoints, t as LocationVectorTileService, u as replaceLocationPoints } from "./LocationVectorTileService.js";
import { t as isDevEnvironment } from "./env.js";
import "./registerLocationWorkerStores.js";
import { t as worker_exports } from "./worker.js";
import { i as useTranslation, n as formatNumber, t as formatBytes } from "./i18n.js";
import { t as LocationPanel } from "./LocationPanel2.js";
import { a as SelectionMatrix, i as normalizeMatrix, n as LocationSelectionStep, r as buildCheckboxState, t as LocationMapPreview } from "./LocationMapPreview.js";
import { n as closeEphemeralLocationDB, r as getEphemeralLocationDB } from "./EphemeralLocationDB2.js";
import "./RuntimeWorkerClient.js";
import { t as LocationBatchManager } from "./services.js";
import { i as init_registry, l as registry_exports } from "./registry2.js";
import { BaseEntityHandler, createDraftWorkingCopyBase, getWorkerBridge } from "@hierarchidb/plugin-service-sdk";
import { TabularQueryService as LocationTableQueryService } from "@hierarchidb/tabular-store";
import React, { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { Alert, Box, Button, Card, CardContent, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, LinearProgress, List, ListItem, ListItemIcon, ListItemText, SpeedDial, SpeedDialAction, SpeedDialIcon, Step, StepContent, StepLabel, Stepper, Tab, Tabs, Typography } from "@mui/material";
import { Assessment, CheckCircle, Close, Download, Error as Error$1, HourglassEmpty, Map, Pause, PlayArrow, Stop, TableView, Timeline, Warning } from "@mui/icons-material";
...
//#endregion
//#region src/common/entities/LocationEntityHandler.ts
function cloneMatrix(matrix) {
	return matrix ? matrix.map((row) => [...row]) : [];
}
var LocationEntityHandler = class extends BaseEntityHandler {
	tableRef;
	constructor(table) {
		super();
	...
	get table() {
		if (!this.tableRef) throw new Error("LocationEntityHandler table is not initialized.");
	...
	async createWorkingCopy(entity) {
		const draft = {
			...entity,
			selectionMatrix: cloneMatrix(entity.selectionMatrix)
		};
		const base = createDraftWorkingCopyBase({
			draft,
			meta: {
	...
	async createNewDraftWorkingCopy(nodeId) {
		const now = Date.now();
		const targetNodeId = nodeId ?? crypto.randomUUID();
		const draft = {
			nodeId: targetNodeId,
			...
			batchSessionId: void 0,
			lastProcessedAt: void 0,
			createdAt: now,
			updatedAt: now,
			version: 1,
			processingStatus: "pending"
	...
	async updateWorkingCopy(treeNodeId, updates) {
		const existing = await this.table.get(treeNodeId);
		if (!existing) throw new Error(`Location entity not found: ${treeNodeId}`);
...
//#endregion
//#region src/common/components/batch/BatchProgressDialog.tsx
/**
...
var RuntimeWiring = class {
	static registerAuthNotifier() {
		try {
			const { registerLocationAuthNotifier } = (init_registry(), __toCommonJS(registry_exports));
			registerLocationAuthNotifier((info) => {
				try {
					const scope = globalThis;
					(scope.AuthNotificationRegistry?.getInstance?.() ?? scope.authNotificationRegistry ?? scope.authRegistry)?.onAuthRequired?.(info);
				} catch {}
			});
		} catch {}
	}
	static async registerRuntimeWorkerAdapters() {
		try {
			await (await import("./registerRuntimeWorker.js")).registerLocationRuntimeWorkerAdapters();
		} catch {}
	}
};
...
async function onRegister() {
	...
	try {
		const { getEphemeralLocationDB: getEphemeralLocationDB$1 } = await import("./services/index.js");
		const db = getEphemeralLocationDB$1();
...
//#endregion
export { BatchProgressDialog, LocationBatchManager, LocationBatchSessionManager, LocationEntityHandler, LocationMapPreview, LocationPanel, PLUGIN_MANIFEST as LocationPluginManifest, LocationSelectionStep, LocationTableQueryService, LocationVectorTileService, RuntimeWiring, SelectionMatrix, UnifiedLocationBatchManager, appendLocationPoints, buildCheckboxState, clearLocationPoints, closeEphemeralLocationDB, createLocationBatchManager, deleteLocationPoints, getEphemeralLocationDB, isLocationBatchAPIV2Enabled, listLocationPoints, normalizeMatrix, onRegister, replaceLocationPoints, worker_exports as worker };
