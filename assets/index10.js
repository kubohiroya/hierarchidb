import { c as generateUrlMetadata, l as mergeProcessingConfig, p as validateProcessingConfig, s as calculateSelectionStats } from "../previewFlags.js";
import { a as SHAPE_DATA_SOURCES, i as DEFAULT_PROCESSING_CONFIG, n as getEphemeralShapeDB } from "../EphemeralShapeDB.js";
import { n as shapeDB } from "../ShapeDB.js";
...
const shapePluginAPI = {
	...
	startBatchProcessing: async (draftId, config, urlMetadata, progressCallback) => {
		...
		const batchConfig = {
			...
			dataSource: config.dataSource ?? toDataSourceName(draftLike.draftData?.dataSourceName ?? "naturalearth"),
			download: {
			...
			simplify2: {
				...
				simplify: simplificationConfig?.tolerance ?? .01,
				tolerance: simplificationConfig?.tolerance ?? .1,
	...
	getBatchSession: async (sessionId) => {
		try {
			...
			const config = mergeProcessingConfig((await getShapeEntityHandler().getEntity(nodeId))?.processingConfig ?? DEFAULT_PROCESSING_CONFIG);
...
const ShapeWorkerPlugin = {
	...
	validation: { validateEntity: async (entity) => {
		const errors = [];
		if (!entity.dataSourceName) errors.push("Data source is required");
		if (!entity.processingConfig) errors.push("Processing configuration is required");
		return {
			isValid: errors.length === 0,
			errors: errors.length > 0 ? errors : void 0
		};
