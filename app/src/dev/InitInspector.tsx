...
export const InitInspector: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const update = async () => {
      try {
        ...
        const initMod = await import('../initWorkerClient');
    ...
    const onEvt = () => setState(s => ({ ...s, lastInitEventTs: Date.now(), channelMessageCount: s.channelMessageCount + 1 }));
    try { window.addEventListener('hierarchidb-worker-init-complete', onEvt as any); } catch {}
    return () => { mounted = false; window.clearInterval(id); try { window.removeEventListener('hierarchidb-worker-init-complete', onEvt as any); } catch {}; };
  }, []);
  ...
  return (
    <div style={{ position: 'fixed', bottom: 10, right: 10, zIndex: 2147483647, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace' }}>
      <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.12)', padding: 10, minWidth: 280 }}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
          <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: 9999, background: color, marginRight: 8 }} />
        ...
        <div style={{ fontSize: 12, lineHeight: 1.5 }}>
          ...
          <div>Event count: {state.channelMessageCount} / last: {state.lastInitEventTs ? new Date(state.lastInitEventTs).toLocaleTimeString() : '-'}</div>
          <div>Route: {state.route}</div>
        </div>
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
          <button onClick={ping} style={{ fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid #d1d5db', background: '#f3f4f6' }}>Ping</button>
          <button onClick={forceEvent} style={{ fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid #d1d5db', background: '#f3f4f6' }}>Dispatch INIT</button>
          <button onClick={clearCaches} style={{ fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid #fecaca', background: '#fee2e2', color: '#b91c1c' }}>Clear Caches</button>
        </div>
