...
const createWorkerEndpoint = (): { endpoint: Endpoint; terminate?: () => void } => {
  assertOriginCoordinatorOwnedClientCreationAllowed();
  const worker = new Worker(new URL('./createGeosWorkerApi.ts', import.meta.url), {
    type: 'module',
  });
...
const getWorkerEndpointFactory = (): WorkerEndpointFactory =>
  createWorkerEndpointOverride ?? createWorkerEndpoint;
...
const callWithClient = async <T>(
  call: (client: Remote<GeosWorkerApi>) => Promise<T>
): Promise<T> => {
...
export const geosWorkerClient = {
  ...
  bbox: (geojson: GeoJSON): Promise<Bbox | null> =>
    callWithClient((client) => client.bbox(geojson)),
  clip: (feature: Feature, bbox: Bbox): Promise<Feature | null> =>
    callWithClient((client) => client.clip(feature, bbox)),
  simplify: (
    geojson: GeoJSON,
    tolerance: number,
  ...
  isValid: (geojson: GeoJSON): Promise<boolean> =>
    callWithClient((client) => client.isValid(geojson)),
  ...
  contains: (left: GeoJSON, right: GeoJSON): Promise<boolean> =>
    callWithClient((client) => client.contains(left, right)),
  shutdown: (): void => shutdownWorker(),
};

// Test-only hook to inject a custom Worker endpoint factory.
export const setGeosWorkerEndpointFactoryForTests = (
  factory: WorkerEndpointFactory | null
): void => {
