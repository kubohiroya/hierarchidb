...
const logDatabaseWarning = (nodeType: string, error: unknown): void => {
  ...
  console.warn(`[plugin-runtime/databases] Failed to prewarm ${nodeType} database`, error);
};
...
function logPrewarmDescriptorWarning(
  ...
  if (error) {
    console.warn(`[plugin-runtime/databases] ${detail}`, error);
  } else {
    console.warn(`[plugin-runtime/databases] ${detail}`);
  }
}
