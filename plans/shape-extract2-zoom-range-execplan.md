...
- Task input data types live in `packages//src/ShapeDB.ts` and `packages/plugin-service-api/src/types/shapeBuildTypes.ts`.
...
1. Extend `Extract2TaskInputData` in `packages//src/ShapeDB.ts` and `packages/plugin-service-api/src/types/shapeBuildTypes.ts` with `zoomRange`, `zoomRangeLabel`, and `tolerance`.
