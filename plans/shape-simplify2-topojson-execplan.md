# Simplify2 を TopoJSON ベースに切り替え、Step4 デフォルトを適正化する
...
ユーザーが geoBoundaries の簡略化済み GeoJSON を使ってビルドしたとき、Step4 の過度なデフォルト値が原因で境界が崩れるのを避ける。simplify2 の処理を TopoJSON ベースに切り替え、同一タイル座標内で境界を共有したまま簡略化することで、国境や行政境界の破綻を抑える。完了後は、Step4 のデフォルト値が穏当な設定に戻り、simplify2 の結果がタイル内で整合的になることを UI とビルドログで確認できる。

## Progress

- [x] 2025-12-30 18:20 JST ExecPlan を作成した。
- [x] 2025-12-30 18:55 JST topojson-simplify を追加し、TopoJSON 簡略化ユーティリティを実装した。
- [x] 2025-12-30 19:05 JST simplify2 の worker/local 実装を TopoJSON ベースの簡略化に切り替えた。
...
- Decision: TopoJSON ベースの簡略化は simplify2 のみに適用する。
  Rationale: simplify1 は広域・大量処理のためコストが大きく、境界整合の恩恵は simplify2 の最終段で最大化できるため。
...
- Decision: TopoJSON の簡略化は `topojson-simplify` を導入して行う。
  ...
  Date/Author: 2025-12-30, Codex
- Decision: タイル単位の grouping は simplify2 タスクの `zoomLevels` を使い、bbox 中心点でタイルを一意に決める。
...
simplify2 の処理は次の2経路がある。

- Worker 経由: `plugins/shape-plugin/src/services/batch/workers/shapeStageWorker.ts` の `processSimplify2Task` が `simplifyGeoJson` を使って簡略化している。
- ローカル経由: `plugins/shape-plugin/src/services/batch/adapters/LocalSimplifyAdapters.ts` の `LocalSimplify2Adapter` が同じ `simplifyGeoJson` を使う。

`simplifyGeoJson` は `packages/features/gis-sdk/src/processing/geometrySimplify.ts` にあり、各 Feature を独立に簡略化するため共有境界がずれる可能性がある。
...
まず simplify2 の TopoJSON 化を行うために `topojson-simplify` を `plugins/shape-plugin/package.json` に追加する。次に、TopoJSON 変換・簡略化・復元を担う小さなユーティリティを `plugins/shape-plugin/src/services/batch/utils/topojsonSimplify.ts`（新規）に実装する。ユーティリティは `FeatureCollection` を受け取り、タイル単位で Topology を作る。タイル単位の grouping は `Simplify2Task.zoomLevels` の最小値を使い、bbox の中心点から tile 座標を算出して Feature を一意に割り当てる。bbox が取得できない Feature は変換せず、そのまま戻す。各タイルごとに Topology を作り、`presimplify`→`simplify` を実行し、`feature` で FeatureCollection に戻す。最後にタイルごとの結果を結合し、元の Feature の properties と id を保持する。

simplify2 の worker 版 (`shapeStageWorker.ts`) と local 版 (`LocalSimplifyAdapters.ts`) の両方で、`simplifyGeoJson` 直前にこの TopoJSON 簡略化を挟み込む。既存の retry による tolerance/quantize の調整は継続するが、TopoJSON の simplify には「同じ tolerance を使う」方針で統一する。変換に失敗した場合は従来の per-feature 簡略化へフォールバックし、失敗がタスク全体を止めないようにする。

Step4 のデフォルト値は、geoBoundaries の `simplifiedGeometryGeoJSON` を前提に過度な値を戻す。具体的には次の値に変更する。

- simplify1: tolerance 8.0、minVertexCountForAreaFilter 1500、areaThreshold 10000（据え置き）、enablePerFeatureSimplification は true
- simplify2: tolerance 2.5、quantize 2000、enablePerFeatureSimplification は true
...
2. `plugins/shape-plugin/src/services/batch/utils/topojsonSimplify.ts` を新規作成し、以下の機能を実装する。
...
3. `plugins/shape-plugin/src/services/batch/workers/shapeStageWorker.ts` の `processSimplify2Task` に TopoJSON 簡略化を挿入し、失敗時は既存の `simplifyGeoJson` にフォールバックする。
4. `plugins/shape-plugin/src/services/batch/adapters/LocalSimplifyAdapters.ts` の `LocalSimplify2Adapter` でも同様に TopoJSON 簡略化を使用する。
...
- simplify2 の Completed メッセージが出ること（従来通り）。
...
- 主要な変更対象:
  ...
  - `plugins/shape-plugin/src/services/batch/adapters/LocalSimplifyAdapters.ts`
  - `plugins/shape-plugin/src/services/batch/utils/topojsonSimplify.ts`（新規）
