...
- [ ] (2026-02-08 10:10 JST) 既存の WorkerAPI 初期化経路と BuildSession の Broadcast 経路を調査し、影響箇所の一覧を作成する。
- [ ] (2026-02-08 10:10 JST) SharedWorker エントリと Dedicated Worker エントリを両立させ、Web Locks の singleton 制御を追加する。
- [ ] (2026-02-08 10:10 JST) WorkerClient 側を SharedWorker 対応に再編し、Dedicated Worker フォールバックを保つ。
- [ ] (2026-02-08 10:10 JST) BroadcastChannel を `packages/session-coordinator` の設計に寄せて再統合する。
- [ ] (2026-02-08 10:10 JST) typecheck を実行し、結果を TASKS.md に記録する。
...
次に `WorkerAPIClient` と `WorkerStateStore` を SharedWorker 対応のクライアント取得 API に寄せる。`WorkerAPIClient.getOrInit()` の内部で「SharedWorker が使えるなら SharedWorker を優先」するロジックに置き換え、Dedicated Worker でしか利用できない API を分離する。`getRawWorkerInstance` に相当する関数は MessagePort 版を用意し、初期化チャネルが同じ手順で完了判定できるようにする。
...
ビルドセッションの通知は `packages/runtime-worker/src/services/buildSessionBroadcast.ts` を再設計する。`packages/session-coordinator` の `channelName` と整合する BroadcastChannel を使い、タブ超えの「存在/動作状況」通知のみを送る。`subscribeBuildSessionRecordsByStatus` の refresh トリガはこの BroadcastChannel を使い続けるが、payload の定義を `session-coordinator` と一致させる。TreeSubscriptionAPI など重要通知は Comlink callback のまま。
...
3. `app/src/worker-runtime/WorkerAPIClient.ts` と `app/src/worker-runtime/WorkerStateStore.ts` を SharedWorker でのクライアント取得に対応させる。`getRawWorkerInstance` は `getRawWorkerHandle`（`Worker | MessagePort | null`）に置き換え、初期化チャネルで共通に扱えるようにする。

4. `app/src/contexts/WorkerProvider.tsx` で SharedWorker 接続の初期化状態を正しく反映できるように調整する。`WorkerClientProxy` の API は維持し、内部の `ensureWorkerInitialized` が SharedWorker を優先する。

5. SharedWorker のエントリファイルを新設し、`Comlink.expose` が `MessagePort` を `Comlink.expose(api, endpoint)` で公開する構成にする。Dedicated Worker では現行 `worker.ts` を維持する。

6. `packages/runtime-worker/src/services/buildSessionBroadcast.ts` の BroadcastChannel 実装を `packages/session-coordinator` の channel 名と payload 仕様に合わせる。`useBuildSessionSnapshots` が受け取る更新トリガが「タブ超えの存在/動作状況通知」に限定されるようにする。

7. 影響範囲の typecheck を `pnpm -w turbo run typecheck --filter @hierarchidb/app --filter @hierarchidb/runtime-worker --filter @hierarchidb/ui-worker-client --filter @hierarchidb/session-coordinator` で実行し、結果を TASKS.md に記録する。
...
- BroadcastChannel の通知 payload は `packages/session-coordinator` のハートビート設計と整合させ、`useBuildSessionSnapshots` が最小限の更新トリガとして扱えること。
