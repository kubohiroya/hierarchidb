...
The shape plugin’s generated features are stored in IndexedDB via Dexie in `packages/features/shape-store/src/ShapeDB.ts`, exposed as the singleton `shapeDB`. The route plugin’s generated line strings are stored in `packages/features/route-store/src/RouteDB.ts` under the `lineStrings` table. The location plugin writes tabular data into the tabular store (RowStoreDB + metadata) via `TabularWriter` with filenames like `location-<nodeId>.json` and persists the `tableId` into `EphemeralLocationDB.sessions` (see `packages/features/location-store/src/LocationDB.ts`). The tabular store can be queried from the UI with `TabularQueryService` and `SimpleTableMetadataManager` in `@hierarchidb/tabular-store`.
