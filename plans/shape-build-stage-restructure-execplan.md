...
Finally, wire the new pipeline into `shapeVtPipeline.ts` behind a default-off flag. When the flag is off, keep the existing behavior (legacy pipeline). When the flag is on, run the new stage handlers and use the new storage tables. Update any cleanup logic (`cleanupConfig`) to delete the new persistent tables when requested. Validate that Step5 still shows the three stages and that Step6 renders tiles generated by the new pipeline. Record and document rollback steps: turn off the flag, delete new tables if needed, and revert code.
...
Identify pipeline and storage touch points:
  ...
  rg -n "shapeVtPipeline|shapeFetchStage|createTransformByBandHandler|vtStage" plugins/shape-plugin packages
...
- `plugins/shape-plugin/src/services/vt/shapeVtPipeline.ts`: select legacy vs new pipeline based on the feature flag and wire the new handlers.
