...
次に、shape の pipeline と vt-orchestrator を3段階に組み替える。`shapeVtPipeline.ts` から transform-by-zoom タスク作成・実行・削除処理を削除し、transform の出力を vt 段で直接参照するようにする。vt-orchestrator 側では `createTransformByZoomHandler.ts` と関連する types を削除または使用箇所から外し、transform の結果に tile index を併設する現在の仕様を保ちつつ、vt 段が参照するストアの形を明示する。taskQueue のステージ名は `fetch` / `transform` / `vt` に統一し、UI に表示されるステージ名と整合させる。
