...
Users need to understand how vertex and polygon counts change from raw downloads through simplification and into vector tiles. After this change, the Step6 metadata table shows one row per origin dataset (the downloaded file unit) with vertex and polygon totals for four stages: raw, simplify1, simplify2, and vector tiles. A user can build a dataset and immediately see those four pairs of numbers for each origin in the metadata table, instead of relying on a single per-feature statistic whose stage was unclear.
...
- Observation: The existing `ShapeTileMetadataDB.featureMetadata` rows are created from simplify2 feature buffers inside `plugins/shape-plugin/src/services/batch/SessionController.ts` (`ensureTileFeatureIndex`), not from raw downloads or vector tiles.
  Evidence: `ensureTileFeatureIndex` decodes simplify2 buffers and writes `vertexCount`/`polygonCount` to `featureMetadata`.
...
2) Ensure simplify1 and simplify2 buffers can be traced back to the same origin key.

To make vector tile aggregation possible, add a small metadata property on features so the origin survives through the tiling pipeline. Update `plugins/shape-plugin/src/services/batch/workers/shapeStageWorker.ts` to set a property like `__hdbOriginKey` on every feature when writing simplify1 outputs (and keep it when simplifying). This property will survive into simplify2 outputs and vector tiles via GeoJSON-VT.

After simplify1 and simplify2 stages complete, compute totals by reading the corresponding simplified buffers and summing geometry stats per origin key. Store the totals by updating the `sourceMetadata` table.
...
2) Add origin key handling and stage aggregation in `plugins/shape-plugin/src/services/batch/SessionController.ts`:
   ...
   - After simplify1 and simplify2, compute totals from simplified buffers and update `sourceMetadata`.
...
3) Preserve origin info in feature properties in `plugins/shape-plugin/src/services/batch/workers/shapeStageWorker.ts`:
   - When constructing simplify1 outputs, set `properties.__hdbOriginKey` to the origin key passed via task config.
   - Ensure simplify2 uses the property already present; only set if missing.
...
4) Update UI data loading and table columns:
   ...
   - `plugins/shape-plugin/src/ui/hooks/preview/useVectorTilePreviewTable.ts`: map source rows to columns for raw/simplify1/simplify2/vector tile vertex/polygon counts.
