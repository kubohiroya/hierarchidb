...
Shape の Step4 で指定した worker 数に応じて、download/simplify1/simplify2/vector tile の各ステージが実際に WebWorker を必要数だけ起動して並列処理するようにする。ユーザーは Step5 のビルド中に「ワーカー数を上げたら処理が並列化される」ことを体感できる。確認方法は、Step4 で worker 数を上げてビルドし、コンソールログとタスク進捗が従来より短時間で進むこと、および Worker の起動数が指定に追従することを観察する。

## Progress

- [x] (2025-12-29 00:04 JST) LRUSplitView の 0/0 表示を 0% に補正し、進捗の母数計算を完了+失敗+スキップに統一する方向で整理した。
- [x] (2025-12-29 00:09 JST) simplify2 の入力バッファ欠落を simplify1 失敗として扱う暫定対策を入れた。
- [x] (2025-12-29 01:25 JST) shape-plugin の専用 worker entry を追加し、download/simplify の実処理を worker 側へ移した。
...
- Observation: simplify2 で `Simplify2 input buffer not found` が発生するのは simplify1 失敗タスクを simplify2 が参照していたため。
  Evidence: ブラウザログで `Simplify2 input buffer not found: <session>-simplify1-<index>` が出る。

## Decision Log

- Decision: download/simplify は shape-plugin 内の専用 WebWorker（新規 entry）を起動し、vectortile は既存の runtime-worker worker を複数起動して並列化する。
  Rationale: download/simplify は `DataSourceStrategyFactory` など shape-plugin 依存が強く runtime-worker へ移せないため、プラグイン内 worker で完結させるほうが安全。vectortile は既存 worker を活かせる。
...
- `plugins/shape-plugin/src/services/batch/adapters/LocalSimplifyAdapters.ts`: simplify1/2 の実処理（現在は UI Worker 内）。
...
- `packages/runtime-worker/src/services/StageProcessingService.ts`: stage worker の API 実装（download/simplify/vectortile）。
...
最初に shape-plugin 内に download/simplify 用の専用 worker entry を追加する。worker 側は `DataSourceStrategyFactory` と `LocalSimplifyAdapters.ts` 相当の処理を実行し、`shapeDB` と `EphemeralShapeDB` を直接更新する。API は「タスク1件を処理する」関数として公開し、main 側のアダプタが worker プールで並列実行する。

次に shape-plugin 側でステージごとに worker プールを作る。download/simplify1/simplify2 は新規の shape-plugin worker を `new Worker(new URL('...entry.ts', import.meta.url))` で worker 数分生成し、BatchService の並列タスク実行で round-robin で worker を選ぶ。vectortile は既存の runtime-worker の `createStageWorkerClient()` を worker 数分生成する。プールのライフサイクルは「ステージ開始時に生成・ステージ終了時に terminate」とし、進捗は「登録数 = 完了+失敗+スキップ」に合わせて計算する。
...
1) shape-plugin の専用 worker entry を追加し、download/simplify 実処理を worker 側に移す。
   ...
   - 実装の要点:
    ...
    - simplify1/2: `Simplify1Task` / `Simplify2Task` を受け取り、`LocalSimplifyAdapters` と同等の処理を実行し、`EphemeralShapeDB` を更新する（`shapeDB.batchTasks` は main 側で更新）。
...
2) shape-plugin のステージアダプタを worker プール対応にする。
   ...
   - 編集: `plugins/shape-plugin/src/services/batch/adapters/LocalSimplifyAdapters.ts` を置換 or 新規 `RuntimeWorkerSimplify1Adapter` / `RuntimeWorkerSimplify2Adapter` を追加
...
3) SessionController で Step4 の worker 数を各ステージに渡す。
   ...
   - 実装の要点: download は `downloadConfig.maxConcurrent`、simplify1/2 は `simplify1Config.workers`/`simplify2Config.workers`、tile は `tileConfig.workers` を `StageControls.maxConcurrent` として渡す。
...
- Step4 で worker 数を 4 に設定してビルドすると、download/simplify/vector tile が 4 worker で並列実行される。
...
- 期待ログ例:
    ...
    [Session <id>] Simplify1 stage completed: X/Y successful

## Interfaces and Dependencies

- `plugins/shape-plugin/src/services/batch/workers/shapeStageWorker.ts` が download/simplify の処理を担う。
...
Plan revision note: download/simplify の worker 実装を runtime-worker ではなく shape-plugin 専用 worker に変更する決定を追記し、手順を更新した。
