...
- Observation: Location metadata uses `LocationEntitiesDB` with table `groupEntities` in `getDBName('location')`, while vector tiles live in `EphemeralLocationDB` using `getDBName('location-ephemeral')`.
  Evidence: `plugins/location-plugin/src/worker/locationEntitiesDB.ts`, `packages/features/location-store/src/LocationDB.ts`.
- Observation: Route vector tiles are stored in `TilesDB` (`getDBName('vectortile')`) and route metadata uses `RouteDatabase` with `lineStrings` in `getDBName('route-db')`.
  Evidence: `packages/runtime-worker/src/services/RouteQueryService.ts`, `packages/features/route-store/src/RouteDB.ts`, `packages/features/gis-sdk/src/TilesDB.ts`.
...
- `packages/features/location-store/src/LocationDB.ts` and `plugins/location-plugin/src/worker/locationEntitiesDB.ts` for location storage.
- `packages/features/route-store/src/RouteDB.ts` and `packages/features/gis-sdk/src/TilesDB.ts` for route storage.
...
2) Update location storage to `LocationDB` with `features` and `vectorTiles`.
   ...
   - Edit `packages/features/location-store/src/LocationDB.ts` to replace with the unified `LocationDB` (or rename and move) and change `getDBName('location-ephemeral')` to `getDBName('location')`.
...
3) Update route storage to `RouteDB` with `features` and `vectorTiles`.
   - Edit `packages/features/route-store/src/RouteDB.ts` to rename the class to `RouteDB` and change DB name to `getDBName('route')`.
