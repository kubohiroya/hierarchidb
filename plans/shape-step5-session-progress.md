...
4) Update `ShapeBuildStep.tsx` to derive `sessionId` from `data.nodeId` (and/or `data.batchSessionId` if present), use `useShapeProgress`, and map progress/status to `BuildStep`. Add action handlers that call worker batch control APIs.
