...
try {
  ...
  console.warn('[generate-plugin-loader] Failed to load manifest loader:', message);
...
function createLoaderEntries(packages: Array<{ pkgName: string; manifest: any }>) {
  const workerRegistrations: string[] = [];
  const uiLoaderEntries: string[] = [];
  const workerLoaderEntries: string[] = [];
  const pluginDefinitionsEntries: string[] = [];
  const pluginRegistryEntries: string[] = [];
  const muiIconNames = new Set<string>();
  const uiLoadOrder: string[] = [];
  ...
  for (const { pkgName, manifest } of packages) {
    ...
    workerRegistrations.push(`import { register${className}Worker } from '${pkgName}/worker';
register${className}Worker();`);
    uiLoaderEntries.push(`  '${nodeType}': () => import('${pkgName}/ui'),`);
    workerLoaderEntries.push(`  '${nodeType}': () => import('${pkgName}/worker'),`);
    uiLoadOrder.push(nodeType);

    const normalizedDeps = filterValidDependencies((manifest?.dependencies ?? []).map(normalizeDependency));
    const version = manifest?.version ?? '0.0.0';
    const pluginName = typeof manifest?.name === 'string' && manifest.name.trim().length > 0 ? manifest.name.trim() : className;
    const displayName = typeof manifest?.displayName === 'string' && manifest.displayName.trim().length > 0
      ? manifest.displayName.trim()
      : pluginName;
    const priority = typeof manifest?.priority === 'number' && Number.isFinite(manifest.priority) ? manifest.priority : 0;

    pluginDefinitionsEntries.push(`  {
    ...
    priority: ${priority},
    dependencies: ${JSON.stringify(normalizedDeps)},
  },`);
...
function buildUiLoaderModule(entries: string[], loadOrder: string[]) {
...
`;
  ...
  const pluginsArray = JSON.stringify([...loadOrder].sort());
  const loadOrderArray = JSON.stringify(loadOrder);
...
export const uiPlugins = ${pluginsArray} as const;
export const uiLoadOrder = ${loadOrderArray} as const;
...
async function main() {
  ...
  const uiLoaderModuleContent = buildUiLoaderModule(uiLoaderEntries, uiLoadOrder);

  const workerContent = `${workerRegistrations.join('\n\n')}
`;
  const registryIndexContent = `export const pluginDefinitions = [
${pluginDefinitionsEntries.join('\n')}
];

export const pluginRegistry = [
${pluginRegistryEntries.join('\n')}
];

export const pluginMapUI = {
${uiLoaderEntries.join('\n')}
};

export const pluginMapWorker = {
${workerLoaderEntries.join('\n')}
};
`;
  ...
  writes.push({ file: outUiFile, changed: await writeFileIfChanged(outUiFile, uiLoaderModuleContent) });
